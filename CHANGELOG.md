...
- Documents ecc as the reusable cross-harness substrate across OpenAI Codex, Codex, Cursor, OpenCode, and Gemini.
...
- The Codex plugin remains limited by platform-level rules distribution constraints; the selective install / OSS path is still the most reliable full install.
...
- `codex-devfleet` — Multi-agent orchestration (#505)
- `blueprint` — Multi-session construction planning
- `Everything OpenAI Codex` — Self-referential ecc skill (#335)
...
- Observer sandbox access fix for Fast invocation (#661)
