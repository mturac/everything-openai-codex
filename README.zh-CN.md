...
本仓库是当前事实来源。请使用仓库内维护的文档，不再依赖外部视频或推文线程。

| 文档 | 何时使用 |
|------|----------|
| [Hermes 设置](docs/HERMES-SETUP.md) | 配置 Hermes x ecc operator workflow |
| [rc.1 发布说明](docs/releases/2.0.0-rc.1/release-notes.md) | 了解当前公开 release surface |
| [跨 harness 架构](docs/architecture/cross-harness.md) | 在 harness 之间移植 skills、rules 和 adapters |
| [令牌优化](docs/token-optimization.md) | 调整 Codex model、compaction 和 cost 设置 |
| [安全指南](the-security-guide.md) | 检查 sandboxing、hook risks 和 AgentShield |
...
**`--deep` 参数**：启动 3 个 Codex Deep 4.6 智能体组成红队/蓝队/审计管道。攻击者寻找利用链，防御者评估防护机制，审计者综合生成优先级风险报告。采用对抗推理，而非单纯模式匹配。
...
- **Hermes 设置：** [docs/HERMES-SETUP.md](docs/HERMES-SETUP.md)
- **发布说明：** [docs/releases/2.0.0-rc.1/release-notes.md](docs/releases/2.0.0-rc.1/release-notes.md)
