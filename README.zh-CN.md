...
**来自 OpenAI 黑客马拉松获胜者的完整 OpenAI Codex 配置集合。**
...
可在 **OpenAI Codex**、**Codex**、**Cursor**、**OpenCode**、**Gemini** 及其他 AI 智能体框架中通用。
...
<img src="https://github.com/user-attachments/assets/1a471488-59cc-425b-8345-5245c7efbcef" alt="The Shorthand Guide to Everything OpenAI Codex" />
</a>
</td>
<td width="33%">
<a href="https://x.com/mehmet_turac/status/2014040193557471352">
<img src="https://github.com/user-attachments/assets/c9ca43bc-b149-427f-b551-af6840c368f0" alt="The Longform Guide to Everything OpenAI Codex" />
</a>
</td>
<td width="33%">
...
> 注意：插件安装方式较为便捷，但如果你的 OpenAI Codex 版本无法正常解析自托管市场条目，建议使用下方的开源安装脚本，稳定性更高。
...
> 安装名称说明：较早的帖子里可能还会出现较长的旧标识符。OpenAI 的 marketplace/plugin 安装是按规范化插件标识符寻址的，因此 ecc 现在统一为 `ecc@ecc`，让工具名和 slash command 命名空间保持简短。
...
或在 OpenAI Codex 中使用 `/setup-pm` 命令。
...
这个仓库是一个 **OpenAI Codex 插件** - 直接安装或手动复制组件。

```
Everything OpenAI Codex/
|-- .codex-plugin/   # 插件与应用商店清单
...
|   |-- saas-nextjs-CODEX.md   # 真实 SaaS 项目（Next.js + Supabase + Stripe）
|   |-- go-microservice-CODEX.md # 真实 Go 微服务（gRPC + PostgreSQL）
|   |-- django-api-CODEX.md      # 真实 Django REST API（DRF + Celery）
|   |-- laravel-api-CODEX.md     # 真实 Laravel API（PostgreSQL + Redis）（新增）
|   |-- rust-api-CODEX.md        # 真实 Rust API（Axum + SQLx + PostgreSQL）（新增）
|
...
两个选项都创建：
- **SKILL.md 文件** - 可直接用于 OpenAI Codex 的技能
- **直觉集合** - 用于 continuous-learning-v2
- **模式提取** - 从你的提交历史中学习

### AgentShield — 安全审计工具

> 于 OpenAI Codex 黑客松（Cerebral Valley x OpenAI，2026 年 2 月）开发完成。包含 1282 项测试、98% 覆盖率、102 条静态分析规则。

扫描你的 OpenAI Codex 配置，检测漏洞、错误配置与注入风险。
...
**扫描范围：** CODEX.md、settings.json、MCP 配置、钩子、智能体定义与技能模块，覆盖 5 大类别 —— 密钥检测（14 种模式）、权限审计、钩子注入分析、MCP 服务风险评估、智能体配置审查。

**`--opus` 参数**：启动 3 个 Codex Opus 4.6 智能体组成红队/蓝队/审计管道。攻击者寻找利用链，防御者评估防护机制，审计者综合生成优先级风险报告。采用对抗推理，而非单纯模式匹配。

**输出格式：** 终端（彩色等级 A-F）、JSON（CI 流水线）、Markdown、HTML。发现严重问题时返回退出码 2，可用于构建门禁。

在 OpenAI Codex 中使用 `/security-scan` 运行，或通过 [GitHub Action](https://github.com/mehmet-turac/agentshield) 集成到 CI。
...
由于插件系统处理钩子的机制发生变更，本插件要求 OpenAI Codex CLI 版本不低于 v2.1.0。
...
### 重要提示：钩子自动加载机制
> 警告：**贡献者请注意**：请勿在 `.codex-plugin/plugin.json` 中添加 `"hooks"` 字段。回归测试已强制禁止该操作。

OpenAI Codex v2.1+ 会**按照约定自动加载**已安装插件中的 `hooks/hooks.json`。若在 `plugin.json` 中显式声明该文件，会触发重复检测错误：
```
...
使用此仓库的最简单方法 - 作为 OpenAI Codex 插件安装：
...
> **注意：** OpenAI Codex 插件系统不支持通过插件分发 `rules`（[上游限制](https://code.openai-codex.com/docs/en/plugins-reference)）。你需要手动安装规则：
>
> ```bash
> # 首先克隆仓库
> git clone https://github.com/mehmet-turac/everything-openai-codex.git
...
> # 方案 A：用户级规则（对所有项目生效）
> mkdir -p ~/.codex/rules
> cp -r Everything OpenAI Codex/rules/common ~/.codex/rules/
> cp -r Everything OpenAI Codex/rules/typescript ~/.codex/rules/   # 选择你使用的技术栈
> cp -r Everything OpenAI Codex/rules/python ~/.codex/rules/
> cp -r Everything OpenAI Codex/rules/golang ~/.codex/rules/
> cp -r Everything OpenAI Codex/rules/php ~/.codex/rules/
>
> # 方案 B：项目级规则（仅对当前项目生效）
> mkdir -p .codex/rules
> cp -r Everything OpenAI Codex/rules/common .codex/rules/
...
仅适用于手动安装：如果你没有通过 Codex 插件方式安装 ecc，可以将 `hooks/hooks.json` 中的钩子配置复制到你的 `~/.codex/settings.json` 文件中。

如果你是通过 `/plugin install` 安装 ecc，请不要再把这些钩子复制到 `settings.json`。OpenAI Codex v2.1+ 会自动加载插件中的 `hooks/hooks.json`，重复注册会导致重复执行以及 `${CODEX_PLUGIN_ROOT}` 无法解析。

#### 配置 MCP 服务
从 `mcp-configs/mcp-servers.json` 中复制需要的 MCP 服务定义，粘贴到官方 OpenAI Codex 配置文件 `~/.codex/settings.json` 中；
...
基于 Everything OpenAI Codex 构建或受其启发的项目：
...
[成为赞助者](https://github.com/sponsors/mehmet-turac) | [赞助档位](SPONSORS.md) | [赞助计划](SPONSORING.md)
...
- **快速上手指南（入门首选）：** [Everything OpenAI Codex 简明指南](https://x.com/mehmet_turac/status/2012378465664745795)
- **长文指南（高阶进阶）：** [Everything OpenAI Codex 完整版深度指南](https://x.com/mehmet_turac/status/2014040193557471352)
- **安全指南：** [安全指南](./the-security-guide.md) | [推文详解](https://x.com/mehmet_turac/status/2033263813387223421)
- **关注作者：** [@mehmet_turac](https://x.com/mehmet_turac)
