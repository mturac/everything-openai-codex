...
Launch an Agent (subagent_type: `code-reviewer`, model: `deep`) with the full rubric + all files under review. The prompt must include:
- The complete rubric
...
Launch a second Codex Agent (subagent_type: `code-reviewer`, model: `deep`). Log a warning that both reviewers share the same model family — true model diversity was not achieved but context isolation is still enforced.
...
Reviewer A (Codex Deep):   [PASS/FAIL]
...
- Reviewer A (Codex Deep) always runs — guarantees at least one strong reviewer regardless of tooling.
...
- Strongest available models are used: Deep for Reviewer A, GPT-5.4 or Gemini 2.5 Pro for Reviewer B.
