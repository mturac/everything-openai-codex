---
description: Save current session state to a dated file in ~/.codex/session-data/ so work can be resumed in a future session with full context.
---
...
- End of a work session before closing OpenAI Codex
...
Create `~/.codex/session-data/YYYY-MM-DD-<short-id>-session.tmp`, using today's actual date and a short-id that satisfies the rules enforced by `SESSION_FILENAME_REGEX` in `session-manager.js`:
