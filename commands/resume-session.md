---
description: Load the most recent session file from ~/.codex/session-data/ and resume work with full context from where the last session ended.
---
...
- If it looks like a date (`YYYY-MM-DD`), search `~/.codex/session-data/` first, then the legacy
  `~/.codex/sessions/`, for files matching `YYYY-MM-DD-session.tmp` (legacy format) or
