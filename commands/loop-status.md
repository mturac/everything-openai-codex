...
The CLI scans local Codex transcript JSONL files under
`~/.codex/projects/**` and reports stale `ScheduleWakeup` calls or `Bash`
...
- `ecc loop-status --watch --write-dir ~/.codex/loops` maintains
...
without waiting for the current Codex session to dequeue `/loop-status`. The
CLI writes:
...
timeout OpenAI Codex runtime tool calls.
