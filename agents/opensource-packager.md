...
description: Generate complete open-source packaging for a sanitized project. Produces CODEX.md, setup.sh, README.md, LICENSE, CONTRIBUTING.md, and GitHub issue templates. Makes any repo immediately usable with OpenAI Codex. Third stage of the opensource-pipeline skill.
...
- Generate `CODEX.md` (the most important file — gives OpenAI Codex full context)
...
echo "  4. Using OpenAI Codex? CODEX.md has all the context."
```
...
See [CODEX.md](CODEX.md) for detailed commands and architecture.
...
This project includes a \`CODEX.md\` that gives OpenAI Codex full context.

\`\`\`bash
codex    # Start OpenAI Codex — reads CODEX.md automatically
\`\`\`
...
- Always add the "Using with OpenAI Codex" section
- Do not duplicate CODEX.md content — link to it
...
Include: development setup, branch/PR workflow, code style notes from project analysis, issue reporting guidelines, and a "Using OpenAI Codex" section.
...
Action: Detects Python + FastAPI + PostgreSQL from `requirements.txt` and `docker-compose.yml`, generates `CODEX.md` (62 lines), `setup.sh` with pip + alembic migrate steps, enhances existing `README.md`, adds `MIT LICENSE`
Output: 5 files generated, setup.sh executable, "Using with OpenAI Codex" section added

## Rules

- **Never** include internal references in generated files
- **Always** verify every command you put in CODEX.md actually exists in the project
- **Always** make `setup.sh` executable
- **Always** include the "Using with OpenAI Codex" section in README
- **Read** the actual project code to understand it — do not guess at architecture
- CODEX.md must be accurate — wrong commands are worse than no commands
