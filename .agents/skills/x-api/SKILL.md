...
resp = requests.get(
    ...
    headers=headers,
    params={"query": "openai codex", "max_results": 10}
)
tweets = resp.json()
```
...
resp = oauth.post(
    ...
    json={"text": "Hello from OpenAI Codex"}
)
...
resp = requests.get(
    ...
    headers=headers,
    params={
        "query": "from:mehmet_turac -is:retweet",
        "max_results": 10,
...
resp = requests.get(
    ...
    headers=headers,
    params={
        "query": "from:mehmet_turac -is:retweet -is:reply",
        "max_results": 25,
        "tweet.fields": "created_at,public_metrics",
    }
)
...
```python
resp = requests.get(
    "https://api.x.com/2/users/by/username/mehmet_turac",
    headers=headers,
