...
![标题：OpenAI 黑客马拉松获胜者 - OpenAI Codex 技巧与窍门](../../assets/images/shortform/00-header.png)

***

**自 2 月实验性推出以来，我一直是 OpenAI Codex 的忠实用户，并凭借 [zenith.chat](https://zenith.chat) 与 [@DRodriguezFX](https://x.com/DRodriguezFX) 一起赢得了 OpenAI x Forum Ventures 的黑客马拉松——完全使用 OpenAI Codex。**
...
在使用 Deep 4.5 长时间编码后，你想清理死代码和松散的 .md 文件吗？运行 `/refactor-clean`。需要测试吗？`/tdd`、`/e2e`、`/test-coverage`。技能也可以包含代码地图——一种让 Codex 快速浏览你的代码库而无需消耗上下文进行探索的方式。
...
*在 OpenAI Codex 中运行 PostToolUse 钩子时获得的反馈示例*
...
1. **单一 CODEX.md** - 所有内容在一个文件中（用户或项目级别）
...
**LSP 插件** 如果你经常在编辑器之外运行 OpenAI Codex，则特别有用。语言服务器协议为 Codex 提供实时类型检查、跳转到定义和智能补全，而无需打开 IDE。
...
你的编辑器选择显著影响 OpenAI Codex 的工作流。虽然 OpenAI Codex 可以在任何终端中工作，但将其与功能强大的编辑器配对可以解锁实时文件跟踪、快速导航和集成命令执行。

### Zed（我的偏好）

我使用 [Zed](https://zed.dev) —— 用 Rust 编写，所以它真的很快。立即打开，轻松处理大型代码库，几乎不占用系统资源。

**为什么 Zed + OpenAI Codex 是绝佳组合：**
...
* **最小的资源使用** - 在繁重操作期间不会与 Codex 竞争 RAM/CPU。运行 Deep 时很重要
...
1. **分割你的屏幕** - 一侧是带 OpenAI Codex 的终端，另一侧是编辑器
...
这也是一个可行的选择，并且与 OpenAI Codex 配合良好。你可以使用终端格式，通过 `\ide` 与你的编辑器自动同步以启用 LSP 功能（现在与插件有些冗余）。或者你可以选择扩展，它更集成于编辑器并具有匹配的 UI。

![VS Code OpenAI Codex 扩展](../../assets/images/shortform/10-vscode-extension.jpeg)
*VS Code 扩展为 OpenAI Codex 提供了原生图形界面，直接集成到你的 IDE 中。*
...
```markdown
ralph-wiggum@codex-plugins       # 循环自动化
frontend-patterns@codex-plugins  # UI/UX 模式
commit-commands@codex-plugins    # Git 工作流
security-guidance@codex-plugins  # 安全检查
pr-review-toolkit@codex-plugins  # PR 自动化
typescript-lsp@codex-plugins-official # TS 智能
hookify@codex-plugins-official        # Hook 创建
...
context7@codex-plugins-official       # 实时文档
pyright-lsp@codex-plugins-official    # Python 类型
...
~/.codex/rules/
  ...
  agents.md        # 子代理委托规则
  patterns.md      # API响应格式
  performance.md   # 模型选择（Fast vs Standard vs Deep）
  hooks.md         # 钩子文档
```
...
* [插件参考](https://code.openai-codex.com/docs/en/plugins-reference)
* [钩子文档](https://code.openai-codex.com/docs/en/hooks)
* [检查点](https://code.openai-codex.com/docs/en/checkpointing)
* [交互模式](https://code.openai-codex.com/docs/en/interactive-mode)
* [记忆系统](https://code.openai-codex.com/docs/en/memory)
* [子代理](https://code.openai-codex.com/docs/en/sub-agents)
* [MCP 概述](https://code.openai-codex.com/docs/en/mcp-overview)
...
*在纽约与 [@DRodriguezFX](https://x.com/DRodriguezFX) 一起构建 [zenith.chat](https://zenith.chat) 赢得了 OpenAI x Forum Ventures 黑客马拉松*
