...
使用下面的模板编写完整的计划文档。保存到 `.codex/PRPs/plans/{kebab-case-feature-name}.plan.md`。
...
- **文件**：.codex/PRPs/plans/{kebab-case-feature-name}.plan.md
- **来源PRD**：[路径或“N/A”]
- **阶段**：[阶段名称或“独立”]
- **复杂度**：[级别]
- **范围**：[N个文件，M个任务]
...
> 下一步：运行 `/prp-implement .codex/PRPs/plans/{name}.plan.md` 来执行此计划。
```
