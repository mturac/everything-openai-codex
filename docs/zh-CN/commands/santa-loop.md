...
针对当前任务输出，运行两个独立的评审者（Codex Deep + 一个外部模型）。两者都必须返回 NICE 后才能推送代码。如果任一返回 NAUGHTY，则修复所有标记的问题，提交，并重新运行全新的评审者——最多 3 轮。
...
#### 评审者 A：Codex Agent（始终运行）

启动一个 Agent（subagent\_type: `code-reviewer`，model: `deep`），包含完整的评分标准 + 所有被审查的文件。提示必须包括：
...
#### 评审者 B：外部模型（仅当未安装外部 CLI 时回退到 Codex）
...
**Codex Agent 回退**（仅当 `codex` 和 `gemini` 均未安装时）
启动第二个 Codex Agent（subagent\_type: `code-reviewer`，model: `deep`）。记录一条警告，说明两个评审者共享相同的模型家族——未实现真正的模型多样性，但上下文隔离仍然得到强制执行。
...
* 评审者 A（Codex Deep）始终运行——无论工具如何，保证至少有一个强大的评审者。
* 模型多样性是评审者 B 的目标。GPT-5.4 或 Gemini 2.5 Pro 提供真正的独立性——不同的训练数据、不同的偏见、不同的盲点。仅 Codex 的回退通过上下文隔离仍然提供价值，但失去了模型多样性。
* 使用最强可用模型：Deep 用于评审者 A，GPT-5.4 或 Gemini 2.5 Pro 用于评审者 B。
