---
description: 通过Codex DevFleet协调并行OpenAI Codex代理——从自然语言规划项目，在隔离的工作树中调度代理，监控进度，并读取结构化报告。
---

# DevFleet — 多智能体编排

通过 Codex DevFleet 编排并行的 OpenAI Codex 智能体。每个智能体在隔离的 git worktree 中运行，并配备完整的工具链。

需要 DevFleet MCP 服务器：`codex mcp add devfleet --transport http http://localhost:18801/mcp`
