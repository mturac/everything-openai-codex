---
description: 管理OpenAI Codex会话历史、别名和会话元数据。
---

# Sessions 命令

管理 OpenAI Codex 会话历史 - 列出、加载、设置别名和编辑存储在 `~/.codex/session-data/` 中的会话，同时兼容读取旧的 `~/.codex/sessions/` 文件。
...
* 会话以 Markdown 文件形式存储在 `~/.codex/session-data/`，并继续兼容读取旧的 `~/.codex/sessions/`
