...
**AgentShield。** 5 个类别共 102 条规则。扫描 OpenAI Codex 配置、钩子、MCP 服务器、权限和智能体定义。附带一个由 Codex Deep 驱动的 3 智能体对抗管道（红队/蓝队/审计员），用于发现静态规则遗漏的链式漏洞利用。通过 GitHub Action 原生支持 CI/CD。对于 OpenAI Codex 用户来说是最全面的选择。
