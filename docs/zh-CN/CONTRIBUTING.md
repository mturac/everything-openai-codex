# 为 Everything OpenAI Codex 做贡献

感谢您想要贡献！这个仓库是 OpenAI Codex 用户的社区资源。
...
| `model` | 复杂度级别 | `fast`（简单），`standard`（编码），`deep`（复杂） |
...
* **mcp-configs/mcp-servers.json** 包含一个 Context7 条目；用户在其工具链（例如 OpenAI Codex, Cursor）中启用它，以使用文档查找技能（位于 `skills/documentation-lookup/`）和 `/docs` 命令。
