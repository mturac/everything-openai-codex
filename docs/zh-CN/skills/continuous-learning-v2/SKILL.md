...
| 特性 | v2.0 | v2.1 |
|---------|------|------|
| 存储 | 全局 (~/.codex/homunculus/) | 项目作用域 (projects/<hash>/) |
...
1. **`CODEX_PROJECT_DIR` 环境变量** (最高优先级)
...
每个项目都会获得一个 12 字符的哈希 ID (例如 `a1b2c3d4e5f6`)。`~/.codex/homunculus/projects.json` 处的注册表文件将 ID 映射到人类可读的名称。
...
不需要在 `~/.codex/settings.json` 中额外添加 hooks。OpenAI Codex v2.1+ 会自动加载插件的 `hooks/hooks.json`，其中已经注册了 `observe.sh`。

如果您之前把 `observe.sh` 复制到了 `~/.codex/settings.json`，请删除重复的 `PreToolUse` / `PostToolUse` 配置。重复注册会导致重复执行，并触发 `${CODEX_PLUGIN_ROOT}` 解析错误，因为该变量只会在插件自己的 `hooks/hooks.json` 中展开。

**如果手动安装**到 `~/.codex/skills`，请将以下内容添加到 `~/.codex/settings.json`：
...
> "v1 依赖技能来观察。技能是概率性的 -- 根据 Codex 的判断，它们触发的概率约为 50-80%。"
...
* `~/.codex/homunculus/instincts/` 中现有的全局本能仍然作为全局本能工作
* 来自 v1 的现有 `~/.codex/skills/learned/` 技能仍然有效
...
* [长篇指南](https://x.com/mehmet_turac/status/2014040193557471352) - 持续学习部分

***

*基于本能的学习：一次一个项目，教会 Codex 您的模式。*
