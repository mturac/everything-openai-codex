---
name: observer
description: 分析会话观察以检测模式并创建本能的背景代理。使用Fast以实现成本效益。v2.1版本增加了项目范围的本能。
model: fast
---

# Observer Agent

一个后台代理，用于分析 OpenAI Codex 会话中的观察结果，以检测模式并创建本能。
...
* 项目：`~/.codex/homunculus/projects/<project-hash>/observations.jsonl`
* 全局后备：`~/.codex/homunculus/observations.jsonl`
...
* 项目：`~/.codex/homunculus/projects/<project-hash>/instincts/personal/`
* 全局：`~/.codex/homunculus/instincts/personal/`（用于通用模式）
