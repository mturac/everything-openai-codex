---
name: ck
description: OpenAI Codex 的每个项目持久化记忆。在会话启动时自动加载项目上下文，通过 git 活动追踪会话，并写入原生记忆。命令运行确定性的 Node.js 脚本——行为在不同模型版本间保持一致。
origin: community
version: 2.0.0
author: sreedhargs89
...
脚本位于：`~/.codex/skills/ck/commands/`（使用 `$HOME` 展开 `~`）。
...
位于 `~/.codex/skills/ck/hooks/session-start.mjs` 的钩子必须在
`~/.codex/settings.json` 中注册，以便在会话启动时自动加载项目上下文：
...
未保存的会话、自上次保存以来的 git 活动，以及与 CODEX.md 的目标不匹配。
