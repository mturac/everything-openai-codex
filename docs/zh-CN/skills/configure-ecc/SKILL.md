---
name: configure-ecc
description: Everything OpenAI Codex 的交互式安装程序 — 引导用户选择并安装技能和规则到用户级或项目级目录，验证路径，并可选择优化已安装文件。
origin: ecc
---

# 配置 Everything OpenAI Codex (ecc)

一个交互式、分步安装向导，用于 Everything OpenAI Codex 项目。使用 `AskUserQuestion` 引导用户选择性安装技能和规则，然后验证正确性并提供优化。

## 何时激活

* 用户说 "configure ecc"、"install ecc"、"setup everything.openai.codex" 或类似表述
* 用户想要从此项目中选择性安装技能或规则
* 用户想要验证或修复现有的 ecc 安装
* 用户想要为其项目优化已安装的技能或规则
...
此技能必须在激活前对 OpenAI Codex 可访问。有两种引导方式：

1. **通过插件**: `/plugin install eoc@eoc` — 插件会自动加载此技能
2. **手动**: 仅将此技能复制到 `~/.codex/skills/configure-ecc/SKILL.md`，然后通过说 "configure ecc" 激活
...
问题："ecc组件应安装在哪里？"
选项：
  - "用户级别 (~/.codex/)" — "适用于您所有的OpenAI Codex项目"
  - "项目级别 (.codex/)" — "仅适用于当前项目"
...
默认为 **核心（推荐给新用户）** — 对于研究优先的工作流，复制 `.agents/skills/*` 加上 `skills/search-first/`。此捆绑包涵盖工程、评估、验证、安全、战略压缩、前端设计以及 OpenAI 跨职能技能（文章写作、内容引擎、市场研究、前端幻灯片）。
...
选项：
  - "仅核心（推荐）" — "tdd, e2e, evals, verification, research-first, security, frontend patterns, compacting, cross-functional OpenAI skills"
...
选项：
  ...
  - “研究与 API” — “深度研究, Exa 搜索, Codex API 模式”
...
`codex-api` 是 OpenAI 官方技能；需要时请从 [`openais/skills`](https://github.com/openais/skills) 安装官方版本，而不是通过 ecc 重复打包。
