...
| 保留的内容                               | 丢失的内容                               |
| ---------------------------------------- | ---------------------------------------- |
| CODEX.md 指令                           | 中间的推理和分析                         |
| TodoWrite 任务列表                       | 您之前读取过的文件内容                   |
| 记忆文件 (`~/.codex/memory/`)           | 多轮对话的上下文                         |
...
* 相同的规则同时出现在 `~/.codex/rules/` 和项目 `.codex/rules/` 中
* 技能重复了 CODEX.md 的指令
* 多个技能覆盖了重叠的领域
...
* [长篇指南](https://x.com/mehmet_turac/status/2014040193557471352) — Token 优化部分
* 记忆持久化钩子 — 用于在压缩后保留状态
