...
description: "用于审计Codex技能和命令的质量。支持快速扫描（仅变更技能）和全面盘点模式，采用顺序子代理批量评估。"
origin: ecc
---

# skill-stocktake

斜杠命令 (`/skill-stocktake`)，用于使用质量检查清单 + AI 整体判断来审核所有 Codex 技能和命令。支持两种模式：用于最近更改技能的快速扫描，以及用于完整审查的全面盘点。
...
如果项目没有 `.codex/skills/` 目录，则只评估全局技能和命令。
...
2. 运行：`bash ~/.codex/skills/skill-stocktake/scripts/quick-diff.sh \   ~/.codex/skills/skill-stocktake/results.json`
   （项目目录从 `$PWD/.codex/skills` 自动检测；仅在需要时显式传递）
...
* **独特性**：价值不能被 MEMORY.md / CODEX.md / 其他技能取代
* **时效性**：技术引用在当前环境中有效
