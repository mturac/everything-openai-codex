...
通过三阶段流水线安全地开源任何项目：**分叉**（剥离密钥）→ **净化**（验证清洁）→ **打包**（CODEX.md + setup.sh + README）。
...
| `/opensource package PROJECT` | 生成 CODEX.md + setup.sh + README |
...
显示每个项目及其流水线进度（FORK\_REPORT.md、SANITIZATION\_REPORT.md、CODEX.md 是否存在）。
