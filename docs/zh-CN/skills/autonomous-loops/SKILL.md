...
description: "自主Codex代码循环的模式与架构——从简单的顺序管道到基于RFC的多智能体有向无环图系统。"
origin: ecc
---
...
在循环中自主运行 OpenAI Codex 的模式、架构和参考实现。涵盖从简单的 `codex -p` 管道到完整的 RFC 驱动的多智能体 DAG 编排的一切。
...
| 模式 | 复杂度 | 最适合 |
|---------|-----------|----------|
| [顺序管道](#1-顺序管道-codex--p) | 低 | 日常开发步骤，脚本化工作流 |
| [NanoClaw REPL](#2-nanoclaw-repl) | 低 | 交互式持久会话 |
| [无限智能体循环](#3-无限智能体循环) | 中 | 并行内容生成，规范驱动的工作 |
| [持续 Codex PR 循环](#4-持续-codex-pr-循环) | 中 | 具有 CI 门的跨天迭代项目 |
...
**最简单的循环。** 将日常开发分解为一系列非交互式 `codex -p` 调用。每次调用都是一个具有清晰提示的专注步骤。

### 核心见解

> 如果你无法想出这样的循环，那意味着你甚至无法在交互模式下驱动 LLM 来修复你的代码。

`codex -p` 标志以非交互方式运行 OpenAI Codex 并附带提示，完成后退出。链式调用来构建管道：
...
1. **每个步骤都是隔离的** — 每次 `codex -p` 调用都是一个新的上下文窗口，意味着步骤之间没有上下文泄露。
...
codex -p --model deep "Analyze the codebase architecture and write a plan for adding caching..."

# Implement with Standard (fast, capable)
codex -p "Implement the caching layer according to the plan in docs/caching-plan.md..."

# Review with Deep (thorough)
codex -p --model deep "Review all changes for security issues, race conditions, and edge cases..."
```
...
**ecc 内置的持久循环。** 一个具有会话感知的 REPL，它使用完整的对话历史同步调用 `codex -p`。
...
1. 从 `~/.codex/claw/{session}.md` 加载对话历史
2. 每个用户消息都连同完整历史记录作为上下文发送给 `codex -p`
...
### 通过 OpenAI Codex 命令实现

创建 `.codex/commands/infinite.md`：
...
**一个生产级的 shell 脚本**，在持续循环中运行 OpenAI Codex，创建 PR，等待 CI，并自动合并。由 AnandChowdhary 创建（致谢：@AnandChowdhary）。
...
│  持续 CODEX 迭代                                   │
│                                                     │
│  1. 创建分支 (continuous-codex/iteration-N)       │
│  2. 使用增强提示运行 codex -p                      │
│  3. (可选) 审查者通过 — 单独的 codex -p            │
│  4. 提交更改 (codex 生成提交信息)                  │
│  5. 推送 + 创建 PR (gh pr create)                   │
│  6. 等待 CI 检查 (轮询 gh pr checks)                │
│  7. CI 失败？ → 自动修复通过 (codex -p)             │
...
> **警告：** 请在审阅代码后，从 continuous-codex 的仓库安装。不要将外部脚本直接管道传入 bash。
...
Codex 在迭代开始时读取此文件，并在迭代结束时更新它。这弥合了独立 `codex -p` 调用之间的上下文差距。

### CI 失败恢复

当 PR 检查失败时，持续 Codex 会自动：

1. 通过 `gh run list` 获取失败的运行 ID
2. 生成一个新的带有 CI 修复上下文的 `codex -p`
3. Codex 通过 `gh run view` 检查日志，修复代码，提交，推送
4. 重新等待检查（最多 `--ci-retry-max` 次尝试）

### 完成信号

Codex 可以通过输出一个魔法短语来发出“我完成了”的信号：

```bash
continuous-codex \
  --prompt "Fix all bugs in the issue tracker" \
  --completion-signal "CONTINUOUS_CODEX_PROJECT_COMPLETE" \
...
| 阶段 | 模型 | 目的 |
|-------|-------|---------|
| Research | Standard | 读取代码库 + RFC，生成上下文文档 |
| Plan | Deep | 设计实现步骤 |
| Implement | Codex | 按照计划编写代码 |
| Test | Standard | 运行构建 + 测试套件 |
| PRD Review | Standard | 规范合规性检查 |
| Code Review | Deep | 质量 + 安全检查 |
| Review Fix | Codex | 处理审阅问题 |
| Final Review | Deep | 质量门（仅限大型层级） |
...
2. **连续 Codex + 去草率化** — 为每次迭代添加带有去草率化指令的 `--review-prompt`。

3. **任何循环 + 验证** — 在提交前，使用 ecc 的 `/verify` 命令或 `verification-loop` 技能作为关卡。

4. **Ralphinho 在简单循环中的分层方法** — 即使在顺序流水线中，你也可以将简单任务路由到 Fast，复杂任务路由到 Deep：
   ```bash
   # 简单的格式修复
   codex -p --model fast "Fix the import ordering in src/utils.ts"

   # 复杂的架构变更
   codex -p --model deep "Refactor the auth module to use the strategy pattern"
   ```
...
2. **迭代之间没有上下文桥接** — 每次 `codex -p` 调用都从头开始。使用 `SHARED_TASK_NOTES.md` 或文件系统状态来桥接上下文。
