...
1. 检查工作目录中是否存在 `CODEX.md`——读取它以了解项目惯例
2. 从项目文件中检测技术栈：
...
| 范围 | 推荐模型 | 理由 |
|-------|------------------|-----------|
| 微小-低 | Standard 4.6 | 快速、成本效益高，适合简单任务 |
| 中 | Standard 4.6 | 标准工作的最佳编码模型 |
| 高 | Standard 4.6 (主) + Deep 4.6 (规划) | Deep 用于架构，Standard 用于实现 |
| 史诗级 | Deep 4.6 (蓝图) + Standard 4.6 (执行) | 深度推理用于多会话规划 |
...
推荐：使用 Deep 4.6 进行蓝图规划，使用 Standard 4.6 执行各阶段。
```
