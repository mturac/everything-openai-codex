---
name: dmux-workflows
description: 使用dmux（AI代理的tmux窗格管理器）进行多代理编排。跨OpenAI Codex、Codex、OpenCode及其他工具的并行代理工作流模式。适用于并行运行多个代理会话或协调多代理开发工作流时。
origin: ecc
---
...
* 并行运行多个代理会话时
* 跨 OpenAI Codex、Codex 和其他套件协调工作时
* 需要分而治之并行处理的复杂任务
...
* 按 `n` 创建一个带有提示的新窗格
* 按 `m` 将窗格输出合并回主会话
* 支持：OpenAI Codex、Codex、OpenCode、Cline、Gemini、Qwen
...
| **OpenAI Codex Task 工具** | 进程内子代理生成 | 会话内的程序化并行 |
