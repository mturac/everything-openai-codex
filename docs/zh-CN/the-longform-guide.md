# 关于 OpenAI Codex 的完整长篇指南

![Header: The Longform Guide to Everything OpenAI Codex](../../assets/images/longform/01-header.png)

***

> **前提**：本指南建立在 [关于 OpenAI Codex 的简明指南](the-shortform-guide.md) 之上。如果你还没有设置技能、钩子、子代理、MCP 和插件，请先阅读该指南。

![Reference to Shorthand Guide](../../assets/images/longform/02-shortform-reference.png)
*速记指南 - 请先阅读此指南*

在简明指南中，我介绍了基础设置：技能和命令、钩子、子代理、MCP、插件，以及构成有效 OpenAI Codex 工作流骨干的配置模式。那是设置指南和基础架构。
...
简明指南和长篇指南中涵盖的所有内容都可以在 GitHub 上找到：`github.com/mturac/everything-openai-codex`
...
*会话存储示例 -> <https://github.com/mturac/everything-openai-codex/tree/main/examples/sessions>*
...
一旦你制定了计划并清除了上下文（OpenAI Codex 中计划模式的默认选项），你就可以根据计划工作。当你积累了大量与执行不再相关的探索性上下文时，这很有用。对于策略性压缩，请禁用自动压缩。在逻辑间隔手动压缩，或创建一个为你执行此操作的技能。

**高级：动态系统提示注入**

我学到的一个模式是：与其将所有内容都放在 CODEX.md（用户作用域）或 `.codex/rules/`（项目作用域）中，让它们每次会话都加载，不如使用 CLI 标志动态注入上下文。
...
我已经构建了这些钩子，它们位于仓库的 `github.com/mturac/everything-openai-codex/tree/main/hooks/memory-persistence`
...
**解决方案：** 当 OpenAI Codex 发现一些不平凡的事情时——调试技巧、变通方法、某些项目特定的模式——它会将该知识保存为一个新技能。下次出现类似问题时，该技能会自动加载。

我构建了一个实现此功能的持续学习技能：`github.com/mturac/everything-openai-codex/tree/main/skills/continuous-learning`
...
| 任务类型                 | 模型   | 原因                                       |
| ------------------------- | ------ | ------------------------------------------ |
| 探索/搜索                | Fast  | 快速、便宜，足以用于查找文件               |
| 简单编辑                 | Fast  | 单文件更改，指令清晰                       |
| 多文件实现               | Standard | 编码的最佳平衡                             |
| 复杂架构                 | Deep   | 需要深度推理                               |
| PR 审查                  | Standard | 理解上下文，捕捉细微差别                   |
| 安全分析                 | Deep   | 不能错过漏洞                               |
| 编写文档                 | Fast  | 结构简单                                   |
| 调试复杂错误             | Deep   | 需要将整个系统记在脑中                     |

对于 90% 的编码任务，默认使用 Standard。当第一次尝试失败、任务涉及 5 个以上文件、架构决策或安全关键代码时，升级到 Deep。

**定价参考：**

![Codex Model Pricing](../../assets/images/longform/05-pricing-table.png)
*来源: <https://platform.codex.com/docs/en/about-codex/pricing>*
...
*在我们的 50 个任务基准测试中，mgrep + OpenAI Codex 在相似或更好的判断质量下，使用的 token 数比基于 grep 的工作流少约 2 倍。来源：@mixedbread-ai 的 mgrep*
...
*Boris (OpenAI) 关于运行多个 Codex 实例的说明*
...
当运行多个 OpenAI Codex 实例时，使用“级联”模式进行组织：
...
另请参阅：ccstatusline（用于自定义 OpenAI Codex 状态行的社区项目）

### 语音转录

用你的声音与 OpenAI Codex 对话。对很多人来说比打字更快。
...
* codex-flow — 社区构建的企业级编排平台，包含 54+ 个专业代理

**自我改进记忆：**

* 请参阅本仓库中的 `skills/continuous-learning/`
* rlancemartin.github.io/2025/12/01/codex\_diary/ - 会话反思模式
...
* [OpenAI: 解密 AI 智能体的评估](https://www.openai.com/engineering/demystifying-evals-for-ai-agents)
* [YK: 32 个 OpenAI Codex 技巧](https://agenticcoding.substack.com/p/32-codex-tips-from-basics-to)
* [RLanceMartin: 会话反思模式](https://rlancemartin.github.io/2025/12/01/codex_diary/)
...
*两份指南中涵盖的所有内容都可以在 GitHub 上的 [Everything OpenAI Codex](https://github.com/mturac/everything-openai-codex) 找到*
