...
本文件为 OpenAI Codex (codex.ai/code) 处理此仓库代码时提供指导。

## 项目概述

这是一个 **OpenAI Codex 插件** - 一个包含生产就绪的代理、技能、钩子、命令、规则和 MCP 配置的集合。该项目提供了使用 OpenAI Codex 进行软件开发的经验证的工作流。
...
- 包管理器检测：npm、pnpm、yarn、bun（可通过 `CODEX_PACKAGE_MANAGER` 环境变量或项目配置设置）
