...
* 将 ecc 明确记录为跨 OpenAI Codex、Codex、Cursor、OpenCode 和 Gemini 的可复用 cross-harness 基础层。
...
* Codex plugin 仍受平台级 rules 分发限制影响；selective install / OSS 路径仍是最可靠的完整安装方式。
...
* `codex-devfleet` — 多代理编排 (#505)
* `blueprint` — 多会话构建规划
* `Everything OpenAI Codex` — 自引用 ecc 技能 (#335)
