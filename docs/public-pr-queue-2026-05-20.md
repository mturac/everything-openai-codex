...
I reviewed the public Everything OpenAI Codex PR queue after the rc.1 release-surface cleanup.

## Current State

- Main repo: no open PRs in `mturac/everything-openai-codex`.
- Public listing PRs: 24 open Everything OpenAI Codex PRs.
...
These are clean wait states. I should not bump them unless maintainers ask for changes.
...
I updated the remaining older PR titles/bodies that still used shorthand or pre-EOC naming:
...
I also pushed cleaner listing copy to the open branches for `#3`, `#231`, and `#104`, so the visible diff now starts with `Everything OpenAI Codex`.
