...
| `docs/architecture/cross-harness.md` | Shared substrate model for OpenAI Codex, Codex, OpenCode, Cursor, Gemini, Hermes, and terminal-only use | Names portability boundaries and does not claim unsupported native parity |
...
| `docs/releases/2.0.0-rc.1/naming-and-publication-matrix.md` | Naming, slug, and publication-path decision record | Keeps `Everything OpenAI Codex / ecc`, npm `ecc-universal`, and plugin slug `ecc` for rc.1 |
...
| --- | --- |
| OpenAI Codex | Native plugin, skills, commands, hooks, MCP conventions, and statusline-oriented workflows |
