...
OpenAI Codex remains a core target. Codex, OpenCode, Cursor, Gemini, and other harnesses are treated as execution surfaces that can share the same skills, rules, MCP conventions, and operator workflows. ecc is the reusable substrate; Hermes is documented as the operator shell that can sit on top of that layer.
...
- cross-harness substrate work for OpenAI Codex, Codex, OpenCode, Cursor,
  Gemini, Zed, and terminal-only workflows;
- stronger package and plugin publication surfaces for npm, Codex plugin,
...
ecc is no longer only a OpenAI Codex plugin or config bundle.
...
- cross-harness install surfaces for OpenAI Codex, Codex, OpenCode, Cursor, and related tools
...
publish, Codex plugin tag/marketplace path, Codex Plugin Directory status,
