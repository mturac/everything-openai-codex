...
| PR #1933 | Expanded home-scan IOC coverage to Codex `settings.local.json`, `.codex/hooks/hooks.json`, and user-level VS Code / Code Insiders `tasks.json` across macOS, Linux, and Windows |
...
| AgentShield PR #83 | Merged Mini Shai-Hulud IOC coverage for TanStack, Mistral, OpenSearch, Guardrails, UiPath, Squawk, OpenAI Codex / VS Code persistence, and dead-man switch artifacts |
...
version-pinned package detections, `.codex` / `.vscode` automation-surface
...
OpenAI Codex and VS Code automation paths, including user-level config files that
...
- explicit blockers for GitHub release, npm `next` publish, Codex plugin,
