...
> **Os badges GitHub e npm acima são a fonte de verdade.** ecc é um sistema MIT para workflows OpenAI Codex com catálogo validado e trilha pública rc.1.
...
**O sistema operacional para trabalho sério com OpenAI Codex.**
...
Funciona com **OpenAI Codex**, **Codex**, **Cursor**, **OpenCode**, **Gemini** e outros harnesses de agentes de IA.

---

## Documentação Principal

Esta página é a tradução de entrada. Para detalhes operacionais atuais, use os documentos mantidos no próprio repositório, não threads ou vídeos externos.

| Documento | Quando usar |
|-----------|-------------|
| [Guia de setup Hermes](../HERMES-SETUP.md) | Configurar o fluxo operador Hermes x ecc |
| [Notas da versão rc.1](../releases/2.0.0-rc.1/release-notes.md) | Entender o release público atual |
| [Arquitetura cross-harness](../architecture/cross-harness.md) | Portar skills, regras e adapters entre harnesses |
| [Otimização de tokens](../token-optimization.md) | Ajustar modelo, compactação e custo em Codex |
| [Guia de segurança](../../the-security-guide.md) | Revisar sandboxing, riscos de hooks e AgentShield |
...
- **Crescimento de framework e superfície de produto** — `nestjs-patterns`, superfícies de instalação mais ricas para Codex/OpenCode e melhorias de empacotamento cross-harness ampliam o uso além do OpenAI Codex.
...
- **Paridade entre harnesses** — comportamento unificado em OpenAI Codex, Cursor, OpenCode e Codex app/CLI.
...
> WARNING: **Importante:** Plugins do OpenAI Codex não podem distribuir `rules` automaticamente. Instale-as manualmente:
...
1. **Variável de ambiente**: `CODEX_PACKAGE_MANAGER`
2. **Config do projeto**: `.codex/package-manager.json`
...
5. **Config global**: `~/.codex/package-manager.json`
...
Ou use o comando `/setup-pm` no OpenAI Codex.
...
|-- rules/            # Diretrizes sempre seguidas (copiar para ~/.codex/rules/)
...
Dois modos de gerar skills do OpenAI Codex a partir do seu repositório:
...
> AgentShield é a superfície complementar de auditoria de segurança para ecc. Use os testes do pacote e o catálogo de regras como fonte de verdade para a cobertura atual.
...
# Análise profunda com três agentes Deep 4.6
...
Ou adicione diretamente ao seu `~/.codex/settings.json`:
...
> **Nota:** O sistema de plugins do OpenAI Codex não suporta distribuição de `rules` via plugins. Você precisa instalar as regras manualmente:
>
> ```bash
> # Clone o repositório primeiro
> git clone https://github.com/mturac/everything-openai-codex.git
...
> cp -r Everything OpenAI Codex/rules/typescript ~/.codex/rules/typescript   # escolha sua stack
>
...
Este é o problema mais comum. **NÃO adicione um campo `"hooks"` ao `.codex-plugin/plugin.json`.** O OpenAI Codex v2.1+ carrega automaticamente `hooks/hooks.json` de plugins instalados. Declarar explicitamente causa erros de detecção de duplicatas.
</details>
...
- **OpenAI Codex**: Nativo — este é o alvo principal
</details>
