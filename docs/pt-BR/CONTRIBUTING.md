# Contribuindo para o Everything OpenAI Codex

Obrigado por querer contribuir! Este repositório é um recurso comunitário para usuários do OpenAI Codex.
...
# 4. Testar localmente
cp -r skills/minha-skill ~/.codex/skills/  # para skills
# Em seguida teste com o OpenAI Codex
...
Skills são módulos de conhecimento que o OpenAI Codex carrega baseado no contexto.
...
description: O que este agente faz e quando o Codex deve invocá-lo. Seja específico!
...
| `model` | Nível de complexidade | `fast` (simples), `standard` (codificação), `deep` (complexo) |
...
Hooks são comportamentos automáticos disparados por eventos do OpenAI Codex.
