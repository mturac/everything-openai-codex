...
- The live IOC set includes persistence through OpenAI Codex
  `.codex/settings.json`, VS Code `.vscode/tasks.json`, and OS-level
...
- The scanner also watches for late-reporting markers: `router_init.js`
  ...
  Session protocol strings, `codex@users.noreply.github.com` dead-drop
...
4. Remove persistence hooks before token revocation:
   - `~/.codex/settings.json` `SessionStart` hooks and adjacent
...
6. Confirm npm dist-tag, GitHub release, Codex plugin, Codex plugin, and
...
- `node scripts/ci/scan-supply-chain-iocs.js --home` finds OpenAI Codex,
