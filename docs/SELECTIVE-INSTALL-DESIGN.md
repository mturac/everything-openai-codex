...
3. Keep one consistent UX across Codex, Cursor, Antigravity, Codex, and
   OpenCode.
...
```bash
ecc install --target codex --profile core
```
...
```bash
ecc install --target codex --profile developer --with lang:typescript --with framework:nextjs --without capability:orchestration
```
...
- `target`
  selected harness target such as `codex`, `cursor`, or `antigravity`
- `profile`
