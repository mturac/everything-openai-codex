...
- Codex plugin install/download counts are not currently exposed via a public API.
...
> Codex plugin installs are structurally undercounted publicly, so we use a blended metrics model.
> The project supports OpenAI Codex, Cursor, OpenCode, and Codex app/CLI with production-grade hook reliability and a large passing test suite.
