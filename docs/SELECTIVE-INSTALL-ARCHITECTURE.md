...
- Codex may preserve native hierarchy under `~/.codex`
...
- a target contract
  how those modules land inside Codex, Cursor, Antigravity, Codex, or OpenCode
...
1. Keep `Everything OpenAI Codex` as the canonical source repo.
...
- `developer` is the recommended default for Codex and Cursor
...
```json
{
  ...
  "destination": "/Users/example/.codex/rules/ecc/common/coding-style.md",
  "ownership": "managed",
...
- `--target <codex|cursor|antigravity>`
...
1. extract Codex install behavior into `codex-home` adapter
...
5. Should Codex and OpenCode target adapters ship only after the Codex/Cursor
