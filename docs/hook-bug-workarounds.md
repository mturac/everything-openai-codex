...
Community-tested workarounds for current OpenAI Codex bugs that can affect ecc hook-heavy setups.

This page is intentionally narrow: it collects the highest-signal operational fixes from the longer troubleshooting surface without repeating speculative or unsupported configuration advice. These are upstream OpenAI Codex behaviors, not ecc bugs.
...
- Remove `CODEX_AUTOCOMPACT_PCT_OVERRIDE` if lowering it causes earlier compaction in your build.
...
- If your OpenAI Codex build supports it, add a lightweight `PostCompact` reminder hook that tells you to re-check connector auth.
...
- Route subagent work to a cheaper model such as `CODEX_CODE_SUBAGENT_MODEL=haiku` if your setup exposes that knob.
...
- [issue #644](https://github.com/mehmet-turac/everything-openai-codex/issues/644)
