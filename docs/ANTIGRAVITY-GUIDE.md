...
- **Rules are flattened**: OpenAI Codex nests rules under subdirectories (`rules/common/`, `rules/typescript/`). Antigravity expects a flat `rules/` directory — the installer handles this automatically.
...
| Feature | OpenAI Codex | Cursor | Codex | Antigravity |
|---------|-------------|--------|-------|-------------|
| Install target | `codex-home` | `cursor-project` | `codex-home` | `antigravity` |
| Config root | `~/.codex/` | `.cursor/` | `~/.codex/` | `.agent/` |
