...
| #1478 | HarmonyOS/ArkTS rules, resolver agent, and CODEX example | Salvaged in #1769 with current install wiring; stale `ecc2` session/TUI edits were not carried. |
...
| #1659 | Frontend design direction and interface-polish skills | Salvaged in the May 12 frontend-design maintainer pass with canonical `skills/` layout and current ecc frontend guidance, while preserving the repo guardrail that the official `frontend-design` skill should be installed from `openais/skills`. |
...
| #1478 | Useful HarmonyOS/ArkTS support was already preserved in #1769; current `main` contains the ArkTS rules, resolver agent, CODEX example, and install wiring. |
...
| #1597 | Already present as catalog-count validation for README, AGENTS, zh-CN docs, `.codex-plugin/plugin.json`, and `.codex-plugin/marketplace.json`. |
...
| #1631 | Already present in `scripts/hooks/suggest-compact.js` and `tests/hooks/hooks.test.js`; current code reads `session_id` from stdin JSON before falling back to `CODEX_SESSION_ID`. |
| #1648 | Already present in `src/llm/providers/codex.py`; current Codex provider collects all text and tool-use content blocks and covers the behavior in `tests/test_codex_provider.py`. |
