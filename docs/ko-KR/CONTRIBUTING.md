# Everything OpenAI Codex에 기여하기

기여에 관심을 가져주셔서 감사합니다! 이 저장소는 OpenAI Codex 사용자를 위한 커뮤니티 리소스입니다.
...
# 4. 로컬 테스트
cp -r skills/my-skill ~/.codex/skills/  # 스킬의 경우
# 그런 다음 OpenAI Codex로 테스트
...
스킬은 OpenAI Codex가 컨텍스트에 따라 로드하는 지식 모듈입니다.
...
description: 이 에이전트가 하는 일과 Codex가 언제 호출해야 하는지. 구체적으로 작성!
...
| `model` | 복잡도 수준 | `fast` (단순), `standard` (코딩), `deep` (복잡) |
...
훅은 OpenAI Codex 이벤트에 의해 트리거되는 자동 동작입니다.
