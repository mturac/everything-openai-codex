...
OpenAI Codex 세션을 원자적 "본능(instinct)" -- 신뢰도 점수가 있는 작은 학습된 행동 -- 을 통해 재사용 가능한 지식으로 변환하는 고급 학습 시스템입니다.

**v2.1**에서는 **프로젝트 범위 본능**이 추가되었습니다 -- React 패턴은 React 프로젝트에, Python 규칙은 Python 프로젝트에 유지되며, 범용 패턴(예: "항상 입력 유효성 검사")은 전역으로 공유됩니다.

## 활성화 시점

- OpenAI Codex 세션에서 자동 학습 설정 시
...
| 기능 | v2.0 | v2.1 |
|---------|------|------|
| 저장소 | 전역 (~/.codex/homunculus/) | 프로젝트 범위 (projects/<hash>/) |
...
1. **`CODEX_PROJECT_DIR` 환경 변수** (최우선 순위)
...
각 프로젝트는 12자 해시 ID를 받습니다 (예: `a1b2c3d4e5f6`). `~/.codex/homunculus/projects.json`의 레지스트리 파일이 ID를 사람이 읽을 수 있는 이름에 매핑합니다.

## 빠른 시작

### 1. 관찰 훅 활성화

`~/.codex/settings.json`에 추가하세요.

**플러그인으로 설치한 경우** (권장):

`~/.codex/settings.json`에 추가 hook 블록을 넣지 마세요. OpenAI Codex v2.1+가 플러그인의 `hooks/hooks.json`을 자동으로 로드하며, `observe.sh`는 이미 그곳에 등록되어 있습니다.

이전에 `observe.sh`를 `~/.codex/settings.json`에 복사했다면 중복된 `PreToolUse` / `PostToolUse` 블록을 제거하세요. 중복 등록은 이중 실행과 `${CODEX_PLUGIN_ROOT}` 해석 오류를 일으킵니다. 이 변수는 플러그인 소유 `hooks/hooks.json` 항목에서만 확장됩니다.

**수동으로 `~/.codex/skills`에 설치한 경우**, 아래 내용을 `~/.codex/settings.json`에 추가하세요:
...
> "v1은 관찰에 스킬을 의존했습니다. 스킬은 확률적입니다 -- Codex의 판단에 따라 약 50-80%의 확률로 실행됩니다."
...
- `~/.codex/homunculus/instincts/`의 기존 전역 본능이 전역 본능으로 계속 작동
- v1의 기존 `~/.codex/skills/learned/` 스킬이 계속 작동
...
- [The Longform Guide](https://x.com/mehmet_turac/status/2014040193557471352) - 지속적 학습 섹션

---

*본능 기반 학습: Codex에게 당신의 패턴을 가르치기, 한 번에 하나의 프로젝트씩.*
