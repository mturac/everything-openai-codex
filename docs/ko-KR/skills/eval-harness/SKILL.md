---
name: eval-harness
description: 평가 주도 개발(EDD) 원칙을 구현하는 OpenAI Codex 세션용 공식 평가 프레임워크
origin: ecc
...
OpenAI Codex 세션을 위한 공식 평가 프레임워크로, 평가 주도 개발(EDD) 원칙을 구현합니다.

## 활성화 시점

- AI 지원 워크플로우에 평가 주도 개발(EDD) 설정 시
- OpenAI Codex 작업 완료에 대한 합격/불합격 기준 정의 시
...
### 기능 평가
Codex가 이전에 할 수 없었던 것을 할 수 있는지 테스트:
```markdown
...
```
`.codex/evals/feature-name.md`에 평가 정의 파일 생성
...
- `.codex/evals/<feature>.md` 정의
- `.codex/evals/<feature>.log` 실행 이력
