...
> **140K+ stars** | **21K+ forks** | **170+ contributors** | **12+ language ecosystems** | **OpenAI 해커톤 우승**
...
**AI 에이전트 하네스를 위한 성능 최적화 시스템. OpenAI 해커톤 우승자가 만들었습니다.**
...
**OpenAI Codex**, **Codex**, **Cursor**, **OpenCode**, **Gemini** 등 다양한 AI 에이전트 하네스에서 사용할 수 있습니다.
...
- **크로스 하네스 호환성** — OpenAI Codex, Cursor, OpenCode, Codex 간 동작 일관성 강화.
...
- **AgentShield 통합** — `/security-scan`으로 OpenAI Codex에서 직접 AgentShield 실행; 1282개 테스트, 102개 규칙
...
> WARNING: **중요:** OpenAI Codex 플러그인은 `rules`를 자동으로 배포할 수 없습니다. 수동으로 설치해야 합니다:
...
5. **글로벌 설정**: `~/.codex/package-manager.json`
6. **폴백**: `npm`
...
또는 OpenAI Codex에서 `/setup-pm` 커맨드를 사용하세요.
...
이 저장소는 **OpenAI Codex 플러그인**입니다 - 직접 설치하거나 컴포넌트를 수동으로 복사할 수 있습니다.

```
Everything OpenAI Codex/
|-- .codex-plugin/   # 플러그인 및 마켓플레이스 매니페스트
...
|   |-- 그 외 다수...
|
|-- rules/            # 항상 따르는 가이드라인 (~/.codex/rules/에 복사)
|   |-- common/              # 언어 무관 원칙
...
저장소에서 OpenAI Codex 스킬을 생성하는 두 가지 방법:
...
> OpenAI Codex 해커톤(Cerebral Valley x OpenAI, 2026년 2월)에서 개발. 1282개 테스트, 98% 커버리지, 102개 정적 분석 규칙.

OpenAI Codex 설정에서 취약점, 잘못된 구성, 인젝션 위험을 스캔합니다.
...
**스캔 대상:** CODEX.md, settings.json, MCP 설정, 훅, 에이전트 정의, 스킬 — 시크릿 감지(14개 패턴), 권한 감사, 훅 인젝션 분석, MCP 서버 위험 프로파일링, 에이전트 설정 검토의 5가지 카테고리.

**`--opus` 플래그**는 레드팀/블루팀/감사관 파이프라인으로 3개의 Codex Opus 4.6 에이전트를 실행합니다. 공격자가 익스플로잇 체인을 찾고, 방어자가 보호 조치를 평가하며, 감사관이 양쪽의 결과를 종합하여 우선순위가 매겨진 위험 평가를 작성합니다.

OpenAI Codex에서 `/security-scan`을 사용하거나, [GitHub Action](https://github.com/mehmet-turac/agentshield)으로 CI에 추가하세요.
...
이 플러그인은 훅 시스템 변경으로 인해 OpenAI Codex CLI v2.1.0 이상이 필요합니다.
...
> WARNING: **기여자 참고:** `.codex-plugin/plugin.json`에 `"hooks"` 필드를 추가하지 **마세요**. 회귀 테스트로 이를 강제합니다.

OpenAI Codex v2.1+는 설치된 플러그인의 `hooks/hooks.json`을 **자동으로 로드**합니다. 명시적으로 선언하면 중복 감지 오류가 발생합니다.
...
또는 `~/.codex/settings.json`에 직접 추가:
...
> cp -r Everything OpenAI Codex/rules/typescript ~/.codex/rules/typescript   # 사용하는 스택 선택
>
...
가장 흔한 문제입니다. `.codex-plugin/plugin.json`에 `"hooks"` 필드를 **추가하지 마세요.** OpenAI Codex v2.1+는 설치된 플러그인의 `hooks/hooks.json`을 자동으로 로드합니다.
</details>

<details>
<summary><b>컨텍스트 윈도우가 줄어들어요 / Codex가 컨텍스트가 부족해요</b></summary>

MCP 서버가 너무 많으면 컨텍스트를 잡아먹습니다. 각 MCP 도구 설명이 200k 윈도우에서 토큰을 소비하여 ~70k까지 줄어들 수 있습니다.

**해결:** 프로젝트별로 사용하지 않는 MCP를 비활성화하세요:
```json
// 프로젝트의 .codex/settings.json에서
{
...
OpenAI Codex 사용 비용이 부담된다면 토큰 소비를 관리해야 합니다. 이 설정으로 품질 저하 없이 비용을 크게 줄일 수 있습니다.

### 권장 설정

`~/.codex/settings.json`에 추가:
...
| `CODEX_AUTOCOMPACT_PCT_OVERRIDE` | 95 | **50** | 더 일찍 압축 — 긴 세션에서 더 나은 품질 |
...
[**스폰서 되기**](https://github.com/sponsors/mehmet-turac) | [스폰서 티어](../../SPONSORS.md) | [스폰서십 프로그램](../../SPONSORING.md)
...
- **요약 가이드 (여기서 시작):** [The Shorthand Guide to Everything OpenAI Codex](https://x.com/mehmet_turac/status/2012378465664745795)
- **상세 가이드 (고급):** [The Longform Guide to Everything OpenAI Codex](https://x.com/mehmet_turac/status/2014040193557471352)
- **팔로우:** [@mehmet_turac](https://x.com/mehmet_turac)
