...
> **위 GitHub 및 npm 배지가 현재 기준입니다.** ecc는 검증된 catalog와 공개 rc.1 release track을 갖춘 MIT 라이선스 OpenAI Codex workflow 시스템입니다.
...
**진지한 OpenAI Codex 작업을 위한 운영체제입니다.**
...
이 페이지는 진입용 번역입니다. 현재 운영 세부사항은 외부 영상이나 스레드가 아니라 저장소 안에서 유지되는 문서를 기준으로 확인하세요.

| 문서 | 용도 |
|------|------|
| [Hermes setup guide](../HERMES-SETUP.md) | Hermes x ecc 운영 흐름 설정 |
| [rc.1 release notes](../releases/2.0.0-rc.1/release-notes.md) | 현재 공개 릴리스 표면 확인 |
| [Cross-harness architecture](../architecture/cross-harness.md) | skills, rules, adapters 이식 |
| [Token optimization](../token-optimization.md) | Codex model, compaction, cost 조정 |
| [Security guide](../../the-security-guide.md) | sandboxing, hook risks, AgentShield 점검 |
...
**`--deep` 플래그**는 레드팀/블루팀/감사관 파이프라인으로 3개의 Codex Deep 4.6 에이전트를 실행합니다. 공격자가 익스플로잇 체인을 찾고, 방어자가 보호 조치를 평가하며, 감사관이 양쪽의 결과를 종합하여 우선순위가 매겨진 위험 평가를 작성합니다.
...
| 커맨드 | 사용 시점 |
|--------|----------|
| `/model standard` | 대부분의 작업에서 기본값 |
| `/model deep` | 복잡한 아키텍처, 디버깅, 깊은 추론 |
