...
Every planning artifact is a plain `.md` file under `.codex/`:
...
│  → .codex/prds/X.prd.md  │   Problem · Users · Hypothesis · Scope
└─────────────┬─────────────┘
              │
              ▼
┌───────────────────────────┐
│ /plan <prd-path>          │  Design phase
│  → .codex/plans/X.plan.md│   Patterns · Files · Tasks · Validation
└─────────────┬─────────────┘
              │
              ▼
┌───────────────────────────┐
...
|---|---|---|---|
| `/plan-prd` | *What problem? For whom? How do we know we're done?* | Requirements | `.codex/prds/{name}.prd.md` |
| `/plan` | *What files, patterns, and tasks satisfy the requirement?* | Design + Implementation strategy | `.codex/plans/{name}.plan.md` (PRD mode) or inline (text mode) |
...
# → .codex/prds/per-user-rate-limits.prd.md created
...
/plan .codex/prds/per-user-rate-limits.prd.md

# → .codex/plans/per-user-rate-limits.plan.md created
...
# 4. Open the PR
/pr
# → PR body auto-references .codex/prds/... and .codex/plans/...
```
...
- `/code-review` — reviews local diffs or PRs; auto-detects `.codex/prds/` and `.codex/plans/` as context.

## Compatibility

This pattern adds ecc-native staging-file commands alongside the existing `prp-*` command set. The legacy PRP commands remain available for deeper PRP workflows and for users who already have `.codex/PRPs/` artifacts.

- `/plan-prd` is the lean requirements entry point for `.codex/prds/`.
- `/plan` can consume `.prd.md` files and produce `.codex/plans/` artifacts without requiring the legacy PRP directory layout.
- `/pr` is the ecc-native PR creation command and can reference `.codex/prds/` and `.codex/plans/`.
