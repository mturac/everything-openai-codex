...
- `commands/sessions.md`
  already exposes adjacent session-history concepts from Codex's local store
...
- `scripts/lib/session-adapters/codex-history.js`
  normalizes Codex local session history as a second adapter
...
- what the most recent Codex local session looked like in the same canonical
...
```json
{
  ...
  "coordinationDir": ".../.codex/orchestration/workflow-visual-proof",
  "repoRoot": "...",
  "targetType": "plan",
  "sessionActive": true,
  "paneCount": 2,
...
```json
{
  ...
  "session": {
    ...
    "sourceTarget": {
      "type": "plan",
      "value": ".codex/plan/workflow-visual-proof.json"
    }
  },
  "workers": [
    {
...
1. Add a third adapter, likely `codex-worktree`, so the abstraction moves
   beyond tmux plus Codex-history.
