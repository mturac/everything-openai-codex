---
description: OpenAI Codex session geçmişini, aliasları ve session metadata'sını yönet.
---

# Sessions Komutu

OpenAI Codex session geçmişini yönet - `~/.codex/session-data/` dizininde saklanan session'ları listele, yükle, alias ata ve düzenle; eski `~/.codex/sessions/` dosyalarını da geriye dönük uyumluluk için okuyun.
...
```bash
node -e "
const sm = require((()=>{var e=process.env.CODEX_PLUGIN_ROOT;if(e&&e.trim())return e.trim();var p=require('path'),f=require('fs'),h=require('os').homedir(),d=p.join(h,'.codex'),q=p.join('scripts','lib','utils.js');if(f.existsSync(p.join(d,q)))return d;try{var b=p.join(d,'plugins','cache','Everything OpenAI Codex');for(var o of f.readdirSync(b))for(var v of f.readdirSync(p.join(b,o))){var c=p.join(b,o,v);if(f.existsSync(p.join(c,q)))return c}}catch(x){}return d})()+'/scripts/lib/session-manager');
const aa = require((()=>{var e=process.env.CODEX_PLUGIN_ROOT;if(e&&e.trim())return e.trim();var p=require('path'),f=require('fs'),h=require('os').homedir(),d=p.join(h,'.codex'),q=p.join('scripts','lib','utils.js');if(f.existsSync(p.join(d,q)))return d;try{var b=p.join(d,'plugins','cache','Everything OpenAI Codex');for(var o of f.readdirSync(b))for(var v of f.readdirSync(p.join(b,o))){var c=p.join(b,o,v);if(f.existsSync(p.join(c,q)))return c}}catch(x){}return d})()+'/scripts/lib/session-aliases');
...
- Session'lar `~/.codex/session-data/` dizininde markdown dosyaları olarak saklanır; eski `~/.codex/sessions/` dosyaları da okunmaya devam eder
- Aliaslar `~/.codex/session-aliases.json` dosyasında saklanır
