...
Multi-model işbirlikçi execution - Plandan prototype al → Codex refactor edip implement eder → Multi-model audit ve teslimat.
...
1. **Input Tipini Tanımla**:
   - Plan dosya yolu (örn., `.codex/plan/xxx.md`)
...
1. Gemini'yi çağır (`~/.codex/.ccg/prompts/gemini/frontend.md` kullan)
...
1. Codex'i çağır (`~/.codex/.ccg/prompts/codex/architect.md` kullan)
2. Input: Plan içeriği + alınan context + hedef dosyalar
3. OUTPUT: `Unified Diff Patch ONLY. Strictly prohibit any actual modifications.`
...
**Kod Egemenliği olarak Codex şu adımları çalıştırır**:
...
1. **Kod Egemenliği** – Tüm dosya değişiklikleri Codex tarafından, harici modellerin sıfır yazma erişimi
