...
- **Kod Egemenliği**: Harici modellerin **sıfır dosya sistemi yazma erişimi**, tüm değişiklikler Codex tarafından
- **Stop-Loss Mekanizması**: Mevcut faz çıktısı doğrulanana kadar bir sonraki faza geçme
- **Sadece Planlama**: Bu komut context okumaya ve `.codex/plan/*` plan dosyalarına yazmaya izin verir, ancak **ASLA production kodu değiştirmez**
...
**ace-tool MCP mevcut DEĞİLSE**, fallback olarak OpenAI Codex built-in tool'ları kullan:
...
Codex'un sentezlenmiş planındaki eksiklik riskini azaltmak için, her iki modelin de "plan taslakları" çıktılamasını parallel yaptır (yine **dosya değiştirmesine izin verilmez**):
...
#### 2.4 Implementation Planı Oluştur (Codex Final Version)
...
2. Planı `.codex/plan/<feature-name>.md`'ye kaydet (requirement'tan feature adını çıkar, örn., `user-auth`, `payment-module`)
3. **Kalın metinle** prompt çıktıla (MUTLAKA gerçek kaydedilen dosya yolunu kullan):

   ---
**Plan oluşturuldu ve `.codex/plan/actual-feature-name.md` dosyasına kaydedildi**
...
- **İlk planlama**: `.codex/plan/<feature-name>.md`
- **İterasyon versiyonları**: `.codex/plan/<feature-name>-v2.md`, `.codex/plan/<feature-name>-v3.md`...
...
2. `.codex/plan/<feature-name>.md` dosyasını güncelle
