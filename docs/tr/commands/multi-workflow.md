...
- Multi-model işbirliği: Codex (backend) + Gemini (frontend) + Codex (orkestrasyon)
...
- **Codex (self)** – Orkestrasyon, planlama, execution, teslimat
...
```bash
node scripts/orchestrate-worktrees.js .codex/plan/workflow-e2e-test.json --execute
```
...
**Codex Sentezi**: Codex backend planı + Gemini frontend planını benimsle, kullanıcı onayından sonra `.codex/plan/task-name.md`'ye kaydet.
...
2. Harici modellerin **sıfır dosya sistemi yazma erişimi**, tüm değişiklikler Codex tarafından
