...
- ecc, OpenAI Codex, Codex, Cursor, OpenCode ve Gemini genelinde yeniden kullanılabilir cross-harness altyapı olarak belgelendi.
...
- Codex plugin'i platform seviyesindeki rules dağıtım kısıtlarıyla sınırlı kalır; selective install / OSS yolu hâlâ en güvenilir tam kurulum yoludur.
...
- `codex-devfleet` — Çok ajanlı orkestrasyon (#505)
- `blueprint` — Çok oturumlu yapı planlaması
- `Everything OpenAI Codex` — Öz-referansiyel ecc becerisi (#335)
...
- Fast çağrısı için Observer sandbox erişim düzeltmesi (#661)
