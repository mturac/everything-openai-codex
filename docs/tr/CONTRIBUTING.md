# Everything OpenAI Codex'a Katkıda Bulunma

Katkıda bulunmak istediğiniz için teşekkürler! Bu repo, OpenAI Codex kullanıcıları için bir topluluk kaynağıdır.
...
cp -r skills/my-skill ~/.codex/skills/  # skill'ler için
# Ardından OpenAI Codex ile test edin
...
Skill'ler, OpenAI Codex'un bağlama göre yüklediği bilgi modülleridir.
...
description: Bu agent'ın ne yaptığı ve Codex'un onu ne zaman çağırması gerektiği. Spesifik olun!
...
| `model` | Karmaşıklık seviyesi | `fast` (basit), `standard` (kodlama), `deep` (karmaşık) |
...
Hook'lar, OpenAI Codex olayları tarafından tetiklenen otomatik davranışlardır.
...
- **mcp-configs/mcp-servers.json** bir Context7 girişi içerir; kullanıcılar `documentation-lookup` skill'ini (`skills/documentation-lookup/` içinde) ve `/docs` command'ını kullanmak için bunu harness'lerinde (örn. OpenAI Codex, Cursor) etkinleştirir.
...
- **Issue'lar:** [github.com/mturac/everything-openai-codex/issues](https://github.com/mturac/everything-openai-codex/issues)
