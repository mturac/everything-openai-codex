...
- **Kurallar sistem enjektelidir**: `.codex/rules/*.md` dosyaları her oturumda otomatik yüklenir. İstem talimatlarının aksine, LLM bunları görmezden gelmeyi seçemez.

## Örnek Çağrılar

```bash
codex /mail                    # Yalnızca e-posta triyajı
codex /slack                   # Yalnızca Slack triyajı
codex /today                   # Tüm kanallar + takvim + yapılacaklar
codex /schedule-reply "Yönetim kurulu toplantısı hakkında Sarah'ya yanıt ver"
```

## Ön Koşullar

- [OpenAI Codex](https://docs.openai.com/en/docs/codex)
