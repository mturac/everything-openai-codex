...
Bu dosya, bu depodaki kodlarla çalışırken OpenAI Codex'a (codex.ai/code) rehberlik sağlar.

## Projeye Genel Bakış

Bu bir **OpenAI Codex plugin**'idir - üretime hazır agent'lar, skill'ler, hook'lar, komutlar, kurallar ve MCP konfigürasyonlarından oluşan bir koleksiyondur. Proje, OpenAI Codex kullanarak yazılım geliştirme için test edilmiş iş akışları sağlar.
...
- Package manager algılama: npm, pnpm, yarn, bun (`CODEX_PACKAGE_MANAGER` env var veya proje config ile yapılandırılabilir)
