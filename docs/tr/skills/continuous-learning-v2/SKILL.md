...
OpenAI Codex oturumlarınızı güven skorlaması ile atomik "instinct'ler" - küçük öğrenilmiş davranışlar - aracılığıyla yeniden kullanılabilir bilgiye dönüştüren gelişmiş bir öğrenme sistemi.
...
- OpenAI Codex oturumlarından otomatik öğrenme ayarlarken
...
| Özellik | v2.0 | v2.1 |
|---------|------|------|
| Depolama | Global (~/.codex/homunculus/) | Proje kapsamlı (projects/<hash>/) |
...
| Analiz | Ana bağlam | Arka plan agent'ı (Fast) |
...
+---------------------------------------------+
      |
      | Gözlemci agent okur (arka plan, Fast)
      v
+---------------------------------------------+
|          KALIP TESPİTİ                      |
...
1. **`CODEX_PROJECT_DIR` env var** (en yüksek öncelik)
...
Her proje 12 karakterlik bir hash ID alır (örn. `a1b2c3d4e5f6`). `~/.codex/homunculus/projects.json` dosyasındaki kayıt dosyası ID'leri insanların okuyabileceği isimlerle eşler.

## Hızlı Başlangıç

### 1. Gözlem Hook'larını Aktifleştirin

`~/.codex/settings.json` dosyanıza ekleyin.

**Plugin olarak kuruluysa** (önerilen):

`~/.codex/settings.json` içine ek hook bloğu eklemeyin. OpenAI Codex v2.1+ eklentinin `hooks/hooks.json` dosyasını otomatik yükler; `observe.sh` zaten orada kayıtlıdır.

Daha önce `observe.sh` satırlarını `~/.codex/settings.json` içine kopyaladıysanız, yinelenen `PreToolUse` / `PostToolUse` bloğunu kaldırın. Yinelenen kayıt hem çift çalıştırmaya yol açar hem de `${CODEX_PLUGIN_ROOT}` çözümleme hatası üretir; bu değişken yalnızca eklentiye ait `hooks/hooks.json` girdilerinde genişletilir.

**`~/.codex/skills` dizinine manuel kuruluysa**, aşağıdakini `~/.codex/settings.json` içine ekleyin:
...
```bash
# Global dizinler
mkdir -p ~/.codex/homunculus/{instincts/{personal,inherited},evolved/{agents,skills,commands},projects}
...
> "v1 gözlem için skill'lere güveniyordu. Skill'ler olasılıksaldır -- Codex'un yargısına göre zamanın ~%50-80'inde tetiklenirler."
...
- `~/.codex/homunculus/instincts/` içindeki mevcut global instinct'ler hala global instinct olarak çalışır
- v1'den `~/.codex/skills/learned/` skill'leri hala çalışır
...
- [The Longform Guide](https://x.com/mehmet_turac/status/2014040193557471352) - Sürekli öğrenme bölümü

---

*Instinct tabanlı öğrenme: Codex'a kalıplarınızı öğretmek, her seferinde bir proje.*
