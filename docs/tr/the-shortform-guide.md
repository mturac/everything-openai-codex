...
Deep 4.5 ile uzun bir kodlama oturumundan sonra ölü kodu ve gevşek .md dosyalarını temizlemek mi istiyorsunuz? `/refactor-clean` çalıştırın. Test mi gerekli? `/tdd`, `/e2e`, `/test-coverage`. Skill'ler ayrıca codemap'leri de içerebilir - Codex'un keşfe context harcamadan kod tabanınızda hızlıca gezinmesi için bir yöntem.
...
- **Minimal Kaynak Kullanımı** - Ağır işlemler sırasında Codex ile RAM/CPU için rekabet etmez. Deep çalıştırırken önemli
...
~/.codex/rules/
  ...
  performance.md   # Model seçimi (Fast vs Standard vs Deep)
