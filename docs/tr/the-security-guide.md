...
_everything.openai.codex / araştırma / güvenlik_
...
Açık kaynak agent'ların yaygın olarak benimsenmesi burada. OpenClaw ve diğerleri bilgisayarınızda dolaşıyor. OpenAI Codex ve Codex (ecc kullanarak) gibi sürekli çalışma harness'leri yüzey alanını artırıyor; ve 25 Şubat 2026'da, Check Point Research konuşmanın "bu olabilir ama olmaz / abartılıyor" fazını kesinlikle sona erdirmesi gereken bir OpenAI Codex ifşası yayınladı. Araçlar kritik kütleye ulaştıkça, exploit'lerin ağırlığı katlanır.

Bir sorun, CVE-2025-59536 (CVSS 8.7), proje içeren kodun kullanıcı güven diyaloğunu kabul etmeden önce çalışmasına izin verdi. Bir diğeri, CVE-2026-21852, API trafiğinin saldırgan tarafından kontrol edilen bir `OPENAI_BASE_URL` üzerinden yönlendirilmesine izin vererek, güven onaylanmadan önce API anahtarını sızdırdı. Tek yapmanız gereken repo'yu klonlamak ve aracı açmaktı.
...
## OpenAI Codex CVE'leri (Şubat 2026)

Check Point Research, OpenAI Codex bulgularını 25 Şubat 2026'da yayınladı. Sorunlar Temmuz ve Aralık 2025 arasında bildirildi, ardından yayından önce yamalandı.
...
>
> Sahte hook eylemleriyle zehirlenmiş yapılandırma dosyaları aracılığıyla OpenAI Codex kullanıcılarını ele geçirme.
>
...
> _OpenAI Codex'u hack'ledim! "Agentic"in sadece shell almanın süslü yeni bir yolu olduğu ortaya çıktı. Tam RCE elde ettim ve organizasyon API anahtarlarını ele geçirdim. CVE-2025-59536 | CVE-2026-21852_
> [research.checkpoint.com](https://research.checkpoint.com/2026/rce-and-api-token-exfiltration-through-codex-project-files-cve-2025-59536/)
...
**CVE-2026-21852.** Saldırgan tarafından kontrol edilen bir proje `OPENAI_BASE_URL`'i geçersiz kılabilir, API trafiğini yönlendirebilir ve güven onayı öncesinde API anahtarını sızdırabilirdi. NVD manuel güncelleyicilerin `2.0.65` veya sonrasında olması gerektiğini söylüyor.
...
OpenAI'in kendi dokümanları bu gerçeği yansıtıyor. Proje ayarları `.codex/` içinde yaşıyor. Proje kapsamlı MCP server'ları `.mcp.json` içinde yaşıyor. Kaynak kontrol aracılığıyla paylaşılıyorlar. Bir güven sınırı tarafından korunmaları gerekiyor. Bu güven sınırı tam olarak saldırganların peşine düşeceği şey.

## Son Bir Yılda Ne Değişti

Bu konuşma 2025 ve erken 2026'da hızlı ilerledi.

OpenAI Codex'un repo tarafından kontrol edilen hook'ları, MCP ayarları ve env-var güven yolları kamuya açık olarak test edildi. Amazon Q Developer, VS Code extension'ında kötü niyetli prompt payload içeren 2025 tedarik zinciri olayına, ardından yapı altyapısında aşırı geniş GitHub token maruziyetiyle ilgili ayrı bir ifşaya sahipti. Zayıf kimlik bilgisi sınırları artı agent'a yakın araçlar, fırsatçılar için bir giriş noktasıdır.
...
| İstatistik | Detay |
|------|--------|
| **CVSS 8.7** | OpenAI Codex hook / güven öncesi yürütme sorunu: CVE-2025-59536 |
...
Root erişimi tehlikelidir. Geniş yerel erişim tehlikelidir. Aynı makinede uzun ömürlü kimlik bilgileri tehlikelidir. "YOLO, Codex beni koruyor" burada doğru yaklaşım değildir. Cevap izolasyondur.
...
```bash
rg -n 'curl|wget|nc|scp|ssh|enableAllProjectMcpServers|OPENAI_BASE_URL'
```
...
Agent'ın neyi okuduğunu, hangi aracı çağırdığını ve hangi ağ hedefine gitmeye çalıştığını göremezseniz, onu güvenli hale getiremezsiniz (bu bariz olmalı, yine de bir ralph döngüsünde codex --dangerously-skip-permissions'ı çalıştırdığınızı ve hiçbir endişe olmadan uzaklaştığınızı görüyorum). Sonra karmaşık bir kod tabanıyla geri geliyorsunuz, agent'ın ne yaptığını bulmaya iş yapmaktan daha fazla zaman harcıyorsunuz.
...
OpenAI, OpenAI Codex'un oturum başlangıcında memory yüklediğini belgeliyor. Bu yüzden memory'yi dar tutun:
...
OpenAI, OpenAI Codex'u sertleştirdi ve güven, izinler, MCP, memory, hook'lar ve izole ortamlar etrafında somut güvenlik rehberliği yayınladı.
...
Kurulumunuzu tarayın: [github.com/mehmet-turac/agentshield](https://github.com/mehmet-turac/agentshield)

---

## Referanslar

- Check Point Research, "Caught in the Hook: RCE and API Token Exfiltration Through OpenAI Codex Project Files" (25 Şubat 2026): [research.checkpoint.com](https://research.checkpoint.com/2026/rce-and-api-token-exfiltration-through-codex-project-files-cve-2025-59536/)
...
- OpenAI, "Defending against indirect prompt injection attacks": [openai.com](https://www.openai.com/news/prompt-injection-defenses)
- OpenAI Codex docs, "Settings": [code.openai-codex.com](https://code.openai-codex.com/docs/en/settings)
- OpenAI Codex docs, "MCP": [code.openai-codex.com](https://code.openai-codex.com/docs/en/mcp)
- OpenAI Codex docs, "Security": [code.openai-codex.com](https://code.openai-codex.com/docs/en/security)
- OpenAI Codex docs, "Memory": [code.openai-codex.com](https://code.openai-codex.com/docs/en/memory)
...
> [OpenAI Codex'un Her Şeyine Dair Kısa Kılavuz](https://x.com/mehmet_turac/status/2012378465664745795)
>
