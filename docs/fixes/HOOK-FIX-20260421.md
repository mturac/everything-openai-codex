...
OpenAI Codex CLI v2.1.116 on Windows was failing all Bash tool hook invocations with:
...
`C:\Users\sugig\.codex\settings.local.json` had two defects:
...
```json
"command": "\"C:\\Program Files\\Git\\bin\\bash.exe\" \"C:\\Users\\sugig\\.codex\\skills\\continuous-learning\\hooks\\observe-wrapper.sh\""
```

When OpenAI Codex spawns this on Windows, argument splitting does not preserve
...
```json
"command": "C:\\Users\\sugig\\.codex\\skills\\continuous-learning\\hooks\\observe.sh"
```

OpenAI Codex on Windows detects `.sh` and invokes it via Git Bash itself — no
manual `bash.exe` wrapping needed.

## Fix

`C:\Users\sugig\.codex\settings.local.json` rewritten as UTF-8 (no BOM), LF
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "C:/Users/sugig/.codex/skills/continuous-learning/hooks/observe-wrapper.sh post"
          }
        ]
      }
    ]
...
echo '{"tool_name":"Bash","tool_input":{"command":"pwd"},"session_id":"post-fix-verify-001","cwd":"...","hook_event_name":"PostToolUse"}' \
  | "C:/Users/sugig/.codex/skills/continuous-learning/hooks/observe-wrapper.sh" post
# exit=0

# PreToolUse path
echo '{"tool_name":"Bash","tool_input":{"command":"ls"},"session_id":"post-fix-verify-pre-001","cwd":"...","hook_event_name":"PreToolUse"}' \
  | "C:/Users/sugig/.codex/skills/continuous-learning/hooks/observe-wrapper.sh" pre
# exit=0
```
...
`codex` CLI session launch. Restart the CLI and run a Bash tool call to
confirm new rows appear in `observations.jsonl` from the actual CLI session.

## Files Touched

- `C:\Users\sugig\.codex\settings.local.json` — rewritten
- `C:\Users\sugig\.codex\settings.local.json.bak-hookfix-20260421-145718` — pre-fix backup
