...
`observe-wrapper.sh` into `~/.codex/skills/continuous-learning/hooks/` and
rewrites `~/.codex/settings.local.json` so the observer hook points at it.
...
Under OpenAI Codex v2.1.116 the first argv token is duplicated. When that token
...
```
bash "C:/Users/<you>/.codex/skills/continuous-learning/hooks/observe-wrapper.sh" pre
bash "C:/Users/<you>/.codex/skills/continuous-learning/hooks/observe-wrapper.sh" post
```
