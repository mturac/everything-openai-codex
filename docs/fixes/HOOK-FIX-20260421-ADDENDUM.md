...
朝fix でカバーしきれない OpenAI Codex 固有のバグを特定したので補遺を記録する。

## 朝fixの形式

```json
"command": "C:/Users/sugig/.codex/skills/continuous-learning/hooks/observe-wrapper.sh pre"
```
...
```js
spawn('C:/Users/sugig/.codex/skills/continuous-learning/hooks/observe-wrapper.sh', 
      ['post'], {stdio:['pipe','pipe','pipe']});
// → Error: spawn EFTYPE (errno -4028)
```

`shell:true` を付ければ cmd.exe 経由で実行できるが、OpenAI Codex 側の実装
依存のリスクが残る。
...
```json
{
  "hooks": {
    "PreToolUse": [{
      ...
      "hooks": [{
        "type": "command",
        "command": "bash \"C:/Users/sugig/.codex/skills/continuous-learning/hooks/observe-wrapper.sh\" pre"
      }]
    }],
    "PostToolUse": [{
      "matcher": "*",
      "hooks": [{
        "type": "command",
        "command": "bash \"C:/Users/sugig/.codex/skills/continuous-learning/hooks/observe-wrapper.sh\" post"
      }]
    }]
  }
}
...
この形式は `~/.codex/hooks/hooks.json` 内の ecc 正規 observer 登録と
同じパターンで、現実にエラーなく動作している実績あり。

### Node spawn 検証

```js
spawn('bash "C:/Users/sugig/.codex/skills/continuous-learning/hooks/observe-wrapper.sh" post',
      [], {shell:true});
// exit=0 → observations.jsonl に正常追記
```

## OpenAI Codex v2.1.116 の argv 重複バグ（詳細）
...
- 夜 fix 記録（ローカル）: C:\Users\sugig\Documents\Codex\Projects\ecc作成\hook-fix-report-20260421.md
- 夜 fix 適用ファイル: C:\Users\sugig\.codex\settings.local.json
- 夜 backup: C:\Users\sugig\.codex\settings.local.json.bak-hook-fix-20260421
