...
A comprehensive guide to creating effective skills for Everything OpenAI Codex (ecc).
...
Skills are **knowledge modules** that OpenAI Codex loads based on context. They provide:
...
Unlike **agents** (specialized subassistants) or **commands** (user-triggered actions), skills are passive knowledge that OpenAI Codex references when relevant.

### When Skills Activate

Skills activate when:
- The user's task matches the skill's domain
- OpenAI Codex detects relevant context
...
Describe scenarios where Codex should use this skill.
...
Write content that Codex can **immediately use**:
...
1. **Copy to OpenAI Codex skills directory**:
   ```bash
   cp -r skills/your-skill-name ~/.codex/skills/
   ```

2. **Test with OpenAI Codex**:
   ```
   You: "I need to [task that should trigger your skill]"

   Codex should reference your skill's patterns.
   ```

3. **Verify activation**:
   - Ask Codex to explain a concept from your skill
