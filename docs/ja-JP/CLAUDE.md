...
このファイルは、このリポジトリのコードを操作する際にOpenAI Codex (codex.ai/code) にガイダンスを提供します。

## プロジェクト概要

これは**OpenAI Codexプラグイン**です — プロダクション対応のエージェント、スキル、フック、コマンド、ルール、MCP設定のコレクションです。このプロジェクトは、OpenAI Codexを使用したソフトウェア開発のための実戦で鍛えられたワークフローを提供します。
...
- パッケージマネージャー検出：npm、pnpm、yarn、bun（`CODEX_PACKAGE_MANAGER` 環境変数またはプロジェクト設定で設定可能）
...
- スキル配置：キュレート済みは skills/ に、生成/インポートは ~/.codex/skills/ に。docs/SKILL-PLACEMENT-POLICY.md を参照
