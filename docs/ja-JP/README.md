...
**OpenAIハッカソン優勝者による完全なOpenAI Codex設定集。**
...
> WARNING: **重要:** OpenAI Codexプラグインは`rules`を自動配布できません。手動でインストールしてください：
...
cp -r Everything OpenAI Codex/rules/typescript ~/.codex/rules/typescript
...
または OpenAI Codex で `/setup-pm` コマンドを使用。

---

## 含まれるもの

このリポジトリは**OpenAI Codexプラグイン**です - 直接インストールするか、コンポーネントを手動でコピーできます。

```
Everything OpenAI Codex/
|-- .codex-plugin/   # プラグインとマーケットプレイスマニフェスト
...
|
|-- rules/            # 常に従うべきガイドライン（~/.codex/rules/ にコピー）
...
|
|-- examples/         # 設定例とセッション
|   |-- CODEX.md           # プロジェクトレベル設定例
|   |-- user-CODEX.md      # ユーザーレベル設定例
|
...
リポジトリから OpenAI Codex スキルを生成する 2 つの方法：
...
両オプションで生成されるもの：
- **SKILL.mdファイル** - OpenAI Codexですぐに使えるスキル
- **instinctコレクション** - continuous-learning-v2用
- **パターン抽出** - コミット履歴からの学習

### AgentShield — セキュリティ監査ツール

OpenAI Codex 設定の脆弱性、誤設定、インジェクションリスクをスキャンします。
...
CODEX.md、settings.json、MCP サーバー、フック、エージェント定義をチェックします。セキュリティグレード（A-F）と実行可能な結果を生成します。

OpenAI Codexで`/security-scan`を実行、または[GitHub Action](https://github.com/mehmet-turac/agentshield)でCIに追加できます。
...
### OpenAI Codex CLI バージョン

**最小バージョン: v2.1.0 以上**

このプラグインは OpenAI Codex CLI v2.1.0+ が必要です。プラグインシステムがフックを処理する方法が変更されたためです。
...
> WARNING: **貢献者向け:** `.codex-plugin/plugin.json`に`"hooks"`フィールドを追加しないでください。これは回帰テストで強制されます。

OpenAI Codex v2.1+は、インストール済みプラグインの`hooks/hooks.json`（規約）を自動読み込みします。`plugin.json`で明示的に宣言するとエラーが発生します：
...
このリポジトリを使用する最も簡単な方法 - OpenAI Codexプラグインとしてインストール：
...
> **注:** OpenAI Codexプラグインシステムは`rules`をプラグイン経由で配布できません（[アップストリーム制限](https://code.openai-codex.com/docs/en/plugins-reference)）。ルールは手動でインストールする必要があります：
>
> ```bash
> # まずリポジトリをクローン
> git clone https://github.com/mturac/everything-openai-codex.git
...
> mkdir -p ~/.codex/rules
> cp -r Everything OpenAI Codex/rules/common ~/.codex/rules/common
> cp -r Everything OpenAI Codex/rules/typescript ~/.codex/rules/typescript   # スタックを選択
> cp -r Everything OpenAI Codex/rules/python ~/.codex/rules/python
> cp -r Everything OpenAI Codex/rules/golang ~/.codex/rules/golang
>
...
# スキルをコピー
cp -r Everything OpenAI Codex/skills/* ~/.codex/skills/
```

#### settings.json にフックを追加

手動インストール時のみ、`hooks/hooks.json` のフックを `~/.codex/settings.json` にコピーします。

`/plugin install` で ecc を導入した場合は、これらのフックを `settings.json` にコピーしないでください。OpenAI Codex v2.1+ はプラグインの `hooks/hooks.json` を自動読み込みするため、二重登録すると重複実行や `${CODEX_PLUGIN_ROOT}` の解決失敗が発生します。

#### MCP を設定

`mcp-configs/mcp-servers.json` から必要な MCP サーバーを `~/.codex.json` にコピーします。
...
| コンポーネント | OpenAI Codex → Cursor | パリティ |
|-----------|---------------------|--------|
...
| 機能 | OpenAI Codex | OpenCode | ステータス |
|---------|-------------|----------|--------|
| Agents | PASS: 14 エージェント | PASS: 12 エージェント | **OpenAI Codex がリード** |
| Commands | PASS: 30 コマンド | PASS: 24 コマンド | **OpenAI Codex がリード** |
| Skills | PASS: 28 スキル | PASS: 16 スキル | **OpenAI Codex がリード** |
...
OpenCodeのプラグインシステムはOpenAI Codexより高度で、20+イベントタイプ：

| OpenAI Codex フック | OpenCode プラグインイベント |
|-----------------|----------------------|
...
実験的なリリース以来、OpenAI Codexを使用してきました。2025年9月、[@DRodriguezFX](https://x.com/DRodriguezFX)と一緒にOpenAI Codexで[zenith.chat](https://zenith.chat)を構築し、OpenAI x Forum Venturesハッカソンで優勝しました。
...
- **簡潔ガイド（まずはこれ）:** [Everything OpenAI Codex 簡潔ガイド](https://x.com/mehmet_turac/status/2012378465664745795)
- **詳細ガイド（高度）:** [Everything OpenAI Codex 詳細ガイド](https://x.com/mehmet_turac/status/2014040193557471352)
- **フォロー:** [@mehmet_turac](https://x.com/mehmet_turac)
