...
> **上の GitHub / npm バッジが信頼できる現在値です。** ecc は OpenAI Codex workflow 向けの MIT ライセンスシステムで、検証済み catalog と公開 rc.1 release track を持ちます。
...
**OpenAI Codex で本格的に作業するための operating system。**
...
このページは入口用の翻訳です。現在の運用詳細は、外部動画やスレッドではなく、このリポジトリで保守されているドキュメントを参照してください。

| ドキュメント | 用途 |
|--------------|------|
| [Hermes setup guide](../HERMES-SETUP.md) | Hermes x ecc の運用フローを設定する |
| [rc.1 release notes](../releases/2.0.0-rc.1/release-notes.md) | 現在の公開リリース面を確認する |
| [Cross-harness architecture](../architecture/cross-harness.md) | skills、rules、adapters を移植する |
| [Token optimization](../token-optimization.md) | Codex の model、compaction、cost を調整する |
| [Security guide](../../the-security-guide.md) | sandboxing、hook risks、AgentShield を確認する |
...
ecc は日常的な OpenAI Codex 利用から生まれ、繰り返し使う production workflow を再利用可能な skills、hooks、rules、install profiles に変換します。
