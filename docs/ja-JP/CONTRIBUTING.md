# Everything OpenAI Codexに貢献する

貢献いただきありがとうございます！このリポジトリはOpenAI Codexユーザーのためのコミュニティリソースです。
...
スキルは、コンテキストに基づいてOpenAI Codexが読み込む知識モジュールです。
...
| `model` | 複雑さレベル | `fast`（シンプル）、`standard`（コーディング）、`deep`（複雑） |
...
フックはOpenAI Codexイベントによってトリガーされる自動的な動作です。
