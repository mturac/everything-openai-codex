...
マルチモデル協調実装 - 計画からプロトタイプを取得 → Codexがリファクタリングして実装 → マルチモデル監査と配信。
...
- **コード主権**: 外部モデルは**ファイルシステムへの書き込みアクセスがゼロ**、すべての変更はCodexが実行
...
**ace-tool MCPが利用できない場合**、OpenAI Codex組み込みツールでフォールバック:
...
1. **コード主権** – すべてのファイル変更はCodexが実行、外部モデルは書き込みアクセスがゼロ
