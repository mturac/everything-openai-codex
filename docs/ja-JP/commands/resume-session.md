---
description: ~/.codex/session-data/ から最新のセッションファイルを読み込み、前回のセッションが終了した時点の完全なコンテキストを使って作業を再開します。
---
...
- セッションファイルがあり、Codex に作業を続行する前に完全に内容を把握させたいとき

## 使い方

```
/resume-session                                                      # ~/.codex/session-data/ の最新ファイルを読み込む
/resume-session 2024-01-15                                           # その日付の最新セッションを読み込む
/resume-session ~/.codex/session-data/2024-01-15-abc123de-session.tmp  # 現在の短縮IDセッションファイルを読み込む
/resume-session ~/.codex/sessions/2024-01-15-session.tmp               # レガシー形式の特定ファイルを読み込む
```
...
- 日付形式（`YYYY-MM-DD`）の場合、まず `~/.codex/session-data/` を検索し、次にレガシーの
  `~/.codex/sessions/` を検索して、`YYYY-MM-DD-session.tmp`（レガシー形式）または
