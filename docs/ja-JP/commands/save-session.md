---
description: 現在のセッション状態を日付付きファイルとして ~/.codex/session-data/ に保存し、完全なコンテキストを持った状態で将来のセッションで作業を再開できるようにします。
---
...
ユーザーのCodexホームディレクトリに正規のセッションフォルダを作成します：
...
`~/.codex/session-data/YYYY-MM-DD-<short-id>-session.tmp` を作成します。今日の実際の日付と、`session-manager.js` の `SESSION_FILENAME_REGEX` が適用するルールを満たすshort-idを使用します：
...
- このファイルは次のセッションの開始時に `/resume-session` を通じてCodexに読み込まれることを目的としています
- 正規のグローバルセッションストアを使用してください: `~/.codex/session-data/`
