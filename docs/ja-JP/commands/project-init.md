...
1. デフォルトはドライラン。ユーザーが具体的な計画を承認するまで、`CODEX.md`、設定ファイル、ルール、スキル、またはインストール状態を変更しない。
2. 既存のプロジェクトガイダンスを保持。`CODEX.md`、`.codex/settings.local.json`、`.cursor/`、`.codex/`、`.gemini/`、`.opencode/`、`.codebuddy/`、`.joycode/`、または`.qwen/`が既に存在する場合、内容を検査し上書きではなくマージ/追記計画を提案。
...
1. ターゲットハーネスを特定。ユーザーが`cursor`、`codex`、`gemini`、`opencode`、`codebuddy`、`joycode`、または`qwen`を要求しない限りデフォルトは`codex`。
...
ユーザーが`CODEX.md`スターターを求める場合、インストーラー計画とは別に生成し最小限に保つ:
...
diffを表示して承認を得ずに既存の`CODEX.md`を置換しないこと。
