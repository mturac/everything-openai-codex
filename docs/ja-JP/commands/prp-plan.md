...
以下のテンプレートを使用して完全な計画文書を作成します。`.codex/PRPs/plans/{kebab-case-feature-name}.plan.md` に保存します。
...
- **ファイル**: .codex/PRPs/plans/{kebab-case-feature-name}.plan.md
- **ソースPRD**: [パスまたは "N/A"]
...
> 次のステップ: `/prp-implement .codex/PRPs/plans/{name}.plan.md` を実行してこの計画を実行します。
```
