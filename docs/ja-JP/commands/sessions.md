...
OpenAI Codexセッション履歴を管理 - `~/.codex/session-data/` に保存されたセッションのリスト表示、読み込み、エイリアス設定、編集を行います。旧 `~/.codex/sessions/` のファイルも後方互換のために読み取ります。
...
- セッションは `~/.codex/session-data/` にMarkdownファイルとして保存され、旧 `~/.codex/sessions/` のファイルも引き続き読み取られます
