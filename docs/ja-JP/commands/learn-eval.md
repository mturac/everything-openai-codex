...
3. **保存場所の決定:**
   ...
   - **グローバル** (`~/.codex/skills/learned/`): 2つ以上のプロジェクトで使える汎用パターン（bash互換性、LLM APIの動作、デバッグテクニックなど）
   - **プロジェクト** (現在のプロジェクトの`.codex/skills/learned/`): プロジェクト固有のナレッジ（特定の設定ファイルの癖、プロジェクト固有のアーキテクチャ決定など）
...
5. **品質ゲート — チェックリスト + 総合判定**
   ...
   - [ ] `~/.codex/skills/`および関連プロジェクトの`.codex/skills/`ファイルをキーワードでgrepし、内容の重複を確認
