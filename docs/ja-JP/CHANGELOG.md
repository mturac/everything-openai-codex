...
- OpenAI Codex、Codex、Cursor、OpenCode、Gemini全体で再利用可能なクロスハーネス基盤としてeccをドキュメント化。
...
- Codexプラグインはプラットフォームレベルのルール配布の制約により制限されたまま；選択的インストール/OSSパスが依然として最も信頼性の高い完全インストール方法。
...
- `codex-devfleet` — マルチエージェントオーケストレーション (#505)
- `blueprint` — マルチセッション構築計画
- `Everything OpenAI Codex` — 自己参照型eccスキル (#335)
...
- Fast呼び出しのためのオブザーバーサンドボックスアクセス修正 (#661)
