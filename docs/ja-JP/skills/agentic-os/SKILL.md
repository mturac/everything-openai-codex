---
name: agentic-os
description: OpenAI Codex 上に永続的なマルチエージェントオペレーティングシステムを構築します。カーネルアーキテクチャ、スペシャリストエージェント、スラッシュコマンド、ファイルベースのメモリ、スケジュールされた自動化、外部データベースなしの状態管理をカバーします。
origin: ecc
---
...
各エージェントは `agents/` のスタンドアロン Markdown ファイルです。Codex はタスクをルーティングする際に関連するエージェントファイルをロードします。
...
- テストランナーアクセス
- `.codex/mcp.json` で設定された MCP サーバー
...
並行実行のために、OpenAI Codex のバックグラウンドタスク機能や特定のエージェントコンテキストで OpenAI Codex を呼び出すシェルスクリプトを使用します。

## コマンドと毎日のワークフロー

スラッシュコマンドは `.codex/commands/` の Markdown ファイルです。再利用可能なワークフローを定義します。
...
コマンドファイルを `.codex/commands/<command-name>.md` に配置します。OpenAI Codex はそれらを自動検出します。ユーザーは `/<command-name>` で呼び出します。
...
エージェニック OS タスクは、セッションが終了すると停止する OpenAI Codex の組み込み cron ではなく、外部 cron を使用してスケジュールで実行されます。
...
```markdown
# 悪い例 - エージェントファイルまたは CODEX.md に API キー
...
ルーティングを `CODEX.md` の Markdown テーブルで宣言的に保ちます。検査・編集・デバッグが可能です。

## ベストプラクティス

- [ ] `CODEX.md` は 200 行未満でコンテキストウィンドウに収まる
...
- [ ] スケジュールされたタスクは OpenAI Codex のセッション cron ではなく外部 cron（LaunchAgent、systemd、pm2）を使用する
- [ ] コスト追跡: `data/logs/<date>-costs.json` にセッションごとの API 支出をログに記録する
- [ ] 1 プロジェクト = 1 エージェニック OS。無関係なプロジェクト間で単一の `CODEX.md` を共有しない。
