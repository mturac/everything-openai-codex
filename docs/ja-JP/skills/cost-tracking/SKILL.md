---
name: cost-tracking
description: ローカルのコスト追跡データベースからOpenAI Codexのトークン使用量、支出、予算を追跡・レポートします。コスト、支出、使用量、トークン、予算、またはプロジェクト、ツール、セッション、日付によるコスト内訳について質問する場合に使用します。
origin: community
---

# コスト追跡

このスキルを使用して、ローカルSQLiteデータベースからOpenAI Codexのコストと使用履歴を分析します。これは、`~/.codex-cost-tracker/usage.db`に使用行を書き込むコスト追跡フックまたはプラグインをすでに持っているユーザーを対象としています。
...
| `session_id` | OpenAI Codexセッション識別子 |
| `model` | イベントに使用されたモデル |
