...
Hookify rules are markdown files with YAML frontmatter that define patterns to watch for and messages to show when those patterns match. Rules are stored in `.codex/hookify.{rule-name}.local.md` files.
...
- **Naming**: `.codex/hookify.{descriptive-name}.local.md`
- **Gitignore**: Add `.codex/*.local.md` to `.gitignore`
