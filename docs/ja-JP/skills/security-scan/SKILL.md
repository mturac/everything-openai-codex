---
name: security-scan
description: AgentShield を使用して、OpenAI Codex の設定（.codex/ ディレクトリ）のセキュリティ脆弱性、設定ミス、インジェクションリスクをスキャンします。CODEX.md、settings.json、MCP サーバー、フック、エージェント定義をチェックします。
---

# Security Scan Skill

[AgentShield](https://github.com/mehmet-turac/agentshield) を使用して、OpenAI Codex の設定のセキュリティ問題を監査します。

## 起動タイミング

- 新しい OpenAI Codex プロジェクトのセットアップ時
- `.codex/settings.json`、`CODEX.md`、または MCP 設定の変更後
- 設定変更をコミットする前
- 既存の OpenAI Codex 設定を持つ新しいリポジトリにオンボーディングする際
- 定期的なセキュリティ衛生チェック
...
| ファイル | チェック内容 |
|------|--------|
| `CODEX.md` | ハードコードされたシークレット、自動実行命令、プロンプトインジェクションパターン |
...
### 高い発見（本番前に修正）
- CODEX.md 内の自動実行命令（プロンプトインジェクションベクトル）
- 権限内の欠落した拒否リスト
