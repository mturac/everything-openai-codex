...
3段階のパイプラインを通じて任意のプロジェクトを安全にオープンソース化する: **フォーク**（シークレット除去）→ **サニタイズ**（クリーンな状態を確認）→ **パッケージ**（CODEX.md + setup.sh + README）。
...
| `/opensource package PROJECT` | CODEX.md + setup.sh + READMEを生成 |
...
FORK_REPORT.md、SANITIZATION_REPORT.md、CODEX.mdの存在でパイプラインの進捗を各プロジェクトと共に表示する。
