...
2. `~/.codex/agents/**/*.md` + `~/.codex/agents/*.md` —— グローバルエージェント（両方の深さ）
...
* **並列Agent呼び出し（TeamCreateではない）。** このスキルは独立した作業の処理に並列Agentツール呼び出しを使用する。エージェントが議論したり互いに回答する必要がある場合のみ、TeamCreate（マルチエージェント会話のためのOpenAI Codexツール）が必要になる。
