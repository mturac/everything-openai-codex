---
name: context-budget
description: エージェント、スキル、MCPサーバー、ルールにわたってOpenAI Codexのコンテキストウィンドウ消費を監査します。肥大化、冗長なコンポーネントを特定し、優先順位付けされたトークン節約の推奨事項を生成します。
origin: ecc
---
...
| バケット | 基準 | アクション |
|--------|----------|--------|
| **常に必要** | CODEX.mdで参照されている、アクティブなコマンドをサポート、または現在のプロジェクトタイプに一致 | 保持 |
| **時々必要** | ドメイン固有（例：言語パターン）、CODEX.mdで未参照 | オンデマンドアクティベーションを検討 |
