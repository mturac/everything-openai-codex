...
> 59のスラッシュコマンドがグローバルにインストール済み。任意のOpenAI Codexセッションで `/` と入力して呼び出せます。
...
| `/devfleet` | DevFleet経由での並列OpenAI Codexエージェントのオーケストレーション |
...
| コマンド | 機能 |
|---------|------|
| `/save-session` | 現在のセッション状態を `~/.codex/session-data/` に保存 |
| `/resume-session` | 正規のセッションストアから最新の保存済みセッションを読み込み、中断した箇所から再開 |
| `/sessions` | `~/.codex/session-data/` のセッション履歴を閲覧、検索、管理（`~/.codex/sessions/` からのレガシー読み取りも対応） |
...
| `/eval` | 評価ハーネスを実行 |
| `/model-route` | タスクを適切なモデル（Fast / Standard / Deep）にルーティング |
