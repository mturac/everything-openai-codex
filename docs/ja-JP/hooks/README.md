...
フックはイベント駆動の自動化で、OpenAI Codexのツール実行の前後に起動します。コード品質を強制し、ミスを早期に検出し、繰り返しのチェックを自動化します。

## フックの仕組み

```
ユーザーリクエスト → Codexがツールを選択 → PreToolUseフックが実行 → ツールが実行 → PostToolUseフックが実行
```

- **PreToolUse** フックはツール実行前に動作します。**ブロック**（終了コード2）または**警告**（stderrへの出力、ブロックなし）が可能です。
- **PostToolUse** フックはツール完了後に動作します。出力を分析できますが、ブロックはできません。
- **Stop** フックはCodexの各レスポンス後に動作します。
...
OpenAI Codexを手動でインストールする場合、リポジトリの生の `hooks.json` を `~/.codex/settings.json` に貼り付けたり、`~/.codex/hooks/hooks.json` に直接コピーしたりしないでください。チェックインされたファイルはプラグイン/リポジトリ向けであり、eccインストーラーを通じてインストールされるか、プラグインとして読み込まれることを想定しています。

代わりにインストーラーを使用することで、フックコマンドが実際のCodexルートに対して書き換えられます：
...
これにより解決済みのフックが `~/.codex/hooks/hooks.json` にインストールされます。Windowsでは、Codex設定ルートは `%USERPROFILE%\\.codex` です。
...
| **ドキュメントファイル警告** | `Write` | 非標準の `.md`/`.txt` ファイルについて警告（README、CODEX、CONTRIBUTING、CHANGELOG、LICENSE、SKILL、docs/、skills/ は許可）；クロスプラットフォームのパス処理 | 0（警告） |
...
`hooks.json` のフックエントリを削除またはコメントアウトします。プラグインとしてインストールされている場合は、`~/.codex/settings.json` でオーバーライドします：
...
process.stdin.on('end', () => {
  ...
  console.error('[Hook] Codexに表示される警告メッセージ');
