...
- **ルールはシステム注入**: `.codex/rules/*.md`ファイルはセッションごとに自動的に読み込まれる。プロンプト指示とは異なり、LLMはこれらを無視することを選択できない。

## 呼び出し例

```bash
codex /mail                    # メールのみのトリアージ
codex /slack                   # Slackのみのトリアージ
codex /today                   # 全チャネル + カレンダー + TODO
codex /schedule-reply "取締役会についてサラに返信"
```
