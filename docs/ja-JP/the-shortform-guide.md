...
Opus 4.5での長いコーディングセッション後にデッドコードや散らかった.mdファイルを整理したい？`/refactor-clean`を実行。テストが必要？`/tdd`、`/e2e`、`/test-coverage`。これらのスラッシュエントリーは便利ですが、真に持続的な単位は基盤となるスキルです。スキルにはコードマップも含められます — コンテキストを探索に消費せずにCodex がコードベースを素早くナビゲートする方法です。
...
- **最小限のリソース使用** — 重い操作中にCodexとRAM/CPUを競合しない。Opus実行時に重要
