# Everything OpenAI Codex 簡潔ガイド

![Header: OpenAI Hackathon Winner - Tips & Tricks for OpenAI Codex](./assets/images/shortform/00-header.png)

---

**2月の実験的ロールアウト以来、熱心なOpenAI Codexユーザーとして活動し、[@DRodriguezFX](https://x.com/DRodriguezFX)と共に[zenith.chat](https://zenith.chat)でOpenAI x Forum Venturesハッカソンで優勝しました — すべてOpenAI Codexを使用して。**
...
Deep 4.5での長いコーディングセッション後にデッドコードや散らかった.mdファイルを整理したい？`/refactor-clean`を実行。テストが必要？`/tdd`、`/e2e`、`/test-coverage`。これらのスラッシュエントリーは便利ですが、真に持続的な単位は基盤となるスキルです。スキルにはコードマップも含められます — コンテキストを探索に消費せずにCodex がコードベースを素早くナビゲートする方法です。
...
- **スキル**: `~/.codex/skills/` - 正規のワークフロー定義
- **コマンド**: `~/.codex/commands/` - まだ必要な場合のレガシースラッシュエントリーシム
...
*PostToolUseフック実行中のOpenAI Codexでのフィードバック例*
...
サブエージェントは、オーケストレーター（メインのCodex）が限定されたスコープでタスクを委任できるプロセスです。バックグラウンドまたはフォアグラウンドで実行でき、メインエージェントのコンテキストを解放します。
...
`.rules`フォルダには、Codexが**常に**従うべきベストプラクティスを含む`.md`ファイルが格納されます。2つのアプローチ：

1. **単一CODEX.md** - すべてを1ファイルに（ユーザーまたはプロジェクトレベル）
...
MCPはCodexを外部サービスに直接接続します。APIの置き換えではなく、プロンプト駆動のラッパーであり、情報のナビゲーションに柔軟性を提供します。

**例：** Supabase MCPにより、Codexはコピー&ペーストなしで特定のデータを取得し、上流で直接SQLを実行できます。データベース、デプロイメントプラットフォームなども同様です。

![Supabase MCP listing tables](./assets/images/shortform/04-supabase-mcp.jpeg)
*Supabase MCPがpublicスキーマ内のテーブルを一覧表示している例*

**Codex内のChrome：** Codexがブラウザを自律的に制御する組み込みプラグインMCP — クリックして動作を確認できます。
...
# 未使用のものを ~/.codex/settings.json または現在のリポジトリの .mcp.json で無効化
```
...
# Codexを開き、/plugins を実行、新しいマーケットプレイスを見つけてインストール
```

![Marketplaces tab showing mgrep](./assets/images/shortform/06-marketplaces-mgrep.jpeg)
*新しくインストールされたMixedbread-Grepマーケットプレイスの表示*

**LSPプラグイン**は、エディタ外でOpenAI Codexを頻繁に使用する場合に特に便利です。Language Server Protocolにより、IDEを開かずにCodexにリアルタイムの型チェック、定義ジャンプ、インテリジェント補完を提供します。

```bash
# 有効なプラグインの例
typescript-lsp@codex-plugins-official  # TypeScriptインテリジェンス
pyright-lsp@codex-plugins-official     # Python型チェック
hookify@codex-plugins-official         # フックを会話的に作成
...
- `Shift+Enter` - 複数行入力
- `Tab` - シンキング表示の切り替え
- `Esc Esc` - Codexの中断 / コード復元

### 並列ワークフロー

- **フォーク**（`/fork`）— 会話をフォークして重複しないタスクを並列実行（キューイングされたメッセージの連打の代わりに）
- **Gitワークツリー** — 競合なしで重複する並列Codexを実行。各ワークツリーは独立したチェックアウト

```bash
git worktree add ../feature-branch feature-branch
# 各ワークツリーで別々のCodexインスタンスを実行
```

### 長時間実行コマンド用tmux

Codexが実行するログ/bashプロセスのストリーミングと監視：

```bash
tmux new -s dev
# Codexがここでコマンドを実行、デタッチして再アタッチ可能
tmux attach -t dev
```
...
GitHub ActionsでPRにコードレビューを設定。設定すればCodexがPRを自動的にレビューできます。
...
リスクのある操作にはサンドボックスモードを使用 — Codexは実際のシステムに影響を与えない制限された環境で実行されます。

---

## エディタについて

エディタの選択はOpenAI Codexのワークフローに大きく影響します。OpenAI Codexは任意のターミナルから動作しますが、高機能なエディタと組み合わせることで、リアルタイムのファイル追跡、素早いナビゲーション、統合されたコマンド実行が可能になります。

### Zed（私の推奨）

[Zed](https://zed.dev)を使用しています — Rustで書かれているため、本当に高速です。即座に開き、巨大なコードベースも問題なく処理し、システムリソースをほとんど消費しません。

**Zed + OpenAI Codexが優れた組み合わせである理由：**

- **速度** — Rustベースのパフォーマンスにより、Codexがファイルを素早く編集しても遅延なし。エディタがついてこれる
- **エージェントパネル統合** — ZedのCodex統合により、Codexの編集に伴うファイル変更をリアルタイムで追跡。Codexが参照するファイル間をエディタを離れずにジャンプ
- **CMD+Shift+R コマンドパレット** — カスタムスラッシュコマンド、デバッガー、ビルドスクリプトへの検索可能なUIでの素早いアクセス
- **最小限のリソース使用** — 重い操作中にCodexとRAM/CPUを競合しない。Deep実行時に重要
...
1. **画面を分割** — 片側にOpenAI Codexのターミナル、もう片側にエディタ
2. **Ctrl + G** — Codexが現在作業中のファイルをZedで素早く開く
3. **自動保存** — 自動保存を有効にしてCodexのファイル読み取りが常に最新に
4. **Git統合** — エディタのgit機能を使ってコミット前にCodexの変更をレビュー
5. **ファイルウォッチャー** — ほとんどのエディタは変更されたファイルを自動リロード、有効になっているか確認

### VSCode / Cursor

これも実用的な選択肢でOpenAI Codexとうまく連携します。`\ide`でLSP機能を有効にしたターミナル形式（プラグインでやや冗長になりました）、またはエディタにより統合されたマッチするUIの拡張機能を選択できます。

![VS Code OpenAI Codex Extension](./assets/images/shortform/10-vscode-extension.jpeg)
*VS Code拡張機能はOpenAI CodexのネイティブグラフィカルインターフェースをIDE内に直接統合して提供。*
...
```markdown
ralph-wiggum@codex-plugins       # ループ自動化
frontend-patterns@codex-plugins  # UI/UXパターン
commit-commands@codex-plugins    # Gitワークフロー
security-guidance@codex-plugins  # セキュリティチェック
pr-review-toolkit@codex-plugins  # PR自動化
typescript-lsp@codex-plugins-official # TSインテリジェンス
hookify@codex-plugins-official        # フック作成
...
context7@codex-plugins-official       # ライブドキュメント
pyright-lsp@codex-plugins-official    # Python型
...
```json
{
  "PreToolUse": [
    ...
    { "matcher": "Write && .mdファイル", "hooks": ["README/CODEX以外はブロック"] },
...
~/.codex/rules/
  ...
  performance.md   # モデル選択（Fast vs Standard vs Deep）
  hooks.md         # フックドキュメント
```
...
- [プラグインリファレンス](https://code.openai-codex.com/docs/en/plugins-reference)
- [フックドキュメント](https://code.openai-codex.com/docs/en/hooks)
- [チェックポイント](https://code.openai-codex.com/docs/en/checkpointing)
- [インタラクティブモード](https://code.openai-codex.com/docs/en/interactive-mode)
- [メモリシステム](https://code.openai-codex.com/docs/en/memory)
- [サブエージェント](https://code.openai-codex.com/docs/en/sub-agents)
- [MCP概要](https://code.openai-codex.com/docs/en/mcp-overview)
...
*NYCでのOpenAI x Forum Venturesハッカソンで[@DRodriguezFX](https://x.com/DRodriguezFX)と共に[zenith.chat](https://zenith.chat)を構築して優勝*
