...
進階學習系統，透過原子「本能」（帶信心評分的小型學習行為）將你的 OpenAI Codex 工作階段轉化為可重用知識。
...
不需要在 `~/.codex/settings.json` 中額外加入 hook。OpenAI Codex v2.1+ 會自動載入外掛的 `hooks/hooks.json`，其中已經註冊了 `observe.sh`。

如果你之前把 `observe.sh` 複製到 `~/.codex/settings.json`，請移除重複的 `PreToolUse` / `PostToolUse` 區塊。重複註冊會造成重複執行，並觸發 `${CODEX_PLUGIN_ROOT}` 解析錯誤；這個變數只會在外掛自己的 `hooks/hooks.json` 中展開。

**如果手動安裝到 `~/.codex/skills`**，新增到你的 `~/.codex/settings.json`：
...
- [Longform Guide](https://x.com/mehmet_turac/status/2014040193557471352) - 持續學習章節
