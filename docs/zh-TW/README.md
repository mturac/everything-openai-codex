...
**來自 OpenAI 黑客松冠軍的完整 OpenAI Codex 設定集合。**
...
<img src="https://github.com/user-attachments/assets/1a471488-59cc-425b-8345-5245c7efbcef" alt="Everything OpenAI Codex 簡明指南" />
</a>
</td>
<td width="50%">
<a href="https://x.com/mehmet_turac/status/2014040193557471352">
<img src="https://github.com/user-attachments/assets/c9ca43bc-b149-427f-b551-af6840c368f0" alt="Everything OpenAI Codex 完整指南" />
</a>
</td>
</tr>
<tr>
...
> WARNING: **重要提示：** OpenAI Codex 外掛程式無法自動分發 `rules`，需要手動安裝：
...
或在 OpenAI Codex 中使用 `/setup-pm` 指令。

---

## 內容概覽

本儲存庫是一個 **OpenAI Codex 外掛程式** - 可直接安裝或手動複製元件。

```
Everything OpenAI Codex/
|-- .codex-plugin/   # 外掛程式和市集清單
...
使用本儲存庫最簡單的方式 - 安裝為 OpenAI Codex 外掛程式：
...
# 複製規則
cp Everything OpenAI Codex/rules/*.md ~/.codex/rules/
...
僅在手動安裝時，才將 `hooks/hooks.json` 中的鉤子複製到您的 `~/.codex/settings.json`。

如果您是透過 `/plugin install` 安裝 ecc，請不要再把這些鉤子複製到 `settings.json`。OpenAI Codex v2.1+ 會自動載入外掛中的 `hooks/hooks.json`，重複註冊會導致重複執行以及 `${CODEX_PLUGIN_ROOT}` 無法解析。

#### 設定 MCP

將 `mcp-configs/mcp-servers.json` 中所需的 MCP 伺服器複製到您的 `~/.codex.json`。
...
我從實驗性推出就開始使用 OpenAI Codex。2025 年 9 月與 [@DRodriguezFX](https://x.com/DRodriguezFX) 一起使用 OpenAI Codex 打造 [zenith.chat](https://zenith.chat)，贏得了 OpenAI x Forum Ventures 黑客松。
...
- **簡明指南（從這裡開始）：** [Everything OpenAI Codex 簡明指南](https://x.com/mehmet_turac/status/2012378465664745795)
- **完整指南（進階）：** [Everything OpenAI Codex 完整指南](https://x.com/mehmet_turac/status/2014040193557471352)
- **追蹤：** [@mehmet_turac](https://x.com/mehmet_turac)
