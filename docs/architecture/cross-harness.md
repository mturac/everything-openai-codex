...
OpenAI Codex, Codex, OpenCode, Cursor, Gemini, and future harnesses should adapt those assets at the edge instead of requiring a new workflow model for every tool.
...
|---------|---------------|-----------------|----------------|
| Skills | `skills/*/SKILL.md` | Codex plugin, Codex plugin, `.agents/skills`, Cursor skill copies, OpenCode plugin/config | Supported with harness-specific packaging |
| Rules and instructions | `rules/`, `AGENTS.md`, translated docs | Codex rules install, Codex `AGENTS.md`, Cursor rules, OpenCode instructions | Supported, but not identical across harnesses |
| Hooks | `hooks/hooks.json`, `scripts/hooks/` | Codex native hooks, OpenCode plugin events, Cursor hook adapter | Hook-backed in Codex/OpenCode/Cursor; instruction-backed in Codex |
| MCPs | `.mcp.json`, `mcp-configs/` | Native MCP config import per harness | Supported where the harness exposes MCP |
| Commands | `commands/`, CLI scripts | Codex slash commands, compatibility shims, CLI entrypoints | Supported, but command semantics vary |
...
- OpenAI Codex loads plugin assets and has native hook execution.
...
OpenAI Codex gets the skill through the Codex plugin surface and can enforce related hooks natively.
