...
OpenAI Codex statusline, Codex pane, dmux session, OpenCode run, or terminal-only
...
| `todos` | Current in-progress task and todo counts | Codex todos, local task files, plan metadata |
...
- `ecc session-inspect <target> --write <path>` emits canonical session
  snapshots from dmux and Codex-history adapters.
- `scripts/hooks/ecc-statusline.js` renders compact model, task, cost, tool,
  file, duration, directory, and context pressure signals inside OpenAI Codex.
