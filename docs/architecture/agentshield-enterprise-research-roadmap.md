...
- [standardagents/dmux](https://github.com/standardagents/dmux): tmux/worktree
  ...
  merge/PR workflows.
- [jarrodwatts/codex-hud](https://github.com/jarrodwatts/codex-hud): Codex
...
- [openais/codex](https://github.com/openais/codex):
...
Local OpenAI Codex source inspection:

- Reviewed only non-secret local file/module shape from a private OpenAI Codex
  source snapshot.
...
- No code was copied. The takeaway is that AgentShield should track permissions,
  ...
  first-class audit domains rather than treating a `.codex/` tree as the only
  source of truth.
...
- Rule coverage spans secrets, permissions, hooks, MCP servers, agent configs,
  ...
  evaluation, runtime repair/status, corpus validation, MiniClaw, and Deep
  analysis.
...
- Accuracy work is active: `runtimeConfidence`, template/example weighting,
  docs-example downgrades, installed Codex plugin-cache confidence,
...
`26bb44650663816d07180e0d20c1895e431a326c`. It classifies installed Codex
...
non-Codex `plugins/cache` paths, and makes plugin-cache classification win
...
- `agentshield baseline write --path .codex --output agentshield-baseline.json`
...
Superset, dmux, OpenCode, OpenAI Codex, Codex, Gemini, Zed, and terminal
multiplexers all create different security surfaces.

Target capability:

- A small adapter registry for `codex`, `opencode`, `codex`, `gemini`,
...
- Adapter metadata for OpenAI Codex, OpenCode, Codex, Gemini, dmux, generic
...
- It creates a stable surface for future Zed, Orca, Superset, and Hermes
  integration without pretending all harnesses share Codex's config model.
