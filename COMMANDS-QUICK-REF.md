...
> 59 slash commands installed globally. Type `/` in any OpenAI Codex session to invoke.
...
| `/devfleet` | Orchestrate parallel OpenAI Codex agents via DevFleet |
...
| Command | What it does |
|---------|-------------|
| `/save-session` | Save current session state to `~/.codex/session-data/` |
| `/resume-session` | Load the most recent saved session from the canonical session store and resume from where you left off |
| `/sessions` | Browse, search, and manage session history with aliases from `~/.codex/session-data/` (with legacy reads from `~/.codex/sessions/`) |
...
| `/model-route` | Route a task to the right model (Fast / Standard / Deep) |
