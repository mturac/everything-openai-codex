...
`session/manager.rs` — `agent_program()` only supports `"codex"`. The CLI accepts `--agent` but anything other than `"codex"` fails. No codex, opencode, or custom agent support.

### 3.4 Config — File-Only

`Config::load()` reads `~/.codex/ecc2.toml` only. The implementation lacks environment variable overrides (e.g., `ecc_DB_PATH`, `ecc_WORKTREE_ROOT`) and CLI flags for configuration.
...
- **No input sanitization on session task strings.** The task string is passed directly to `codex --print`. If the task contains shell metacharacters, it could be exploited depending on how `Command` handles argument quoting. Currently safe (arguments are not shell-interpreted), but worth auditing.
...
8. **Task string security audit** — The session task uses `codex --print` via `tokio::process::Command`. Verify arguments are never shell-interpreted. Checklist: confirm `Command` arg usage, threat-model metacharacter injection, input validation/escaping strategy, logging of raw inputs, and automated tests. Re-audit if invocation code changes.
