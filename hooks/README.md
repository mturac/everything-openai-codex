...
Hooks are event-driven automations that fire before or after OpenAI Codex tool executions. They enforce code quality, catch mistakes early, and automate repetitive checks.
...
For OpenAI Codex manual installs, do not paste the raw repo `hooks.json` into `~/.codex/settings.json` or copy it directly into `~/.codex/hooks/hooks.json`. The checked-in file is plugin/repo-oriented and is meant to be installed through the ecc installer or loaded as a plugin.

Use the installer instead so hook commands are rewritten against your actual Codex root:
...
That installs resolved hooks to `~/.codex/hooks/hooks.json`. On Windows, the Codex config root is `%USERPROFILE%\\.codex`.
...
| **Doc file warning** | `Write` | Warns about non-standard `.md`/`.txt` files (allows README, CODEX, CONTRIBUTING, CHANGELOG, LICENSE, SKILL, docs/, skills/); cross-platform path handling | 0 (warns) |
