# Contributing to Everything OpenAI Codex

Thanks for wanting to contribute! This repo is a community resource for OpenAI Codex users.
...
Skills are knowledge modules that OpenAI Codex loads based on context.
...
Describe scenarios where Codex should use this skill. This is critical for auto-activation.
...
| `model` | Complexity level | `fast` (simple), `standard` (coding), `deep` (complex) |
...
Hooks are automatic behaviors triggered by OpenAI Codex events.
...
- **mcp-configs/mcp-servers.json** includes a Context7 entry; users enable it in their harness (e.g. OpenAI Codex, Cursor) to use the documentation-lookup skill (in `skills/documentation-lookup/`) and the `/docs` command.
