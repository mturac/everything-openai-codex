# The Shorthand Guide to Everything OpenAI Codex

![Header: OpenAI Hackathon Winner - Tips & Tricks for OpenAI Codex](./assets/images/shortform/00-header.png)

---

**Been an avid OpenAI Codex user since the experimental rollout in Feb, and won the OpenAI x Forum Ventures hackathon with [zenith.chat](https://zenith.chat) alongside [@DRodriguezFX](https://x.com/DRodriguezFX) - completely using OpenAI Codex.**
...
After a long session of coding with GPT-5, you want to clean out dead code and loose .md files? Run `/refactor-clean`. Need testing? `/tdd`, `/e2e`, `/test-coverage`. Those slash entries are convenient, but the real durable unit is the underlying skill. Skills can also include codemaps - a way for Codex quickly navigate your codebase without burning context on exploration.
...
- **Skills**: `~/.codex/skills/` - canonical workflow definitions
- **Commands**: `~/.codex/commands/` - legacy slash-entry shims when you still need them
...
*Example of what feedback you get in OpenAI Codex, while running a PostToolUse hook*
...
1. **Single CODEX.md** - Everything in one file (user or project level)
...
# Disable unused ones in ~/.codex/settings.json or in the current repo's .mcp.json
```
...
**LSP Plugins** are particularly useful if you run OpenAI Codex outside editors frequently. Language Server Protocol gives Codex real-time type checking, go-to-definition, and intelligent completions without needing an IDE open.

```bash
# Enabled plugins example
typescript-lsp@codex-plugins-official  # TypeScript intelligence
pyright-lsp@codex-plugins-official     # Python type checking
hookify@codex-plugins-official         # Create hooks conversationally
...
- **Git Worktrees** - For overlapping parallel Codexes without conflicts. Each worktree is an independent checkout
...
Your editor choice significantly impacts OpenAI Codex workflow. While OpenAI Codex works from any terminal, pairing it with a capable editor unlocks real-time file tracking, quick navigation, and integrated command execution.
...
**Why Zed + OpenAI Codex is a great combo:**
...
- **Minimal Resource Usage** - Won't compete with Codex for RAM/CPU during heavy operations. Important when running Deep
...
1. **Split your screen** - Terminal with OpenAI Codex on one side, editor on the other
...
```markdown
ralph-wiggum@codex-plugins       # Loop automation
frontend-patterns@codex-plugins  # UI/UX patterns
commit-commands@codex-plugins    # Git workflow
security-guidance@codex-plugins  # Security checks
pr-review-toolkit@codex-plugins  # PR automation
typescript-lsp@codex-plugins-official # TS intelligence
hookify@codex-plugins-official        # Hook creation
...
context7@codex-plugins-official       # Live documentation
pyright-lsp@codex-plugins-official    # Python types
...
- [Plugins Reference](https://code.openai-codex.com/docs/en/plugins-reference)
- [Hooks Documentation](https://code.openai-codex.com/docs/en/hooks)
- [Checkpointing](https://code.openai-codex.com/docs/en/checkpointing)
- [Interactive Mode](https://code.openai-codex.com/docs/en/interactive-mode)
- [Memory System](https://code.openai-codex.com/docs/en/memory)
- [Subagents](https://code.openai-codex.com/docs/en/sub-agents)
- [MCP Overview](https://code.openai-codex.com/docs/en/mcp-overview)
...
*Won the OpenAI x Forum Ventures hackathon in NYC building [zenith.chat](https://zenith.chat) with [@DRodriguezFX](https://x.com/DRodriguezFX)*
