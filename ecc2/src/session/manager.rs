...
async fn create_session_in_dir(
    ...
    match spawn_codex_code(agent_program, task, &session.id, working_dir).await {
        Ok(pid) => {
...
fn apply_shared_harness_runtime_env(
    ...
    command.env("CODEX_SESSION_ID", session_id);
    command.env("CODEX_PROJECT_DIR", working_dir);
    command.env("CODEX_ENTRYPOINT", "cli");
    if let Some(package_manager) = resolve_project_package_manager(working_dir) {
        command.env("CODEX_PACKAGE_MANAGER", package_manager);
        command.env("CODEX_CODE_PACKAGE_MANAGER", package_manager);
    }
    if let Some(model) = profile.and_then(|profile| profile.model.as_ref()) {
        command.env("CODEX_MODEL", model);
    }
...
fn resolve_project_package_manager(working_dir: &Path) -> Option<&'static str> {
    if let Ok(package_manager) = std::env::var("CODEX_PACKAGE_MANAGER") {
    ...
    read_package_manager_from_json(
        &working_dir.join(".codex").join("package-manager.json"),
        "packageManager",
    )
    ...
    .or_else(|| {
        dirs::home_dir().and_then(|home_dir| {
            read_package_manager_from_json(
                &home_dir.join(".codex").join("package-manager.json"),
                "packageManager",
            )
        })
    })
...
fn normalize_task_for_harness(
    ...
    match harness {
        HarnessKind::Codex => task.to_string(),
...
async fn spawn_codex_code(
    ...
    let child = command
        ...
        .with_context(|| {
            format!(
                "Failed to spawn OpenAI Codex from {}",
    ...
    child
        .id()
        .ok_or_else(|| anyhow::anyhow!("OpenAI Codex did not expose a process id"))
}
...
mod tests {
    ...
    #[test]
    fn build_agent_command_applies_profile_runner_flags_for_codex() {
    ...
    fn build_agent_command_normalizes_runner_flags_for_codex() {
        ...
        assert_eq!(
            envs.get("CODEX_SESSION_ID"),
            Some(&"sess-1234".to_string())
        );
        assert_eq!(
            envs.get("CODEX_PROJECT_DIR"),
            Some(&"/tmp/repo".to_string())
        );
        assert_eq!(envs.get("CODEX_ENTRYPOINT"), Some(&"cli".to_string()));
        assert_eq!(envs.get("ECC_HARNESS"), Some(&"codex".to_string()));
        assert_eq!(envs.get("CODEX_MODEL"), Some(&"gpt-5.4".to_string()));
        assert!(
            envs.contains_key("CODEX_PLUGIN_ROOT"),
    ...
    fn build_agent_command_normalizes_runner_flags_for_opencode() {
        ...
        let profile = SessionAgentProfile {
            ...
            agent: None,
            model: Some("openai/codex-5".to_string()),
    ...
    fn build_agent_command_uses_configured_runner_for_cursor() {
        ...
        assert_eq!(envs.get("CODEX_MODEL"), Some(&"gpt-5.4".to_string()));
        assert_eq!(envs.get("ECC_PLUGIN_ROOT"), envs.get("CODEX_PLUGIN_ROOT"));
    }
    ...
    fn build_agent_command_exports_detected_package_manager_env_from_lockfile() -> Result<()> {
        ...
        let envs = command_env_map(&command);
        assert_eq!(
            envs.get("CODEX_PACKAGE_MANAGER"),
            Some(&"pnpm".to_string())
        );
        assert_eq!(
            envs.get("CODEX_CODE_PACKAGE_MANAGER"),
            Some(&"pnpm".to_string())
        );
        Ok(())
    }
    ...
    fn direct_delegate_sessions_matches_harness_aliases_for_existing_rows() -> Result<()> {
        ...
        let delegates = direct_delegate_sessions(&db, &cfg, &lead, "codex")?;
    ...
    fn write_fake_codex(root: &Path) -> Result<(PathBuf, PathBuf)> {
        let script_path = root.join("fake-codex.sh");
        let log_path = root.join("fake-codex.log");
        let script = format!(
            "#!/usr/bin/env python3\nimport os\nimport pathlib\nimport signal\nimport sys\nimport time\n\nlog_path = pathlib.Path(r\"{}\")\nlog_path.write_text(os.getcwd() + \"\\n\", encoding=\"utf-8\")\nwith log_path.open(\"a\", encoding=\"utf-8\") as handle:\n    handle.write(\" \".join(sys.argv[1:]) + \"\\n\")\n    handle.write(\"ECC_SESSION_ID=\" + os.environ.get(\"ECC_SESSION_ID\", \"\") + \"\\n\")\n    handle.write(\"CODEX_SESSION_ID=\" + os.environ.get(\"CODEX_SESSION_ID\", \"\") + \"\\n\")\n    handle.write(\"CODEX_PROJECT_DIR=\" + os.environ.get(\"CODEX_PROJECT_DIR\", \"\") + \"\\n\")\n    handle.write(\"CODEX_ENTRYPOINT=\" + os.environ.get(\"CODEX_ENTRYPOINT\", \"\") + \"\\n\")\n    handle.write(\"CODEX_PACKAGE_MANAGER=\" + os.environ.get(\"CODEX_PACKAGE_MANAGER\", \"\") + \"\\n\")\n    handle.write(\"CODEX_CODE_PACKAGE_MANAGER=\" + os.environ.get(\"CODEX_CODE_PACKAGE_MANAGER\", \"\") + \"\\n\")\n    handle.write(\"CODEX_PLUGIN_ROOT=\" + os.environ.get(\"CODEX_PLUGIN_ROOT\", \"\") + \"\\n\")\n    handle.write(\"ECC_HARNESS=\" + os.environ.get(\"ECC_HARNESS\", \"\") + \"\\n\")\n\ndef handle_term(signum, frame):\n    raise SystemExit(0)\n\nsignal.signal(signal.SIGTERM, handle_term)\nwhile True:\n    time.sleep(0.1)\n",
            log_path.display()
        );
    ...
    fn background_runner_stderr_log_path_is_session_scoped() {
        ...
        assert_eq!(
            path,
            PathBuf::from("/tmp/ecc-repo/.codex/ecc2/logs/session-123.runner-stderr.log")
        );
    }
    ...
    fn write_package_manager_project_files(
        ...
        if let Some(project_config_package_manager) = project_config_package_manager {
            ...
            fs::write(
                codex_dir.join("package-manager.json"),
    ...
    async fn create_session_spawns_process_and_marks_session_running() -> Result<()> {
        ...
        let cfg = build_config(tempdir.path());
        let db = StateStore::open(&cfg.db_path)?;
        let (fake_codex, log_path) = write_fake_codex(tempdir.path())?;
        ...
        assert!(log.contains(&format!("CODEX_SESSION_ID={session_id}")));
        assert!(log.contains(&format!(
            "CODEX_PROJECT_DIR={}",
            repo_root.to_string_lossy()
        )));
        assert!(log.contains("CODEX_ENTRYPOINT=cli"));
        assert!(log.contains("CODEX_PACKAGE_MANAGER=pnpm"));
        assert!(log.contains("CODEX_CODE_PACKAGE_MANAGER=pnpm"));
        assert!(log.contains("ECC_HARNESS=codex"));
    ...
    async fn create_session_resolves_auto_agent_from_repo_markers() -> Result<()> {
        ...
        let cfg = build_config(tempdir.path());
        let db = StateStore::open(&cfg.db_path)?;
        let (fake_runner, _log_path) = write_fake_codex(tempdir.path())?;
    ...
    async fn create_session_derives_project_and_task_group_defaults() -> Result<()> {
        ...
        let cfg = build_config(tempdir.path());
        let db = StateStore::open(&cfg.db_path)?;
        let (fake_codex, _) = write_fake_codex(tempdir.path())?;
    ...
    async fn run_due_schedules_dispatches_due_tasks_and_advances_next_run() -> Result<()> {
        ...
        let cfg = build_config(tempdir.path());
        let db = StateStore::open(&cfg.db_path)?;
        let (fake_runner, log_path) = write_fake_codex(tempdir.path())?;
    ...
    async fn create_session_uses_default_agent_profile_and_persists_launch_settings() -> Result<()>
    {
        ...
        let db = StateStore::open(&cfg.db_path)?;
        let (fake_runner, _) = write_fake_codex(tempdir.path())?;
        ...
        assert_eq!(profile.model.as_deref(), Some("standard"));
    ...
    async fn resume_session_requeues_failed_session() -> Result<()> {
        ...
        let resumed_id =
            resume_session_with_program(&db, &cfg, "deadbeef", Some(&fake_codex)).await?;
        let resumed = db
    ...
    async fn drain_inbox_routes_unread_task_handoffs_and_marks_them_read() -> Result<()> {
        ...
        let outcomes = drain_inbox(&db, &cfg, "lead", "codex", true, 5).await?;
        assert_eq!(outcomes.len(), 1);
    ...
    async fn drain_inbox_routes_high_priority_handoff_first() -> Result<()> {
        ...
        let outcomes = drain_inbox(&db, &cfg, "lead", "codex", true, 1).await?;
        assert_eq!(outcomes.len(), 1);
    ...
    async fn auto_dispatch_backlog_routes_multiple_lead_inboxes() -> Result<()> {
        ...
        let outcomes = auto_dispatch_backlog(&db, &cfg, "codex", true, 10).await?;
    ...
    async fn coordinate_backlog_reports_remaining_backlog_after_limited_pass() -> Result<()> {
        ...
        let outcome = coordinate_backlog(&db, &cfg, "codex", true, 1).await?;
    ...
    async fn coordinate_backlog_classifies_remaining_saturated_pressure() -> Result<()> {
        ...
        let outcome = coordinate_backlog(&db, &cfg, "codex", true, 10).await?;
    ...
    async fn rebalance_team_backlog_moves_work_off_backed_up_delegate() -> Result<()> {
        ...
        let outcomes = rebalance_team_backlog(&db, &cfg, "lead", "codex", true, 5).await?;
        assert_eq!(outcomes.len(), 1);
