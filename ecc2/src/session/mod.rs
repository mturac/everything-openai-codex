...
impl HarnessKind {
    ...
    fn supports_direct_execution(self) -> bool {
        matches!(
            self,
            Self::Codex | Self::Codex | Self::OpenCode | Self::Gemini
        )
    }
...
impl SessionHarnessInfo {
    ...
    pub fn resolve_requested_agent_type(
        ...
        HarnessKind::Codex.as_str().to_string()
    }
...
mod tests {
    ...
    fn detect_session_harness_prefers_agent_type_and_collects_project_markers(
        ...
        let harness = SessionHarnessInfo::detect("codex", repo.path());
        assert_eq!(harness.primary, HarnessKind::Codex);
        assert_eq!(harness.primary_label, "codex");
        assert_eq!(
            harness.detected,
            vec![HarnessKind::Codex, HarnessKind::Codex]
        );
        assert_eq!(harness.detected_labels, vec!["codex", "codex"]);
        assert_eq!(harness.detected_summary(), "codex");
        Ok(())
    }
    ...
    fn canonical_agent_type_normalizes_known_aliases() {
        assert_eq!(HarnessKind::canonical_agent_type("openai-codex"), "codex");
    ...
    fn runner_key_uses_canonical_label_for_unknown_harnesses() {
        ...
        assert_eq!(SessionHarnessInfo::runner_key("openai-codex"), "codex");
    }
    ...
    #[test]
    fn resolve_requested_agent_type_falls_back_to_codex_without_markers() {
        let resolved = SessionHarnessInfo::resolve_requested_agent_type(
            &crate::config::Config::default(),
            "auto",
            Path::new("."),
