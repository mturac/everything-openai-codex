...
impl Default for Config {
    fn default() -> Self {
        ...
        Self {
            db_path: home.join(".codex").join("ecc2.db"),
...
impl Config {
    ...
    fn project_config_paths_from(start: &std::path::Path) -> Vec<PathBuf> {
        ...
        while let Some(path) = current {
            let legacy = path.join(".codex").join("ecc2.toml");
...
"#,
    ...
    fn project_config_discovery_prefers_nearest_directory_and_new_path() {
        ...
        std::fs::write(project_root.join(".codex").join("ecc2.toml"), "").unwrap();
        ...
        assert_eq!(
            paths,
            vec![
                project_root.join(".codex").join("ecc2.toml"),
...
"#,
        ...
        assert_eq!(template.steps[0].agent.as_deref(), Some("codex"));
