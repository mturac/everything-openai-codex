...
function runCatalogValidator(overrides = {}) {
  ...
  const resolvedOverrides = {
    ...
    PLUGIN_JSON_PATH: path.join(repoRoot, '.codex-plugin', 'plugin.json'),
    MARKETPLACE_JSON_PATH: path.join(repoRoot, '.codex-plugin', 'marketplace.json'),
    ...overrides,
  };
...
function writeCatalogFixture(testDir, options = {}) {
  ...
  const pluginJsonPath = path.join(testDir, '.codex-plugin', 'plugin.json');
  const marketplaceJsonPath = path.join(testDir, '.codex-plugin', 'marketplace.json');
  ...
  fs.mkdirSync(path.join(testDir, '.codex-plugin'), { recursive: true });
...
function runTests() {
  ...
  if (test('rejects agent with empty tools value', () => {
    const testDir = createTestDir();
    fs.writeFileSync(path.join(testDir, 'empty.md'), '---\nmodel: codex-standard-4-5-20250929\ntools:\n---\n# Empty tools');
    const result = runValidatorWithDir('validate-agents', 'AGENTS_DIR', testDir);
  ...
  if (test('accepts all valid model values (fast, standard, deep)', () => {
    const testDir = createTestDir();
    fs.writeFileSync(path.join(testDir, 'fast.md'),
      '---\nmodel: fast\ntools: Read\n---\n# Fast Agent');
    fs.writeFileSync(path.join(testDir, 'standard.md'),
      '---\nmodel: standard\ntools: Read, Write\n---\n# Standard Agent');
    fs.writeFileSync(path.join(testDir, 'deep.md'),
      '---\nmodel: deep\ntools: Read, Write, Bash\n---\n# Deep Agent');
  ...
  if (test('rejects uppercase model value (case-sensitive check)', () => {
    ...
    assert.ok(result.stderr.includes('Invalid model'), 'Should report invalid model');
    assert.ok(result.stderr.includes('Fast'), 'Should show the rejected value');
    cleanupTestDir(testDir);
  })) passed++; else failed++;

  if (test('handles space before colon in frontmatter key', () => {
    const testDir = createTestDir();
    // "model : standard" — space before colon. extractFrontmatter uses indexOf(':') + trim()
