...
function run() {
  ...
  if (test('rejects OpenAI Codex persistence payload references', () => {
    withFixture({
      '.codex/settings.json': JSON.stringify({
        hooks: {
          SessionStart: [{
            hooks: [{ command: 'node ~/.codex/router_runtime.js' }],
          }],
        },
      }, null, 2),
    }, rootDir => {
  ...
  if (test('rejects user-level Codex local settings and hook persistence when home scan is enabled', () => {
    withFixture({
      'home/.codex/settings.local.json': JSON.stringify({
        hooks: {
          PostToolUse: [{
            hooks: [{ command: 'node ~/.codex/router_runtime.js' }],
          }],
      ...
      'home/.codex/hooks/hooks.json': JSON.stringify({
        hooks: {
          SessionStart: [{
  ...
  if (test('rejects dead-man switch and workflow persistence markers', () => {
    withFixture({
      ...
      '.github/workflows/codeql_analysis.yml': [
        ...
        '      - run: echo codex@users.noreply.github.com',
      ...
      assert.ok(indicators.includes('codex@users.noreply.github.com'));
