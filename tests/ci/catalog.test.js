...
function runTests() {
  ...
  if (test('reports mismatches from every tracked catalog document', () => {
    ...
    try {
      ...
      assert.ok(formatted.includes('.codex-plugin/plugin.json description'));
      assert.ok(formatted.includes('.codex-plugin/marketplace.json plugin description'));
  ...
  if (test('write mode syncs counts while preserving plus suffixes and unrelated tables', () => {
    ...
    try {
      ...
      const pluginJson = fs.readFileSync(path.join(testDir, '.codex-plugin', 'plugin.json'), 'utf8');
      const marketplaceJson = fs.readFileSync(path.join(testDir, '.codex-plugin', 'marketplace.json'), 'utf8');
