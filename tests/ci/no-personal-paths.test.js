...
record(test('flags a leaked /Users/<name> path', () => {
  ...
  try {
    writeFile(path.join(testDir, 'skills', 'leaky', 'SKILL.md'), 'See /Users/sugig/.codex/settings.json\n');
...
record(test('flags a leaked C:\\Users\\<name> path case-insensitively', () => {
  const testDir = createTestDir();
  try {
    writeFile(path.join(testDir, 'docs', 'guide.md'), 'See C:\\Users\\codexuser\\projects\\thing\n');
    const result = runValidatorAgainst(testDir);
    assert.strictEqual(result.code, 1, 'expected non-zero exit on leak');
    assert.ok(result.stderr.includes('C:\\Users\\codexuser'), `expected stderr to mention leaked path; got: ${result.stderr}`);
  } finally {
    cleanupTestDir(testDir);
  }
}));
...
record(test('allows /Users/<placeholder> templates', () => {
  ...
  try {
    writeFile(path.join(testDir, 'commands', 'demo.md'), [
      '/Users/you/.codex/session.json',
      '/Users/example/.codex/rules/foo.md',
      '/Users/yourname/projects/app',
      '/Users/your-username/.codex/settings.json',
      'C:\\Users\\USER\\.codex\\settings.json',
    ].join('\n'));
    const result = runValidatorAgainst(testDir);
...
record(test('exempts docs/fixes forensic reports', () => {
  ...
  try {
    writeFile(
      ...
      'Reporter ran: C:\\Users\\sugig\\.codex\\settings.local.json\n',
    );
    const result = runValidatorAgainst(testDir);
...
record(test('reports every leak on a single offending file', () => {
  ...
  try {
    writeFile(path.join(testDir, 'skills', 'multi', 'SKILL.md'), [
      '/Users/sugig/.codex/a.json',
      '/Users/sugig/.codex/b.json',
      'C:\\Users\\foo\\bar',
    ].join('\n'));
    const result = runValidatorAgainst(testDir);
    assert.strictEqual(result.code, 1, 'expected non-zero exit on leak');
