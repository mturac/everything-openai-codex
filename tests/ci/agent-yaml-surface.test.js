...
function run() {
  ...
  if (test('canonical OpenAI skills are not re-bundled in active ecc skill surfaces', () => {
    for (const skillName of CANONICAL_OPENAI_SKILLS) {
      assert.ok(
        !fs.existsSync(path.join(SKILLS_DIR, skillName, 'SKILL.md')),
        `${skillName} should be installed from openais/skills, not ecc skills/`
      );
      assert.ok(
        !fs.existsSync(path.join(CODEX_SKILLS_DIR, skillName, 'SKILL.md')),
        `${skillName} should be installed from openais/skills, not ecc .agents/skills/`
      );
    }
  })) passed++; else failed++;
