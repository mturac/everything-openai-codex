...
for (const relativePath of skillDocs) {
  ...
  test(`${relativePath} does not tell plugin users to register observe.sh through CODEX_PLUGIN_ROOT`, () => {
    assert.ok(
      !content.includes('${CODEX_PLUGIN_ROOT}/skills/continuous-learning-v2/hooks/observe.sh'),
