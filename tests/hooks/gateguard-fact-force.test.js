...
function runHook(input, env = {}) {
  ...
  const result = spawnSync('node', [
    ...
    env: {
      ...
      CODEX_SESSION_ID: TEST_SESSION_ID,
      ...env
    },
    timeout: 15000,
...
function runTests() {
  ...
  // --- Test 16: allows Codex settings edits so the hook can be disabled safely ---
  clearState();
  if (test('allows edits to .codex/settings.json without gating', () => {
    const input = {
      tool_name: 'Edit',
      tool_input: { file_path: '/workspace/app/.codex/settings.json', old_string: '{}', new_string: '{"hooks":[]}' }
    };
    const result = runHook(input);
    const output = parseOutput(result.stdout);
    assert.ok(output, 'should produce valid JSON output');
  ...
  if (test('uses project directory fallback when no session or transcript id exists', () => {
    ...
    const fallbackEnv = {
      ...
      ecc_SESSION_ID: '',
      CODEX_TRANSCRIPT_PATH: '',
      CODEX_PROJECT_DIR: path.join(stateDir, 'project-root'),
    };
  ...
  function runFreshSessionEdit(filePath, extra = {}) {
    return runHook({
      ...
      ...extra
    }, { CODEX_SESSION_ID: '', ecc_SESSION_ID: '' });
  }
