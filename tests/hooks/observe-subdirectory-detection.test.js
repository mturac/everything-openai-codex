...
test('observe.sh resolves cwd to git root before setting CODEX_PROJECT_DIR', () => {
...
test('observe.sh falls back to CODEX_HOOK_EVENT_NAME when no phase argument is passed', () => {
  const testRoot = createTempDir();
  ...
  try {
    ...
    const result = runObserve({
      ...
      cwd: repoDir,
      args: [],
      extraEnv: { CODEX_HOOK_EVENT_NAME: 'PreToolUse' },
    });
    assert.strictEqual(result.status, 0, result.stderr);
