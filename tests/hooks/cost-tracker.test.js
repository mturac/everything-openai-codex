...
function runTests() {
  ...
  (test('passes through input on stdout', () => {
    const input = {
      model: 'codex-standard-4-20250514',
  ...
  (test('creates metrics file when given transcript usage data', () => {
    ...
    const metricsFile = path.join(tmpHome, '.codex', 'metrics', 'costs.jsonl');
    ...
    assert.strictEqual(row.model, 'codex-deep-4-20250514', 'Expected last assistant model to be recorded');
  ...
  (test('handles missing usage fields gracefully', () => {
    const tmpHome = makeTempDir();
    const input = { model: 'codex-standard-4-20250514' };
    const inputStr = JSON.stringify(input);
    const result = runScript(input, withTempHome(tmpHome));
    assert.strictEqual(result.code, 0, `Expected exit code 0, got ${result.code}`);
    assert.strictEqual(result.stdout, inputStr, 'Expected stdout to match original input');
  ...
  (test('prefers ecc_SESSION_ID over CODEX_SESSION_ID when both are present', () => {
    const tmpHome = makeTempDir();
    const input = {
      model: 'codex-standard-4-20250514',
    ...
    const result = runScript(input, {
      ...
      CODEX_SESSION_ID: 'codex-session-9999',
    });
    assert.strictEqual(result.code, 0, `Expected exit code 0, got ${result.code}`);
