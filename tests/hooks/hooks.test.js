...
function getCanonicalSessionsDir(homeDir) {
  return path.join(homeDir, '.codex', 'session-data');
}
...
async function runTests() {
  ...
  if (
    await asyncTest('creates or updates session file', async () => {
      ...
      try {
        ...
        // Note: Without CODEX_SESSION_ID, falls back to project/worktree name (not 'default')
    ...
    await asyncTest('derives shortId from transcript_path UUID when available', async () => {
      ...
      try {
        ...
        await runScript(path.join(scriptsDir, 'session-end.js'), stdinJson, {
          HOME: isoHome,
          USERPROFILE: isoHome,
          // Clear CODEX_SESSION_ID so parent-process env does not leak into the
  ...
  // Regression test for #1494: when CODEX_SESSION_ID and transcript_path refer to the
  ...
  if (
    await asyncTest('matches getSessionIdShort when transcript UUID equals CODEX_SESSION_ID', async () => {
      ...
      const expectedShortId = 'ccddeeff'; // last 8 chars of both transcript UUID and CODEX_SESSION_ID
      ...
      try {
        ...
        assert.ok(fs.existsSync(sessionFile), `Session filename should match the pre-fix CODEX_SESSION_ID-based name: ${sessionFile}`);
      } finally {
        fs.rmSync(isoHome, { recursive: true, force: true });
      }
    })
  ...
  if (
    await asyncTest('reads session_id from stdin JSON (OpenAI Codex wire format)', async () => {
  ...
  if (
    await asyncTest('stdin session_id takes precedence over env CODEX_SESSION_ID', async () => {
      ...
      const result = await runScript(path.join(scriptsDir, 'suggest-compact.js'), stdinJson, {
        CODEX_SESSION_ID: envSession
      });
      assert.strictEqual(result.code, 0);

      const stdinCounter = path.join(os.tmpdir(), `codex-tool-count-${stdinSession}`);
      const envCounter = path.join(os.tmpdir(), `codex-tool-count-${envSession}`);
  ...
  if (
    await asyncTest('respects CODEX_PACKAGE_MANAGER for formatter fallback runner', async () => {
      const testDir = createTestDir();
      ...
      const stdinJson = JSON.stringify({ tool_input: { file_path: filePath } });
      const result = await runScript(path.join(scriptsDir, 'post-edit-format.js'), stdinJson, withPrependedPath(binDir, { CODEX_PACKAGE_MANAGER: 'pnpm' }));
  ...
  if (
    await asyncTest('respects project package-manager config for formatter fallback runner', async () => {
      ...
      fs.writeFileSync(path.join(rootDir, '.codex', 'package-manager.json'), JSON.stringify({ packageManager: 'bun' }));
      fs.writeFileSync(path.join(rootDir, '.prettierrc'), '{}');
      fs.writeFileSync(filePath, 'export const value = 1;\n');
  ...
  if (
    await asyncTest('uses CODEX_TRANSCRIPT_PATH env var as fallback', async () => {
      const testDir = createTestDir();
      const transcriptPath = path.join(testDir, 'transcript.jsonl');
  ...
  if (
    await asyncTest('escapes backticks in user messages in session file', async () => {
      ...
      const codexDir = getCanonicalSessionsDir(testDir);
      if (fs.existsSync(codexDir)) {
        const files = fs.readdirSync(codexDir).filter(f => f.endsWith('.tmp'));
        if (files.length > 0) {
  ...
  if (
    await asyncTest('parses OpenAI Codex JSONL format (entry.message.content)', async () => {
      const testDir = createTestDir();
      const transcriptPath = path.join(testDir, 'transcript.jsonl');

      // OpenAI Codex v2.1.41+ JSONL format: user messages nested in entry.message
  ...
  if (
    await asyncTest('extracts tool_use from assistant message content blocks', async () => {
      ...
      // OpenAI Codex JSONL: tool uses nested in assistant message content array
      const lines = [
        '{"type":"user","content":"Edit the config"}',
        JSON.stringify({
          type: 'assistant',
  ...
  if (
    test('all hook commands use string form for OpenAI Codex schema compatibility', () => {
      const hooksPath = path.join(__dirname, '..', '..', 'hooks', 'hooks.json');
      const hooks = JSON.parse(fs.readFileSync(hooksPath, 'utf8'));
  ...
  if (
    test('SessionStart hook uses safe inline resolver without plugin-tree scanning', () => {
      ...
      assert.strictEqual(typeof sessionStartHook.command, 'string', 'SessionStart should use string command form for OpenAI Codex compatibility');
      assert.ok(
        commandText.includes('session-start-bootstrap.js'),
        'SessionStart should delegate to the extracted bootstrap script'
      );
      assert.ok(commandText.includes('CODEX_PLUGIN_ROOT'), 'SessionStart should use CODEX_PLUGIN_ROOT');
      assert.ok(!commandText.includes('${CODEX_PLUGIN_ROOT}'), 'SessionStart should not depend on raw shell placeholder expansion');
      ...
      assert.ok(bootstrapSrc.includes('CODEX_PLUGIN_ROOT'), 'Bootstrap should consult CODEX_PLUGIN_ROOT');
  ...
  if (
    test('Stop and SessionEnd hooks use the safe inline resolver when plugin root may be unset', () => {
      ...
      for (const hook of [...stopHooks, ...sessionEndHooks]) {
        ...
        assert.ok(commandText.includes('CODEX_PLUGIN_ROOT'), 'Lifecycle hook should consult CODEX_PLUGIN_ROOT');
        assert.ok(!commandText.includes('${CODEX_PLUGIN_ROOT}'), 'Lifecycle hook should not depend on raw shell placeholder expansion');
        ...
        for (const entry of hookArray) {
          for (const hook of entry.hooks) {
            ...
            if (hook.type === 'command' && commandText.includes('scripts/hooks/')) {
              ...
              assert.ok(!commandText.includes('${CODEX_PLUGIN_ROOT}'), `Script paths should not depend on raw shell placeholder expansion: ${commandText.substring(0, 80)}...`);
              assert.ok(
  ...
  if (
    test('plugin.json does NOT have explicit hooks declaration', () => {
      ...
      assert.ok(!plugin.hooks, 'plugin.json should NOT have "hooks" field - OpenAI Codex auto-loads hooks/hooks.json');
    })
  )
    passed++;
  else failed++;
  ...
  if (
    await asyncTest('handles malformed stdin JSON (falls back to env var)', async () => {
      const result = await runScript(path.join(scriptsDir, 'evaluate-session.js'), 'not json at all', { CODEX_TRANSCRIPT_PATH: '' });
  ...
  if (
    test('run-with-flags-shell resolves plugin root when CODEX_PLUGIN_ROOT is unset', () => {
      const wrapperSource = fs.readFileSync(path.join(scriptsDir, 'run-with-flags-shell.sh'), 'utf8');
      assert.ok(wrapperSource.includes('PLUGIN_ROOT="${CODEX_PLUGIN_ROOT:-'), 'Shell wrapper should derive PLUGIN_ROOT from its own script path');
    })
  )
    passed++;
  else failed++;
  ...
  if (
    test('observer-loop uses a configurable max-turn budget with safe default', () => {
      ...
      assert.ok(observerLoopSource.includes('prompt_content="$(cat "$prompt_file" 2>/dev/null || true)"'), 'observer-loop should read prompt_file into memory before codex is spawned');
      assert.ok(observerLoopSource.includes('-p "$prompt_content"'), 'observer-loop should pass in-memory prompt content to codex');
  ...
  if (SKIP_BASH) { console.log("  ⊘ observe.sh skips minimal hook profile (skipped on Windows)"); passed++; } else if (
    await asyncTest('observe.sh skips minimal hook profile before project detection side effects', async () => {
      await assertObserveSkipBeforeProjectDetection({
        ...
        env: { CODEX_ENTRYPOINT: 'cli', ecc_HOOK_PROFILE: 'minimal' }
      });
    })
  )
    passed++;
  ...
  if (SKIP_BASH) { console.log("  ⊘ observe.sh skips cooperative skip env (skipped on Windows)"); passed++; } else if (
    await asyncTest('observe.sh skips cooperative skip env before project detection side effects', async () => {
      await assertObserveSkipBeforeProjectDetection({
        ...
        env: { CODEX_ENTRYPOINT: 'cli', ecc_SKIP_OBSERVE: '1' }
      });
    })
  )
    passed++;
    ...
    await asyncTest('observe.sh skips configured observer-session paths before project detection side effects', async () => {
      await assertObserveSkipBeforeProjectDetection({
        ...
        env: {
          ...
          ecc_OBSERVE_SKIP_PATHS: ' observer-sessions , .codex-mem '
        },
  ...
  if (
    await asyncTest('extracts tool_use from assistant message content blocks', async () => {
      const testDir = createTestDir();
      const transcriptPath = path.join(testDir, 'transcript.jsonl');
      // OpenAI Codex JSONL format: tool_use blocks inside assistant message content array
      const lines = [
  ...
  if (
    await asyncTest('reports available session aliases on startup', async () => {
      ...
      fs.writeFileSync(
        path.join(isoHome, '.codex', 'session-aliases.json'),
        JSON.stringify({
          version: '1.0',
          aliases: {
