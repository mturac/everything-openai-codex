...
test('observer-loop loads prompt content before invoking codex', () => {
  const content = fs.readFileSync(observerLoopPath, 'utf8');
  assert.ok(content.includes('prompt_content="$(cat "$prompt_file" 2>/dev/null || true)"'), 'Prompt should be read into memory before the codex invocation');
  assert.ok(content.includes('-p "$prompt_content"'), 'Codex should receive the in-memory prompt content');
  assert.ok(!content.includes('-p "$(cat "$prompt_file")"'), 'Codex should not depend on re-reading the prompt file during invocation');
});
...
test('prompt references analysis_file not full OBSERVATIONS_FILE', () => {
  ...
  assert.ok(heredocEnd > heredocStart, 'Should find prompt heredoc end');
  const promptSection = content.substring(heredocStart, heredocEnd);
  assert.ok(promptSection.includes('${analysis_relpath}'), 'Prompt should point Codex at the sampled analysis file (via relative path), not the full observations file');
});

test('observer-loop wait helper retries SIGUSR1-interrupted waits while codex child is alive', () => {
  if (process.platform === 'win32') {
    return;
  }

  const content = fs.readFileSync(observerLoopPath, 'utf8');
  const helperMatch = content.match(/wait_for_codex_analysis\(\) \{[\s\S]*?\n\}/);
  assert.ok(helperMatch, 'observer-loop.sh should define wait_for_codex_analysis helper');
  ...
  const script = [
    ...
    'signaler=$!',
    'wait_for_codex_analysis "$codex_child"',
    'status=$?',
...
test('observer-loop wait helper preserves real nonzero codex exits', () => {
  if (process.platform === 'win32') {
    return;
  }
...
// ──────────────────────────────────────────────────────
// Test group 7: Observer Fast invocation flags
// ──────────────────────────────────────────────────────

console.log('\n--- Observer Fast invocation flags ---');

test('codex invocation includes --allowedTools flag', () => {
  const content = fs.readFileSync(observerLoopPath, 'utf8');
  assert.ok(content.includes('--allowedTools'), 'observer-loop.sh should include --allowedTools flag in codex invocation');
});
...
test('codex invocation still includes ecc_SKIP_OBSERVE and ecc_HOOK_PROFILE guards', () => {
  const content = fs.readFileSync(observerLoopPath, 'utf8');
  // Find the codex execution line(s)
  const lines = content.split('\n');
  const codexLine = lines.find(l => l.includes('codex --model fast'));
  assert.ok(codexLine, 'Should find codex --model fast invocation line');
  // The env vars are on the same line as the codex command
  const codexLineIndex = lines.indexOf(codexLine);
  const fullCommand = lines.slice(Math.max(0, codexLineIndex - 1), codexLineIndex + 3).join(' ');
  assert.ok(fullCommand.includes('ecc_SKIP_OBSERVE=1'), 'codex invocation should include ecc_SKIP_OBSERVE=1 guard');
  assert.ok(fullCommand.includes('ecc_HOOK_PROFILE=minimal'), 'codex invocation should include ecc_HOOK_PROFILE=minimal guard');
});
