...
async function runTests() {
  ...
  if (await test('run() emits hook_input_truncated event without logging raw command text', async () => {
    const original = process.env.ecc_GOVERNANCE_CAPTURE;
    const originalHookEvent = process.env.CODEX_HOOK_EVENT_NAME;
    const originalWrite = process.stderr.write;
    const stderr = [];
    process.env.ecc_GOVERNANCE_CAPTURE = '1';
    process.env.CODEX_HOOK_EVENT_NAME = 'PreToolUse';
    ...
    try {
      ...
      if (originalHookEvent !== undefined) {
        process.env.CODEX_HOOK_EVENT_NAME = originalHookEvent;
      } else {
        delete process.env.CODEX_HOOK_EVENT_NAME;
      }
    }
