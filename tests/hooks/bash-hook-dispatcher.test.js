...
function runTests() {
  ...
  if (test('post dispatcher writes both bash audit and cost logs in one pass', () => {
    ...
    try {
      ...
      const auditLog = fs.readFileSync(path.join(homeDir, '.codex', 'bash-commands.log'), 'utf8');
      const costLog = fs.readFileSync(path.join(homeDir, '.codex', 'cost-tracker.log'), 'utf8');
