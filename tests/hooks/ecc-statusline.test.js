...
function runTests() {
  ...
  if (
    test('reads in-progress task for sanitized session ID only', () => {
      ...
      const originalConfig = process.env.CODEX_CONFIG_DIR;
      try {
        process.env.CODEX_CONFIG_DIR = tmpConfig;
        ...
        if (originalConfig === undefined) delete process.env.CODEX_CONFIG_DIR;
        else process.env.CODEX_CONFIG_DIR = originalConfig;
