...
function readMetricRows(homeDir) {
  const metricsFile = path.join(homeDir, '.codex', 'metrics', 'tool-usage.jsonl');
...
function runTests() {
  ...
  (test('passes through input on stdout', () => {
    ...
    const result = runScript(input, {
      CODEX_HOOK_EVENT_NAME: 'PostToolUse',
  ...
  (test('creates tool activity metrics rows with file paths', () => {
    ...
    const metricsFile = path.join(tmpHome, '.codex', 'metrics', 'tool-usage.jsonl');
  ...
  (test('prefers ecc_SESSION_ID over CODEX_SESSION_ID and redacts bash summaries', () => {
    const tmpHome = makeTempDir();
    const input = {
      tool_name: 'Bash',
      tool_input: {
    ...
    const result = runScript(input, {
      ...
      CODEX_SESSION_ID: 'codex-session-2',
    });
    assert.strictEqual(result.code, 0);
  ...
  (test('skips non-PostToolUse events and rows without required identifiers', () => {
    assert.strictEqual(buildActivityRow(
      ...
      { CODEX_HOOK_EVENT_NAME: 'PreToolUse', ecc_SESSION_ID: 'sess' }
    ), null);
    assert.strictEqual(buildActivityRow(
      { tool_name: 'Read', tool_input: { file_path: 'README.md' } },
      { CODEX_HOOK_EVENT_NAME: 'PostToolUse' }
    ...
    assert.strictEqual(buildActivityRow(
      { tool_input: { file_path: 'README.md' } },
      { CODEX_HOOK_EVENT_NAME: 'PostToolUse', ecc_SESSION_ID: 'sess' }
    ), null);
  }) ? passed++ : failed++);
  ...
  (test('sanitizes nested params, long summaries, and output variants', () => {
    ...
    const row = buildActivityRow(
      ...
      { CODEX_HOOK_EVENT_NAME: 'PostToolUse', CODEX_SESSION_ID: 'codex-fallback' }
    );

    assert.strictEqual(row.session_id, 'codex-fallback');
  ...
  (test('passes empty stdin through without creating metrics', () => {
    ...
    assert.strictEqual(
      fs.existsSync(path.join(tmpHome, '.codex', 'metrics', 'tool-usage.jsonl')),
      false
    );
