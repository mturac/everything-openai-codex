...
function runTests() {
  ...
  if (test('creates counter file on first run', () => {
    ...
    cleanup();
    const result = runCompact({ CODEX_SESSION_ID: sessionId });
  ...
  if (test('increments counter on subsequent runs', () => {
    const { sessionId, counterFile, cleanup } = createCounterContext();
    cleanup();
    runCompact({ CODEX_SESSION_ID: sessionId });
    runCompact({ CODEX_SESSION_ID: sessionId });
    runCompact({ CODEX_SESSION_ID: sessionId });
    const count = parseInt(fs.readFileSync(counterFile, 'utf8').trim(), 10);
  ...
  if (test('suggests compact at threshold (COMPACT_THRESHOLD=3)', () => {
    ...
    runCompact({ CODEX_SESSION_ID: sessionId, COMPACT_THRESHOLD: '3' });
    runCompact({ CODEX_SESSION_ID: sessionId, COMPACT_THRESHOLD: '3' });
    const result = runCompact({ CODEX_SESSION_ID: sessionId, COMPACT_THRESHOLD: '3' });
    assert.ok(
  ...
  if (test('does NOT suggest compact before threshold', () => {
    const { sessionId, cleanup } = createCounterContext();
    cleanup();
    runCompact({ CODEX_SESSION_ID: sessionId, COMPACT_THRESHOLD: '5' });
    const result = runCompact({ CODEX_SESSION_ID: sessionId, COMPACT_THRESHOLD: '5' });
    assert.ok(
  ...
  if (test('ignores non-numeric COMPACT_THRESHOLD', () => {
    ...
    cleanup();
    fs.writeFileSync(counterFile, '49');
    const result = runCompact({ CODEX_SESSION_ID: sessionId, COMPACT_THRESHOLD: 'abc' });
  ...
  if (test('rejects COMPACT_THRESHOLD=0 (falls back to 50)', () => {
    ...
    cleanup();
    fs.writeFileSync(counterFile, '49');
    const result = runCompact({ CODEX_SESSION_ID: sessionId, COMPACT_THRESHOLD: '0' });
  ...
  if (test('accepts COMPACT_THRESHOLD=10000 (boundary max)', () => {
    ...
    const result = runCompact({ CODEX_SESSION_ID: sessionId, COMPACT_THRESHOLD: '10000' });
  ...
  if (test('rejects COMPACT_THRESHOLD=10001 (falls back to 50)', () => {
    ...
    cleanup();
    fs.writeFileSync(counterFile, '49');
    const result = runCompact({ CODEX_SESSION_ID: sessionId, COMPACT_THRESHOLD: '10001' });
  ...
  if (test('rejects float COMPACT_THRESHOLD (e.g. 3.5)', () => {
    ...
    cleanup();
    fs.writeFileSync(counterFile, '49');
    const result = runCompact({ CODEX_SESSION_ID: sessionId, COMPACT_THRESHOLD: '3.5' });
  ...
  // OpenAI Codex 2.1+ drops non-blocking PreToolUse stderr; the suggestion has
