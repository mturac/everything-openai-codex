...
function runTests() {
  ...
  if (test('lists registered adapters', () => {
    ...
    assert.ok(payload.adapters.some(adapter => adapter.id === 'codex-history'));
    assert.ok(payload.adapters.some(adapter => adapter.id === 'dmux-tmux'));
  })) passed++; else failed++;

  if (test('prints canonical JSON for codex history targets', () => {
    ...
    try {
      ...
      assert.strictEqual(payload.adapterId, 'codex-history');
  ...
  if (test('supports explicit target types for structured registry routing', () => {
    ...
    try {
      ...
      const result = run(['latest', '--target-type', 'codex-history'], {
        env: { HOME: homeDir }
      });
      ...
      const payload = JSON.parse(result.stdout);
      assert.strictEqual(payload.adapterId, 'codex-history');
      assert.strictEqual(payload.session.sourceTarget.type, 'codex-history');
  ...
  if (test('writes snapshot JSON to disk when --write is provided', () => {
    ...
    try {
      ...
      const result = run(['codex:latest', '--write', outputPath], {
        env: { HOME: homeDir }
      });
      ...
      assert.strictEqual(written.adapterId, 'codex-history');
    } finally {
      fs.rmSync(homeDir, { recursive: true, force: true });
  ...
  if (test('inspects skill health from recorded observations', () => {
    ...
    const observationsDir = path.join(projectRoot, '.codex', 'ecc', 'skills');
