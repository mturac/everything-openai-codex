...
function writeTranscript(homeDir, projectSlug, fileName, entries) {
  const transcriptDir = path.join(homeDir, '.codex', 'projects', projectSlug);
...
function runTests() {
  ...
  if (test('reports overdue ScheduleWakeup calls from Codex transcripts', () => {
    const homeDir = createTempHome();

    try {
      const transcriptPath = writeTranscript(homeDir, '-Users-dev-project-a', 'session-a.jsonl', [
  ...
  if (test('analyzeTranscript applies default thresholds when called directly', () => {
    ...
    try {
      const transcriptPath = writeTranscript(homeDir, '-Users-dev-project-direct', 'session-direct.jsonl', [
  ...
  if (test('reports stale Bash tool_use entries without matching tool_result', () => {
    ...
    try {
      writeTranscript(homeDir, '-Users-dev-project-b', 'session-b.jsonl', [
  ...
  if (test('does not flag Bash tool_use entries that have a matching tool_result', () => {
    ...
    try {
      writeTranscript(homeDir, '-Users-dev-project-c', 'session-c.jsonl', [
  ...
  if (test('does not flag ScheduleWakeup when later assistant progress exists', () => {
    ...
    try {
      writeTranscript(homeDir, '-Users-dev-project-d', 'session-d.jsonl', [
  ...
  if (test('supports inspecting one transcript path directly', () => {
    ...
    try {
      const transcriptPath = writeTranscript(homeDir, '-Users-dev-project-e', 'session-e.jsonl', [
  ...
  if (test('prints text output with state and recommended action', () => {
    ...
    try {
      writeTranscript(homeDir, '-Users-dev-project-f', 'session-f.jsonl', [
  ...
  if (test('text output distinguishes explicit transcript read failures from empty discovery', () => {
    ...
    assert.strictEqual(result.code, 0, result.stderr);
    assert.match(result.stdout, /No readable Codex transcript JSONL files were found/);
    assert.match(result.stdout, /Skipped transcript errors/);
    assert.ok(!result.stdout.includes('No Codex transcript JSONL files found under'));
  })) passed++; else failed++;

  if (test('continues when one transcript directory cannot be read', () => {
    const homeDir = createTempHome();
    const blockedDir = path.join(homeDir, '.codex', 'projects', '-blocked-project');
    const originalReaddirSync = fs.readdirSync;

    try {
      writeTranscript(homeDir, '-Users-dev-project-readable', 'session-readable.jsonl', [
  ...
  if (test('reports malformed JSONL lines as an attention signal', () => {
    ...
    try {
      const transcriptDir = path.join(homeDir, '.codex', 'projects', '-Users-dev-project-malformed');
      fs.mkdirSync(transcriptDir, { recursive: true });
      fs.writeFileSync(
  ...
  if (test('exit-code mode returns 2 when attention signals are present', () => {
    ...
    try {
      writeTranscript(homeDir, '-Users-dev-project-exit-code', 'session-exit-code.jsonl', [
  ...
  if (test('watch mode emits repeated JSON status frames', () => {
    ...
    try {
      writeTranscript(homeDir, '-Users-dev-project-watch', 'session-watch.jsonl', [
  ...
  if (test('watch mode honors exit-code after bounded refreshes', () => {
    ...
    try {
      writeTranscript(homeDir, '-Users-dev-project-watch-exit', 'session-watch-exit.jsonl', [
  ...
  if (test('writes per-session status snapshots and index when write-dir is set', () => {
    ...
    try {
      writeTranscript(homeDir, '-Users-dev-project-snapshot', 'session-snapshot.jsonl', [
  ...
  if (test('keeps index.json reserved when session id sanitizes to index', () => {
    ...
    try {
      writeTranscript(homeDir, '-Users-dev-project-index-collision', 'index.jsonl', [
  ...
  if (test('avoids Windows reserved basenames for session snapshots', () => {
    ...
    try {
      writeTranscript(homeDir, '-Users-dev-project-windows-name', 'con.jsonl', [
        assistantMessage('2026-04-30T09:55:00.000Z', 'con', 'Loop checkpoint.'),
      ]);
      writeTranscript(homeDir, '-Users-dev-project-windows-name', 'con-txt.jsonl', [
  ...
  if (test('write-dir failures do not suppress normal stdout', () => {
    ...
    try {
      ...
      writeTranscript(homeDir, '-Users-dev-project-write-error', 'session-write-error.jsonl', [
