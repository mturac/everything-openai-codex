...
function runTests() {
  ...
  } else if (test('exposes the corrected Codex target help text', () => {
    const result = run(powerShellCommand, ['--help']);
    assert.strictEqual(result.code, 0, result.stderr);
    assert.ok(
      result.stdout.includes('codex       (default) - Install ecc into ~/.codex/'),
      'help text should describe the Codex target as a full ~/.codex install surface'
    );
  })) passed++; else failed++;
