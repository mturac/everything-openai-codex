...
function runTests() {
  ...
  if (test('README marks one default path and warns against stacked installs', () => {
    ...
    assert.ok(
      readme.includes('**Recommended default:** install the OpenAI Codex plugin'),
  ...
  if (test('README documents low-context no-hooks install path', () => {
    ...
    assert.ok(
      readme.includes('./install.sh --profile minimal --target codex'),
    ...
    assert.ok(
      readme.includes('--profile core --without baseline:hooks --target codex'),
  ...
  if (test('README explains plugin-path cleanup and rules scoping', () => {
    assert.ok(
      readme.includes('remove the plugin from OpenAI Codex'),
    ...
    assert.ok(
      readme.includes('~/.codex/rules/ecc/'),
  ...
  if (test('rules README mirrors ecc namespaced install path', () => {
    assert.ok(
      rulesReadme.includes('mkdir -p ~/.codex/rules/ecc'),
      'rules README should create the ecc-owned user-level rules namespace'
    );
    assert.ok(
      rulesReadme.includes('cp -r rules/common ~/.codex/rules/ecc/'),
      'rules README should copy common rules under ~/.codex/rules/ecc/'
    );
    assert.ok(
      rulesReadme.includes('cp -r rules/typescript ~/.codex/rules/ecc/'),
      'rules README should copy language rules under ~/.codex/rules/ecc/'
    );
    assert.ok(
      rulesReadme.includes('mkdir -p .codex/rules/ecc'),
    ...
    assert.ok(
      !rulesReadme.includes('~/.codex/rules/typescript'),
