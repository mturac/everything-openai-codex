...
function runTests() {
  ...
  if (test('installs Codex rules and writes install-state', () => {
    ...
    try {
      ...
      assert.ok(fs.existsSync(path.join(codexRoot, 'rules', 'ecc', 'common', 'coding-style.md')));
      assert.ok(fs.existsSync(path.join(codexRoot, 'rules', 'ecc', 'typescript', 'testing.md')));
      assert.ok(fs.existsSync(path.join(codexRoot, 'commands', 'plan.md')));
      assert.ok(fs.existsSync(path.join(codexRoot, 'scripts', 'hooks', 'session-end.js')));
      assert.ok(fs.existsSync(path.join(codexRoot, 'scripts', 'lib', 'utils.js')));
      assert.ok(fs.existsSync(path.join(codexRoot, 'skills', 'ecc', 'tdd-workflow', 'SKILL.md')));
      assert.ok(fs.existsSync(path.join(codexRoot, 'skills', 'ecc', 'coding-standards', 'SKILL.md')));
      assert.ok(fs.existsSync(path.join(codexRoot, 'plugin.json')));

      const statePath = path.join(homeDir, '.codex', 'ecc', 'install-state.json');
      ...
      assert.ok(
        state.operations.some(operation => (
          operation.destinationPath === path.join(codexRoot, 'rules', 'ecc', 'common', 'coding-style.md')
        )),
  ...
  if (test('installs manifest profiles and writes non-legacy install-state', () => {
    ...
    try {
      ...
      assert.ok(fs.existsSync(path.join(codexRoot, 'commands', 'plan.md')));
      assert.ok(fs.existsSync(path.join(codexRoot, 'hooks', 'hooks.json')));
      assert.ok(fs.existsSync(path.join(codexRoot, 'scripts', 'hooks', 'session-end.js')));
      assert.ok(fs.existsSync(path.join(codexRoot, 'scripts', 'lib', 'session-manager.js')));
      assert.ok(fs.existsSync(path.join(codexRoot, 'plugin.json')));

      const state = readJson(path.join(codexRoot, 'ecc', 'install-state.json'));
      ...
      assert.ok(
        state.operations.some(operation => (
          operation.destinationPath === path.join(codexRoot, 'commands', 'plan.md')
        )),
  ...
  if (test('preserves existing top-level Codex rules and skills during managed install', () => {
    const homeDir = createTempDir('install-apply-home-');
    const projectDir = createTempDir('install-apply-project-');

    try {
      const codexRoot = path.join(homeDir, '.codex');
      const userRulePath = path.join(codexRoot, 'rules', 'common', 'coding-style.md');
      const userSkillPath = path.join(codexRoot, 'skills', 'tdd-workflow', 'SKILL.md');
  ...
  if (test('installs codex hooks without generating settings.json', () => {
    const homeDir = createTempDir('install-apply-home-');
    const projectDir = createTempDir('install-apply-project-');
    ...
    try {
      ...
      const codexRoot = path.join(homeDir, '.codex');
      assert.ok(fs.existsSync(path.join(codexRoot, 'hooks', 'hooks.json')), 'hooks.json should be copied');
      assert.ok(!fs.existsSync(path.join(codexRoot, 'settings.json')), 'settings.json should not be created just to install managed hooks');
    } finally {
      cleanup(homeDir);
      cleanup(projectDir);
    }
  ...
  if (test('installs codex hooks with the safe plugin bootstrap contract', () => {
    const homeDir = createTempDir('install-apply-home-');
    const projectDir = createTempDir('install-apply-project-');
    ...
    try {
      ...
      const codexRoot = path.join(homeDir, '.codex');
      const installedHooks = readJson(path.join(codexRoot, 'hooks', 'hooks.json'));
      ...
      assert.strictEqual(typeof installedBashDispatcherEntry.hooks[0].command, 'string', 'hooks/hooks.json should install string-form commands for OpenAI Codex schema compatibility');
      assert.ok(
      ...
      assert.ok(
        installedBashDispatcherEntry.hooks[0].command.includes('CODEX_PLUGIN_ROOT'),
        'hooks/hooks.json should still consult CODEX_PLUGIN_ROOT for runtime resolution'
      );
      assert.ok(
      ...
      assert.ok(
        !installedBashDispatcherEntry.hooks[0].command.includes('${CODEX_PLUGIN_ROOT}'),
        'hooks/hooks.json should not retain raw CODEX_PLUGIN_ROOT shell placeholders after install'
      );
    } finally {
      cleanup(homeDir);
      cleanup(projectDir);
  ...
  if (test('preserves existing settings.json without mutating it during codex install', () => {
    const homeDir = createTempDir('install-apply-home-');
    const projectDir = createTempDir('install-apply-project-');
    ...
    try {
      ...
      fs.writeFileSync(
        path.join(codexRoot, 'settings.json'),
        JSON.stringify({
          effortLevel: 'high',
          env: { MY_VAR: '1' },
          hooks: {
      ...
      const settings = readJson(path.join(codexRoot, 'settings.json'));
      ...
      assert.deepStrictEqual(
        ...
        'managed Codex hooks should not be injected into settings.json'
      );
    } finally {
      cleanup(homeDir);
      cleanup(projectDir);
  ...
  if (test('reinstall does not create settings.json when only managed hooks are installed', () => {
    ...
    try {
      ...
      assert.ok(!fs.existsSync(path.join(homeDir, '.codex', 'settings.json')));
    } finally {
      cleanup(homeDir);
      cleanup(projectDir);
    }
  ...
  if (test('reinstall leaves pre-existing hook-based settings.json untouched', () => {
    ...
    try {
      const codexRoot = path.join(homeDir, '.codex');
      fs.mkdirSync(codexRoot, { recursive: true });
      const settingsPath = path.join(codexRoot, 'settings.json');
  ...
  if (test('ignores malformed existing settings.json during codex install', () => {
    const homeDir = createTempDir('install-apply-home-');
    const projectDir = createTempDir('install-apply-project-');
    ...
    try {
      ...
      assert.ok(fs.existsSync(path.join(codexRoot, 'hooks', 'hooks.json')), 'hooks.json should still be copied');
      assert.ok(fs.existsSync(path.join(codexRoot, 'ecc', 'install-state.json')), 'install state should still be written');
    } finally {
      cleanup(homeDir);
      cleanup(projectDir);
    }
  ...
  if (test('ignores non-object existing settings.json during codex install', () => {
    const homeDir = createTempDir('install-apply-home-');
    const projectDir = createTempDir('install-apply-project-');
  ...
  if (test('fails when source hooks.json root is not an object before copying files', () => {
    ...
    const targetRoot = path.join(tempDir, '.codex');
  ...
  if (test('installs from ecc-install.json and persists component selections', () => {
    ...
    try {
      ...
      assert.ok(fs.existsSync(path.join(homeDir, '.codex', 'skills', 'ecc', 'security-review', 'SKILL.md')));
      assert.ok(!fs.existsSync(path.join(homeDir, '.codex', 'skills', 'ecc', 'dmux-workflows', 'SKILL.md')));
