...
function runTests() {
  ...
  if (test('detects repo mode from structural markers when package name differs', () => {
    ...
    try {
      ...
      fs.writeFileSync(path.join(projectRoot, '.codex-plugin', 'plugin.json'), JSON.stringify({ name: 'ecc' }, null, 2));
      fs.writeFileSync(
  ...
  if (test('audits consumer projects from cwd instead of the ecc repo root', () => {
    ...
    try {
      fs.mkdirSync(path.join(homeDir, '.codex', 'plugins', 'ecc', '.codex-plugin'), { recursive: true });
      fs.writeFileSync(
        path.join(homeDir, '.codex', 'plugins', 'ecc', '.codex-plugin', 'plugin.json'),
      ...
      fs.writeFileSync(path.join(projectRoot, '.codex', 'settings.json'), JSON.stringify({ hooks: ['PreToolUse'] }, null, 2));
      fs.writeFileSync(
        path.join(projectRoot, 'package.json'),
  ...
  if (test('prints no top actions when consumer checks all pass', () => {
    ...
    try {
      ...
      fs.writeFileSync(path.join(projectRoot, '.codex', 'hooks.json'), JSON.stringify({ hooks: [] }, null, 2));
      fs.writeFileSync(path.join(projectRoot, '.codex', 'settings.local.json'), JSON.stringify({ local: true }, null, 2));
      fs.writeFileSync(path.join(projectRoot, 'CODEX.md'), '# Consumer instructions\n');
  ...
  if (test('detects marketplace-installed Codex plugins under home marketplaces/', () => {
  ...
  if (test('detects marketplace-installed Codex plugins under project marketplaces/', () => {
    const homeDir = createTempDir('harness-audit-marketplace-home-');
    const projectRoot = createTempDir('harness-audit-marketplace-project-');
  ...
  if (test('detects marketplace-installed Codex plugins from USERPROFILE fallback on Windows-style setups', () => {
    const homeDir = createTempDir('harness-audit-marketplace-home-');
    const projectRoot = createTempDir('harness-audit-marketplace-project-');
  ...
  if (test('detects Codex plugin installs from installed_plugins.json', () => {
