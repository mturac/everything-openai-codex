...
function runTests() {
  ...
  if (test('recommends security components and profile for a natural language query', () => {
    ...
    assert.strictEqual(
      ...
      'npx ecc install --profile minimal --target codex --with capability:security'
    );
  ...
  if (test('prints text recommendations with install and plan commands', () => {
    ...
    assert.match(result.stdout, /npx ecc install --profile minimal --target codex --with capability:security/);
    assert.match(result.stdout, /npx ecc plan --profile minimal --target codex --with capability:security/);
  })) passed++; else failed++;
