...
function runTests() {
  ...
  if (test('buildInstallApplyArgs reconstructs legacy installs', () => {
    const record = {
      adapter: { target: 'codex', kind: 'home' },
      state: {
        target: { target: 'codex' },
        request: {
          profile: null,
    ...
    try {
      ...
      const records = [
        makeRecord({
          ...
          resolution: { selectedModules: ['legacy-codex-rules'], skippedModules: [] },
          operations: [
            {
              kind: 'copy-file',
              ...
              destinationPath: path.join(homeDir, '.codex', 'rules', 'common', 'coding-style.md'),
