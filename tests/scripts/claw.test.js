...
function runTests() {
  ...
  if (test('askCodex() handles subprocess error gracefully', () => {
    // Use a non-existent command to trigger an error
    const result = askCodex('sys', 'hist', 'msg');
    // Should return an error string, not throw
    assert.strictEqual(typeof result, 'string', 'Should return a string');
    // If codex is not installed, we get an error message
    // If codex IS installed, we get an actual response — both are valid
  ...
  if (test('module exports all required functions', () => {
    ...
    const required = [
      ...
      'appendTurn', 'loadeccContext', 'askCodex', 'main'
    ];
  ...
  if (test('searchSessions finds query in saved session', () => {
    const tmpDir = makeTmpDir();
    try {
      const clawDir = path.join(tmpDir, '.codex', 'claw');
