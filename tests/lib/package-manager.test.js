...
function runTests() {
  ...
  if (test('falls through on corrupted project config JSON', () => {
    ...
    fs.writeFileSync(path.join(codexDir, 'package-manager.json'), '{not valid json!!!');
    const originalEnv = process.env.CODEX_PACKAGE_MANAGER;
    try {
      delete process.env.CODEX_PACKAGE_MANAGER;
      const result = pm.getPackageManager({ projectDir: testDir });
  ...
  if (test('falls through on project config with unknown PM', () => {
    ...
    const codexDir = path.join(testDir, '.codex');
    fs.mkdirSync(codexDir, { recursive: true });
    fs.writeFileSync(path.join(codexDir, 'package-manager.json'), JSON.stringify({ packageManager: 'nonexistent-pm' }));
    const originalEnv = process.env.CODEX_PACKAGE_MANAGER;
    try {
      delete process.env.CODEX_PACKAGE_MANAGER;
      const result = pm.getPackageManager({ projectDir: testDir });
  ...
  if (test('detects from valid project-config (.codex/package-manager.json)', () => {
    ...
    const codexDir = path.join(testDir, '.codex');
    fs.mkdirSync(codexDir, { recursive: true });
    fs.writeFileSync(path.join(codexDir, 'package-manager.json'), JSON.stringify({ packageManager: 'pnpm' }));
    const originalEnv = process.env.CODEX_PACKAGE_MANAGER;
    try {
      delete process.env.CODEX_PACKAGE_MANAGER;
      const result = pm.getPackageManager({ projectDir: testDir });
  ...
  if (test('project-config takes priority over package.json', () => {
    ...
    fs.writeFileSync(path.join(codexDir, 'package-manager.json'), JSON.stringify({ packageManager: 'bun' }));
  ...
  if (test('setProjectPackageManager creates .codex directory if missing', () => {
    ...
    try {
      const codexDir = path.join(testDir, '.codex');
      assert.ok(!fs.existsSync(codexDir), '.codex should not pre-exist');
      pm.setProjectPackageManager('npm', testDir);
      assert.ok(fs.existsSync(codexDir), '.codex should be created');
      const configPath = path.join(codexDir, 'package-manager.json');
  ...
  if (test('getPackageManager falls through corrupted global config to npm default', () => {
    ...
    try {
      ...
      fs.mkdirSync(codexDir, { recursive: true });
      fs.writeFileSync(path.join(codexDir, 'package-manager.json'), '{ invalid json !!!', 'utf8');
  ...
  if (test('getPackageManager returns source global-config when valid global config exists', () => {
    ...
    try {
      ...
      const codexDir = path.join(tmpDir, '.codex');
      fs.mkdirSync(codexDir, { recursive: true });
      fs.writeFileSync(path.join(codexDir, 'package-manager.json'), JSON.stringify({ packageManager: 'pnpm', setAt: '2026-01-01T00:00:00Z' }), 'utf8');
  ...
  if (test('setPreferredPackageManager throws wrapped error when save fails', () => {
    ...
    const codexDir = path.join(isoHome, '.codex');
    fs.mkdirSync(codexDir, { recursive: true });
  ...
  if (test('setProjectPackageManager throws wrapped error when write fails', () => {
    ...
    const codexDir = path.join(isoProject, '.codex');
    fs.mkdirSync(codexDir, { recursive: true });
