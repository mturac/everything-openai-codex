...
function runTests() {
  ...
  if (test('validates install component filters', () => {
    const codexComponents = listInstallComponents({ family: 'capability', target: 'codex' });
    assert.ok(codexComponents.length > 0, 'Should list Codex capability components');
    assert.ok(codexComponents.every(component => component.family === 'capability'));
    assert.ok(codexComponents.every(component => component.targets.includes('codex')));
  ...
  if (test('exposes continuous-learning-v2 as a single-skill install surface', () => {
    ...
    assert.ok(component.targets.includes('codex'), 'Should support Codex installs');
  ...
  if (test('validates projectRoot and homeDir option types before adapter planning', () => {
    ...
    assert.throws(
      () => resolveInstallPlan({ profileId: 'core', target: 'codex', homeDir: {} }),
  ...
  if (test('skips a requested module when its dependency chain does not support the target', () => {
    ...
    try {
      ...
      const plan = resolveInstallPlan({ repoRoot, profileId: 'core', target: 'codex' });
  ...
  if (test('fails fast when install manifest module targets is not an array', () => {
    ...
    try {
      ...
      assert.throws(
        () => resolveInstallPlan({ repoRoot, profileId: 'core', target: 'codex' }),
