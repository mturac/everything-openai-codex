...
function runTests() {
  ...
  if (test('resolves codex adapter root and install-state path from home dir', () => {
    ...
    assert.strictEqual(root, path.join(homeDir, '.codex'));
    assert.strictEqual(statePath, path.join(homeDir, '.codex', 'ecc', 'install-state.json'));
  })) passed++; else failed++;

  if (test('plans codex rules and skills under ecc-managed subdirectories', () => {
    ...
    assert.ok(
      plan.operations.some(operation => (
        ...
        && operation.destinationPath === path.join(homeDir, '.codex', 'rules', 'ecc')
      )),
      'Should install bundled Codex rules under rules/ecc'
    );
    assert.ok(
      plan.operations.some(operation => (
        normalizedRelativePath(operation.sourceRelativePath) === 'skills/tdd-workflow'
        && operation.destinationPath === path.join(homeDir, '.codex', 'skills', 'ecc', 'tdd-workflow')
      )),
      'Should install bundled Codex skills under skills/ecc'
    );
  })) passed++; else failed++;
  ...
  if (test('exposes validate and planOperations on adapters', () => {
    const codexAdapter = getInstallTargetAdapter('codex');
    const cursorAdapter = getInstallTargetAdapter('cursor');

    assert.strictEqual(typeof codexAdapter.planOperations, 'function');
    assert.strictEqual(typeof codexAdapter.validate, 'function');
    assert.deepStrictEqual(
      codexAdapter.validate({ homeDir: '/Users/example', repoRoot: '/repo/ecc' }),
      []
    );
