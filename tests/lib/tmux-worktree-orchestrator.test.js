...
test('buildOrchestrationPlan normalizes global and worker seed paths', () => {
  const plan = buildOrchestrationPlan({
    ...
    seedPaths: ['scripts/orchestrate-worktrees.js', './.codex/plan/workflow-e2e-test.json'],
    workers: [
      {
        name: 'Docs',
        task: 'Update docs',
...
test('materializePlan keeps worker instructions inside the worktree boundary', () => {
  ...
  try {
    const plan = buildOrchestrationPlan({
      repoRoot: tempRoot,
      coordinationRoot: path.join(tempRoot, '.codex', 'orchestration'),
...
test('overlaySeedPaths copies local overlays into the worker worktree', () => {
  ...
  try {
    ...
    fs.mkdirSync(path.join(repoRoot, '.codex', 'plan'), { recursive: true });
    ...
    fs.writeFileSync(
      path.join(repoRoot, '.codex', 'plan', 'workflow-e2e-test.json'),
      '{"seeded":true}\n',
      'utf8'
    );
    fs.writeFileSync(
    ...
    assert.strictEqual(
      fs.readFileSync(path.join(worktreePath, '.codex', 'plan', 'workflow-e2e-test.json'), 'utf8'),
      '{"seeded":true}\n'
    );
  } finally {
    fs.rmSync(tempRoot, { recursive: true, force: true });
