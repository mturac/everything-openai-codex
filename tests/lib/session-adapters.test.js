...
const {
  ...
  normalizeCodexHistorySession,
  normalizeDmuxSnapshot,
  persistCanonicalSnapshot,
  validateCanonicalSnapshot
} = require('../../scripts/lib/session-adapters/canonical-session');
const { createCodexHistoryAdapter } = require('../../scripts/lib/session-adapters/codex-history');
  ...
  try {
    ...
    const adapter = createDmuxTmuxAdapter({
      ...
      collectSessionSnapshotImpl: () => ({
        ...
        coordinationDir: '/tmp/.codex/orchestration/workflow-visual-proof',
        repoRoot: '/tmp/repo',
        targetType: 'plan',
        sessionActive: true,
        paneCount: 1,
        ...
        workers: [{
          ...
          workerDir: '/tmp/.codex/orchestration/workflow-visual-proof/seed-check',
          status: {
            state: 'running',
    ...
    const adapter = createDmuxTmuxAdapter({
      ...
      collectSessionSnapshotImpl: () => ({
        ...
        workers: [{
          ...
          workerSlug: 'proof',
          workerDir: '/tmp/.codex/orchestration/workflow-visual-proof/proof',
          status: {
            state: 'completed',
...
test('codex-history adapter loads the latest recorded session', () => {
  ...
  try {
    withHome(homeDir, () => {
      ...
      const snapshot = adapter.open('codex:latest').getSnapshot();
      const recordingPath = getFallbackSessionRecordingPath(snapshot, { recordingDir });
      const persisted = JSON.parse(fs.readFileSync(recordingPath, 'utf8'));

      assert.strictEqual(snapshot.schemaVersion, 'ecc.session.v1');
      assert.strictEqual(snapshot.adapterId, 'codex-history');
      ...
      assert.strictEqual(persisted.latest.adapterId, 'codex-history');
      assert.strictEqual(persisted.history.length, 1);
    });
  } finally {
...
test('adapter registry routes plan files to dmux and explicit codex targets to history', () => {
  ...
  fs.writeFileSync(planPath, JSON.stringify({
    sessionName: 'workflow-visual-proof',
    repoRoot,
    coordinationRoot: path.join(repoRoot, '.codex', 'orchestration')
  }));
      ...
      const registry = createAdapterRegistry({
        adapters: [
          createDmuxTmuxAdapter({
            ...
            collectSessionSnapshotImpl: () => ({
              sessionName: 'workflow-visual-proof',
              coordinationDir: path.join(repoRoot, '.codex', 'orchestration', 'workflow-visual-proof'),
              repoRoot,
              targetType: 'plan',
              sessionActive: false,
              paneCount: 0,
          ...
          createCodexHistoryAdapter({ loadStateStoreImpl: () => null })
        ]
      });

      const dmuxSnapshot = registry.open(planPath, { cwd: repoRoot }).getSnapshot();
      const codexSnapshot = registry.open('codex:latest', { cwd: repoRoot }).getSnapshot();

      assert.strictEqual(dmuxSnapshot.adapterId, 'dmux-tmux');
      assert.strictEqual(codexSnapshot.adapterId, 'codex-history');
    });
  } finally {
...
test('adapter registry resolves structured target types into the correct adapter', () => {
  ...
  try {
    withHome(homeDir, () => {
      const registry = createAdapterRegistry({
        adapters: [
          createDmuxTmuxAdapter({
            ...
            collectSessionSnapshotImpl: () => ({
              sessionName: 'workflow-typed-proof',
              coordinationDir: path.join(repoRoot, '.codex', 'orchestration', 'workflow-typed-proof'),
              repoRoot,
              targetType: 'plan',
              sessionActive: true,
              paneCount: 0,
      ...
      const codexSnapshot = registry.open({ type: 'codex-history', value: 'latest' }, { cwd: repoRoot }).getSnapshot();

      assert.strictEqual(dmuxSnapshot.adapterId, 'dmux-tmux');
      assert.strictEqual(dmuxSnapshot.session.sourceTarget.type, 'plan');
      assert.strictEqual(codexSnapshot.adapterId, 'codex-history');
      assert.strictEqual(codexSnapshot.session.sourceTarget.type, 'codex-history');
      assert.strictEqual(codexSnapshot.workers[0].branch, 'feat/typed-targets');
    });
  } finally {
    fs.rmSync(repoRoot, { recursive: true, force: true });
    fs.rmSync(homeDir, { recursive: true, force: true });
...
test('default registry forwards a nested state-store writer to adapters', () => {
  ...
  try {
    withHome(homeDir, () => {
      const snapshot = inspectSessionTarget('codex:latest', {
        cwd: process.cwd(),
        stateStore
      });
      ...
      assert.strictEqual(stateStore.sessions.persisted[0].snapshot.adapterId, 'codex-history');
...
test('adapter registry lists adapter metadata and target types', () => {
  ...
  assert.ok(ids.includes('codex-history'));
  assert.ok(ids.includes('dmux-tmux'));
  assert.ok(
    adapters.some(adapter => adapter.id === 'codex-history' && adapter.targetTypes.includes('codex-history')),
    'codex-history should advertise its canonical target type'
  );
  assert.ok(
...
test('codex history normalization falls back to filename ids and empty metadata defaults', () => {
  const snapshot = normalizeCodexHistorySession({
    shortId: 'no-id',
  ...
  const pathOnly = normalizeCodexHistorySession({
