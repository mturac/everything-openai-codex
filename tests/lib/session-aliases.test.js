...
function runTests() {
  ...
  if (test('updateAliasTitle returns failure when saveAliases fails (read-only dir)', () => {
    ...
    const isoCodexDir = path.join(isoHome, '.codex');
    fs.mkdirSync(isoCodexDir, { recursive: true });
    ...
    try {
      ...
      fs.chmodSync(isoCodexDir, 0o555);
      ...
      try { fs.chmodSync(isoCodexDir, 0o755); } catch { /* best-effort */ }
  ...
  if (test('saveAliases triggers inner restoreErr catch when both save and restore fail', () => {
    ...
    const backupPath = path.join(codexDir, 'session-aliases.json.bak');
    ...
    try {
      ...
      delete require.cache[require.resolve('../../scripts/lib/session-aliases')];
      delete require.cache[require.resolve('../../scripts/lib/utils')];
      try { fs.chmodSync(codexDir, 0o755); } catch { /* best-effort */ }
      fs.rmSync(isoHome, { recursive: true, force: true });
    }
  })) passed++; else failed++;
