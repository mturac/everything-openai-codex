...
function writeManifestSourceFixture(root) {
  ...
  writeJson(root, path.join('.codex-plugin', 'plugin.json'), { name: 'fixture' });
}
...
function runTests() {
  ...
  if (test('plans Codex legacy rules with warnings and state preview', () => {
    const sourceRoot = createTempDir('install-executor-source-');
    const homeDir = createTempDir('install-executor-home-');
    const projectRoot = createTempDir('install-executor-project-');
    const codexRulesDir = path.join(homeDir, 'custom-rules');
    try {
      ...
      assert.strictEqual(plan.target, 'codex');
      assert.strictEqual(plan.installRoot, codexRulesDir);
  ...
  if (test('plans Codex legacy rules under the default ecc-managed rules directory', () => {
    const sourceRoot = createTempDir('install-executor-source-');
    const homeDir = createTempDir('install-executor-home-');
    const projectRoot = createTempDir('install-executor-project-');
    try {
      writeLegacySourceFixture(sourceRoot);
      writeFile(homeDir, path.join('.codex', 'rules', 'common', 'coding-style.md'), '# User custom rule\n');
      ...
      const managedRulesDir = path.join(homeDir, '.codex', 'rules', 'ecc');
      assert.strictEqual(plan.installRoot, managedRulesDir);
      assert.ok(operationFor(plan, path.join('.codex', 'rules', 'ecc', 'common', 'coding-style.md')));
      assert.ok(operationFor(plan, path.join('.codex', 'rules', 'ecc', 'typescript', 'testing.md')));
      assert.ok(!operationFor(plan, path.join('.codex', 'rules', 'common', 'coding-style.md')));
      assert.ok(!plan.warnings.some(warning => warning.includes('files may be overwritten')));
    } finally {
      cleanup(sourceRoot);
      cleanup(homeDir);
  ...
  if (test('applyInstallPlan re-export applies a manifest plan and writes install state', () => {
    ...
    try {
      ...
      assert.ok(fs.existsSync(path.join(homeDir, '.codex', 'rules', 'ecc', 'common', 'coding-style.md')));
      assert.ok(fs.existsSync(path.join(homeDir, '.codex', 'skills', 'ecc', 'demo', 'SKILL.md')));
      assert.ok(fs.existsSync(path.join(homeDir, '.codex', 'src', 'app.js')));
      assert.ok(fs.existsSync(path.join(homeDir, '.codex', 'standalone.txt')));
      assert.ok(fs.existsSync(path.join(homeDir, '.codex', 'plugin.json')));
      const state = JSON.parse(fs.readFileSync(path.join(homeDir, '.codex', 'ecc', 'install-state.json'), 'utf8'));
