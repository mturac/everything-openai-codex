...
function runTests() {
  ...
  const learnedRoot = path.join(homeDir, '.codex', 'skills', 'learned');
  const importedRoot = path.join(homeDir, '.codex', 'skills', 'imported');
  const runsFile = path.join(homeDir, '.codex', 'state', 'skill-runs.jsonl');
  ...
  try {
    ...
    if (test('ignores malformed JSONL rows when reading execution records', () => {
      const malformedRunsFile = path.join(homeDir, '.codex', 'state', 'malformed-skill-runs.jsonl');
      fs.writeFileSync(
        malformedRunsFile,
        `${JSON.stringify({
          skill_id: 'alpha',
    ...
    if (test('resolves default run paths and treats missing JSONL storage as empty', () => {
      ...
      try {
        ...
        assert.strictEqual(
          defaultPath,
          path.join(pathHome, '.codex', 'state', 'skill-runs.jsonl')
        );
        assert.deepStrictEqual(
    ...
    if (test('treats an unsnapshotted SKILL.md as v1 and orders last_run by actual time', () => {
      ...
      const offsetRunsFile = path.join(homeDir, '.codex', 'state', 'offset-skill-runs.jsonl');
