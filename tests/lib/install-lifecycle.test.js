...
function runTests() {
  ...
  if (test('normalizes default targets and dedupes adapter aliases', () => {
    ...
    assert.deepStrictEqual(
      normalizeTargets(['cursor-project', 'cursor', 'codex-home', 'codex']),
      ['cursor', 'codex']
    );
  })) passed++; else failed++;
  ...
  if (test('discovers installed states for multiple targets in the current context', () => {
    ...
    try {
      const codexStatePath = path.join(homeDir, '.codex', 'ecc', 'install-state.json');
      ...
      writeState(codexStatePath, {
        ...
        installStatePath: codexStatePath,
        request: {
          profile: null,
          modules: [],
          legacyLanguages: ['typescript'],
        ...
        resolution: {
          selectedModules: ['legacy-codex-rules'],
          skippedModules: [],
        },
        operations: [],
        source: {
      ...
      assert.strictEqual(records[0].state.target.id, 'codex-home');
        ...
        operations: [
          {
            ...
            moduleId: 'platform-configs',
            sourceRelativePath: '.codex-plugin/plugin.json.template',
            destinationPath,
        ...
        operations: [
          {
            kind: 'render-template',
            moduleId: 'platform-configs',
            sourceRelativePath: '.codex/plugin.json.template',
            destinationPath,
            strategy: 'render-template',
            ownership: 'managed',
            scaffoldOnly: false,
