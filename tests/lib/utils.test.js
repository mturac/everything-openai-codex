...
function runTests() {
  ...
  if (test('getSessionsDir returns path under Codex dir', () => {
    ...
    assert.ok(sessionsDir.endsWith(path.join('.codex', 'session-data')) || sessionsDir.endsWith('/.codex/session-data'), 'Should use canonical session-data directory');
  })) passed++; else failed++;
  ...
  if (test('sanitizeSessionId strips leading dots', () => {
    assert.strictEqual(utils.sanitizeSessionId('.codex'), 'codex');
  })) passed++; else failed++;
  ...
  if (test('sanitizeSessionId is idempotent', () => {
    for (const input of ['.codex', 'my.project', 'project@v2', 'a...b', 'my-project_123']) {
  ...
  if (test('getSessionIdShort falls back to sanitized project name', () => {
    ...
    try {
      ...
      if (original !== undefined) process.env.CODEX_SESSION_ID = original;
  ...
  if (test('getSessionIdShort sanitizes whitespace-only CODEX_SESSION_ID to fallback', () => {
    if (process.platform === 'win32') {
      console.log('    (skipped — root CWD differs on Windows)');
      return true;
    }
