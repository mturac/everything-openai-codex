...
function runTests() {
  ...
  try {
    ...
    if (test('dashboard renders gracefully with no execution records', () => {
      const emptyRunsFile = path.join(homeDir, '.codex', 'state', 'empty-runs.jsonl');
      ...
      const result = dashboard.renderDashboard({
        ...
        learnedRoot: path.join(homeDir, '.codex', 'skills', 'empty-learned'),
        importedRoot: path.join(homeDir, '.codex', 'skills', 'empty-imported'),
        homeDir,
