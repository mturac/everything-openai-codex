 ...
 *   1. CODEX_PLUGIN_ROOT env var
 *   2. Standard install (~/.codex/)
 *   3. Exact legacy plugin roots under ~/.codex/plugins/
...
function setupStandardInstall(homeDir) {
  ...
  const scriptDir = path.join(codexDir, 'scripts', 'lib');
...
function setupLegacyPluginInstall(homeDir, segments) {
  const legacyDir = path.join(homeDir, '.codex', 'plugins', ...segments);
...
function runTests() {
  ...
  if (test('skips whitespace-only CODEX_PLUGIN_ROOT', () => {
    const homeDir = createTempDir();
    try {
      setupStandardInstall(homeDir);
      const result = resolveEccRoot({ envRoot: '   ', homeDir });
  ...
  if (test('finds standard install at ~/.codex/', () => {
    const homeDir = createTempDir();
    try {
      setupStandardInstall(homeDir);
      const result = resolveEccRoot({ envRoot: '', homeDir });
  ...
  if (test('falls back to ~/.codex/ when nothing is found', () => {
    const homeDir = createTempDir();
    try {
      // Create ~/.codex but don't put scripts there
  ...
  if (test('falls back gracefully when ~/.codex/ does not exist', () => {
    const homeDir = createTempDir();
    try {
      const result = resolveEccRoot({ envRoot: '', homeDir });
      assert.strictEqual(result, path.join(homeDir, '.codex'));
  ...
  if (test('supports custom probe path', () => {
    ...
    try {
      const codexDir = path.join(homeDir, '.codex');
      fs.mkdirSync(path.join(codexDir, 'custom'), { recursive: true });
      fs.writeFileSync(path.join(codexDir, 'custom', 'marker.js'), '// probe');
  ...
  if (test('INLINE_RESOLVE returns CODEX_PLUGIN_ROOT when set', () => {
    const { execFileSync } = require('child_process');
    const result = execFileSync('node', [
      '-e', `console.log(${INLINE_RESOLVE})`,
    ], {
      env: { ...process.env, CODEX_PLUGIN_ROOT: '/inline/test/root' },
      encoding: 'utf8',
    }).trim();
  ...
  if (test('INLINE_RESOLVE falls back to ~/.codex/ when nothing found', () => {
    const homeDir = createTempDir();
    try {
      const { execFileSync } = require('child_process');
      const result = execFileSync('node', [
