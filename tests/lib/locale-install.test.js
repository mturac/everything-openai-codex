...
function runTests() {
    ...
    try {
      ...
      assert.ok(
        plan.operations.some(operation => (
          ...
          && normalizePlanPath(operation.destinationPath).endsWith('/.codex/docs/ja-JP')
        )),
        'Should map docs/ja-JP to ~/.codex/docs/ja-JP'
      );
    } finally {
    ...
    try {
      ...
      assert.ok(
        json.plan.operations.some(operation => (
          ...
          && normalizePlanPath(operation.destinationPath).endsWith('/.codex/docs/ja-JP/README.md')
        )),
        'Should copy translated README into ~/.codex/docs/ja-JP'
      );
    } finally {
      fs.rmSync(homeDir, { recursive: true, force: true });
  ...
  if (test('end-to-end: --locale ja installs translated docs side-by-side', () => {
    ...
    try {
      ...
      assert.ok(
        fs.existsSync(path.join(codexRoot, 'docs', 'ja-JP', 'README.md')),
        'Should install Japanese README under docs/ja-JP'
      );
      assert.ok(
        !fs.existsSync(path.join(codexRoot, 'skills', 'ecc', 'configure-ecc', 'SKILL.md')),
