...
\`\`\``;
    ...
    assert.strictEqual(meta.project, 'everything-OpenAI Codex');
...
`;
  ...
  const tmpCanonicalSessionsDir = path.join(tmpHome, '.codex', 'session-data');
  const tmpLegacySessionsDir = path.join(tmpHome, '.codex', 'sessions');
