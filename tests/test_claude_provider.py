...
def make_provider(response: SimpleNamespace) -> CodexProvider:
...
def test_generate_collects_text_and_tool_use_blocks() -> None:
    provider = make_provider(
        make_response(
            [
                ...
                SimpleNamespace(type="tool_use", id="toolu_1", name="search", input={"query": "codex"}),
    ...
    assert output.tool_calls[0].arguments == {"query": "codex"}
...
def test_generate_copies_tool_use_dict_arguments() -> None:
    raw_arguments: dict[str, Any] = {"query": "codex"}
    provider = make_provider(
        make_response(
    ...
    assert (output.tool_calls or [])[0].arguments == {"query": "codex"}
