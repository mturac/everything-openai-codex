...
async function main() {
  ...
  const tests = [
    ...
    [
      "session.created checks CODEX.md through fs instead of shell test",
      async () => withTempProject(["CODEX.md"], async (projectDir) => {
        const client = createClient()
        const $ = createFailingShell()
        const hooks = await eccHooksPlugin({ client, $, directory: projectDir })
        ...
        assert.deepStrictEqual($.calls, [], `Unexpected shell probes: ${$.calls.join(", ")}`)
        assert.ok(
          client.logs.some((entry) => entry.message === "[ecc] Found CODEX.md - loading project context"),
          "Expected CODEX.md detection log"
        )
      }),
    ],
    [
      "session.created ignores directories named CODEX.md",
      async () => {
        const projectDir = fs.mkdtempSync(path.join(os.tmpdir(), "ecc-opencode-plugin-"))
        try {
          fs.mkdirSync(path.join(projectDir, "CODEX.md"))
          ...
          assert.ok(
            !client.logs.some((entry) => entry.message === "[ecc] Found CODEX.md - loading project context"),
            "Directory named CODEX.md should not be treated as project context"
          )
        } finally {
          fs.rmSync(projectDir, { recursive: true, force: true })
        }
