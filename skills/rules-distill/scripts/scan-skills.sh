...
#
# When CWD_SKILLS_DIR is omitted, defaults to $PWD/.codex/skills so the
# script always picks up project-level skills without relying on the caller.
#
# Environment:
#   RULES_DISTILL_GLOBAL_DIR   Override ~/.codex/skills (for testing only;
#                              do not set in production — intended for bats tests)
#   RULES_DISTILL_PROJECT_DIR  Override project dir detection (for testing only)

set -euo pipefail

GLOBAL_DIR="${RULES_DISTILL_GLOBAL_DIR:-$HOME/.codex/skills}"
CWD_SKILLS_DIR="${RULES_DISTILL_PROJECT_DIR:-${1:-$PWD/.codex/skills}}"
# Validate CWD_SKILLS_DIR looks like a .codex/skills path (defense-in-depth).
# Only warn when the path exists — a nonexistent path poses no traversal risk.
if [[ -n "$CWD_SKILLS_DIR" && -d "$CWD_SKILLS_DIR" && "$CWD_SKILLS_DIR" != */.codex/skills* ]]; then
  echo "Warning: CWD_SKILLS_DIR does not look like a .codex/skills path: $CWD_SKILLS_DIR" >&2
fi
