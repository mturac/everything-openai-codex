---
name: configure-ecc
description: Interactive installer for Everything OpenAI Codex — guides users through selecting and installing skills and rules to user-level or project-level directories, verifies paths, and optionally optimizes installed files.
origin: ecc
---

# Configure Everything OpenAI Codex (ecc)

An interactive, step-by-step installation wizard for the Everything OpenAI Codex project. Uses `AskUserQuestion` to guide users through selective installation of skills and rules, then verifies correctness and offers optimization.

## When to Activate

- User says "configure ecc", "install ecc", "setup everything.openai.codex", or similar
...
This skill must be accessible to OpenAI Codex before activation. Two ways to bootstrap:
1. **Via Plugin**: `/plugin install eoc@eoc` — the plugin loads this skill automatically
...
Set `ecc_ROOT=/tmp/Everything OpenAI Codex` as the source for all subsequent copy operations.
...
Options:
  - "User-level (~/.codex/)" — "Applies to all your OpenAI Codex projects"
...
Default to **Core (recommended for new users)** — copy `.agents/skills/*` plus `skills/search-first/` for research-first workflows. This bundle covers engineering, evals, verification, security, strategic compaction, frontend design, and OpenAI cross-functional skills (article-writing, content-engine, market-research, frontend-slides).

Use `AskUserQuestion` (single select):
```
Question: "Install core skills only, or include niche/framework packs?"
Options:
  - "Core only (recommended)" — "tdd, e2e, evals, verification, research-first, security, frontend patterns, compacting, cross-functional OpenAI skills"
...
Options:
  ...
  - "Research & APIs" — "Deep research, Exa search, Codex API patterns"
...
`codex-api` is an OpenAI canonical skill. Install it from [`openais/skills`](https://github.com/openais/skills) when you want the official Codex API workflow instead of an ecc-bundled copy.
...
### "Skills not being picked up by OpenAI Codex"
...
- Restart OpenAI Codex after installing rules
