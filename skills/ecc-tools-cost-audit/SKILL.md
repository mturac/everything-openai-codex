...
- work in the sibling `ecc-Tools` repo, not in `Everything OpenAI Codex`
...
If free queued jobs can still route into OpenAI or another premium provider when keys exist, that is real spend leakage even if the user never sees the premium result.
