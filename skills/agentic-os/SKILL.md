---
name: agentic-os
description: Build persistent multi-agent operating systems on OpenAI Codex. Covers kernel architecture, specialist agents, slash commands, file-based memory, scheduled automation, and state management without external databases.
origin: ecc
---
...
```
project-root/
├── CODEX.md          # Kernel: identity, routing rules, agent registry
...
|---|---|---|
| Kernel (`CODEX.md`) | Identity, routing, model policies, agent registry | Git-tracked |
...
`CODEX.md` is the kernel. It acts as the COO / orchestrator. Codex reads it at session start and uses it to route work.

### Kernel Structure

```markdown
# CODEX.md - Agentic OS Kernel
...
For parallel execution, use OpenAI Codex's background task capability or shell scripts that invoke OpenAI Codex with specific agent contexts.
...
Place command files in `.codex/commands/<command-name>.md`. OpenAI Codex auto-discovers them. Users invoke them with `/<command-name>`.
...
Agentic OS tasks run on a schedule using external cron, not OpenAI Codex's built-in cron (which dies when the session ends).
...
Starting fresh every time OpenAI Codex opens.
```
...
```markdown
# BAD - API keys in agent files or CODEX.md
...
Keep routing declarative in `CODEX.md` markdown tables. It is inspectable, editable, and debuggable.

## Best Practices

- [ ] `CODEX.md` is under 200 lines and fits in context window
...
- [ ] Scheduled tasks use external cron (LaunchAgent, systemd, pm2), not OpenAI Codex's session cron
- [ ] Cost tracking: log API spend per session in `data/logs/<date>-costs.json`
- [ ] One project = one Agentic OS. Do not share a single `CODEX.md` across unrelated projects.
