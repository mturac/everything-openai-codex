...
4. **Review** — Delegates adversarial review to a strongest-model sub-agent (e.g., Deep) against a checklist and anti-pattern catalog. Fixes all critical findings before finalizing.
...
- **Zero runtime risk** — Pure Markdown skill. The entire repository contains only `.md` files — no hooks, no shell scripts, no executable code, no `package.json`, no build step. Nothing runs on install or invocation beyond OpenAI Codex's native Markdown skill loader.

## Installation

This skill ships with Everything OpenAI Codex. No separate installation is needed when ecc is installed.
...
- OpenAI Codex (for `/blueprint` slash command)
