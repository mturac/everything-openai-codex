 ...
 * init.mjs — auto-detect project info and output JSON for Codex to confirm
 *
 * Usage: node init.mjs
...
// ── CODEX.md ─────────────────────────────────────────────────────────────────
const codexMd = readFile('CODEX.md');
if (codexMd) {
  const goal = extractSection(codexMd, 'Current Goal');
  if (goal && !output.goal) output.goal = goal.split('\n')[0].trim();

  const doNot = extractSection(codexMd, 'Do Not Do');
  if (doNot) {
  ...
  const stack = extractSection(codexMd, 'Tech Stack');
  if (stack && output.stack.length === 0) {
    output.stack = stack.split(/[,\n]/).map(s => s.replace(/^[-*]\s+/, '').trim()).filter(Boolean);
  }

  // Description from first section or "What This Is"
  const whatItIs = extractSection(codexMd, 'What This Is') || extractSection(codexMd, 'About');
