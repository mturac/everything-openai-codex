 ...
 * - Goal mismatch detection vs CODEX.md
...
function extractCodexMdGoal(projectPath) {
  const p = resolve(projectPath, 'CODEX.md');
...
function main() {
  ...
  if (entry?.contextDir) {
    ...
    if (context) {
      ...
      const codexMdGoal = extractCodexMdGoal(cwd);
      if (codexMdGoal && context.goal &&
          codexMdGoal.toLowerCase().trim() !== context.goal.toLowerCase().trim()) {
        summaryLines.push(`WARNING Goal mismatch — ck: "${context.goal.slice(0, 40)}" · CODEX.md: "${codexMdGoal.slice(0, 40)}"`);
      ...
      // Instruct Codex to display compact briefing at session start
      parts.push([
        `---`,
        `## ck: SESSION START`,
        ``,
