...
wait_for_codex_analysis() {
  ...
  while true; do
    ...
    # SIGUSR1 can interrupt wait while the Codex child is still running.
...
analyze_observations() {
  ...
  if [ "${CLV2_IS_WINDOWS:-false}" = "true" ] && [ "${ecc_OBSERVER_ALLOW_WINDOWS:-false}" != "true" ]; then
    echo "[$(date)] Skipping codex analysis on Windows due to known non-interactive hang issue (#295). Set ecc_OBSERVER_ALLOW_WINDOWS=true to override." >> "$LOG_FILE"
    return
  fi
...
PROMPT
  ...
  # Prevent observe.sh from recording this automated Fast session as observations.
  ...
  ecc_SKIP_OBSERVE=1 ecc_HOOK_PROFILE=minimal codex --model fast --max-turns "$max_turns" --print \
  ...
  (
    ...
    if kill -0 "$codex_pid" 2>/dev/null; then
      echo "[$(date)] Codex analysis timed out after ${timeout_seconds}s; terminating process" >> "$LOG_FILE"
      kill "$codex_pid" 2>/dev/null || true
    fi
  ) &
  watchdog_pid=$!

  wait_for_codex_analysis "$codex_pid"
  exit_code=$?
