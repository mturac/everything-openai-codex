...
# OpenAI Codex passes hook data via stdin as JSON.
#
...
# wrapper's positional phase argument, but OpenAI Codex still exposes the hook
# event name in CODEX_HOOK_EVENT_NAME.  Fall back to that env var before
...
# Read JSON from stdin (OpenAI Codex hook format)
INPUT_JSON=$(cat)
...
if [ -n "$STDIN_CWD" ] && [ -d "$STDIN_CWD" ]; then
  ...
  export CODEX_PROJECT_DIR="${_GIT_ROOT:-$STDIN_CWD}"
fi
...
#   - ecc observing its own Fast observer sessions (self-loop)
...
_ecc_SKIP_PATHS="${ecc_OBSERVE_SKIP_PATHS:-observer-sessions,.codex-mem}"
if [ -n "$STDIN_CWD" ]; then
...
# Pass HOOK_PHASE via env var since OpenAI Codex does not include hook type in stdin JSON
...
try:
    ...
    # OpenAI Codex does NOT include a "hook_type" field in the stdin JSON,
    # so we rely on the shell argument ("pre" or "post") instead.
    hook_phase = os.environ.get("HOOK_PHASE", "post")
    event = "tool_start" if hook_phase == "pre" else "tool_complete"

    # Extract fields - OpenAI Codex hook format
