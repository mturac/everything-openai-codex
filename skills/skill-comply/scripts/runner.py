"""Run scenarios via codex -p and parse tool calls from stream-json output."""
...
ALLOWED_MODELS = frozenset({"fast", "standard", "deep"})
...
def run_scenario(
    ...
    # codex -p returns rc=1 when --max-turns is reached, but the stream-json
    ...
    if result.returncode != 0 and not nonfatal_max_turns:
        # Include both stderr and stdout tails. codex -p often surfaces the
        ...
        raise RuntimeError(
            f"codex -p failed (rc={result.returncode}): "
...
def _parse_stream_json(stdout: str) -> list[ObservationEvent]:
    """Parse codex -p stream-json output into ObservationEvents.
