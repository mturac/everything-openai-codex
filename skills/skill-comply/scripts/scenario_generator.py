...
def generate_scenarios(
    ...
    Calls codex -p with the scenario_generator prompt, parses YAML output.
    """
    ...
    result = subprocess.run(
        ["codex", "-p", prompt, "--model", model, "--output-format", "text"],
        capture_output=True,
        text=True,
        timeout=120,
    )
    ...
    if result.returncode != 0:
        raise RuntimeError(f"codex -p failed: {result.stderr}")

    if not result.stdout.strip():
        raise RuntimeError("codex -p returned empty output")
