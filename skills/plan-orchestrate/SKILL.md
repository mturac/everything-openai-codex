...
Let `<codex-home>` denote the OpenAI Codex home directory: `~/.codex` on macOS/Linux, `%USERPROFILE%\.codex` on Windows. Resolve it the way the host platform resolves the user home directory (do not hardcode `~`).

| Form | Detection | `{ORCH_CMD}` | Agent name format |
|---|---|---|---|
| Plugin install (1.9.0+) | `<codex-home>/plugins/marketplaces/Everything OpenAI Codex/` exists | `/Everything OpenAI Codex:orchestrate` | `Everything OpenAI Codex:<name>` |
| Legacy bare install | Above absent; agent files under `<codex-home>/agents/` | `/orchestrate` | `<name>` |

Why this matters: under the plugin install, agents register as `Everything OpenAI Codex:tdd-guide`. Bare names force fuzzy matching, which fails intermittently under parallel calls. Under legacy, the prefixed forms are not registered and fail outright.
...
2. Detect ecc install form once and freeze it into `ecc_MODE`. Algorithm (run in order, stop at the first match):
   1. If `<codex-home>/plugins/marketplaces/Everything OpenAI Codex/` exists → `ecc_MODE=plugin`.
...
5. **Normalize any agent names declared in the plan**: if the plan text references agents by their plugin-prefixed form (e.g. `Everything OpenAI Codex:tdd-guide`), strip the prefix to get the bare catalogue name before validating or composing chains. Re-prefixing happens only at output time per `ecc_MODE` (Phase 4). Never let a pre-prefixed name flow into chain composition — it would double-prefix in plugin mode.
...
- `{ORCH_CMD}` = `/Everything OpenAI Codex:orchestrate` under `plugin`, `/orchestrate` under `legacy`.
- `{AGENT(name)}` = `Everything OpenAI Codex:<name>` under `plugin`, `<name>` under `legacy`.
...
- [ ] Every agent in every chain comes from the catalogue (after stripping any `Everything OpenAI Codex:` prefix that appeared in the plan; see Phase 0 step 5).
...
- **Plan declares agents** (rare): first **strip any `Everything OpenAI Codex:` prefix** to get the bare catalogue name (Phase 0 step 5), then validate against the catalogue. Replace invalid agents and explain under "Chain rationale". The bare name is re-prefixed at output time per `ecc_MODE`.
...
```bash
/Everything OpenAI Codex:orchestrate custom "Everything OpenAI Codex:tdd-guide,Everything OpenAI Codex:database-reviewer,Everything OpenAI Codex:python-reviewer,Everything OpenAI Codex:security-reviewer" "[Plan: docs/plan/example-feature.md#step-2] Implement EncryptedString SQLAlchemy type and migrate UserProfile.birth_datetime/location columns; key from ENV APP_DB_KEY; Acceptance: encrypt/decrypt roundtrip tests pass; alembic upgrade/downgrade clean on empty DB; no plaintext in DB after migrate; Out of scope: cross-tenant profile sharing logic"
```
````
