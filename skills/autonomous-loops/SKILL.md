...
description: "Patterns and architectures for autonomous OpenAI Codex loops — from simple sequential pipelines to RFC-driven multi-agent DAG systems."
origin: ecc
---
...
Patterns, architectures, and reference implementations for running OpenAI Codex autonomously in loops. Covers everything from simple `codex -p` pipelines to full RFC-driven multi-agent DAG orchestration.
...
The `codex -p` flag runs OpenAI Codex non-interactively with a prompt, exits when done. Chain calls to build a pipeline:
...
### Implementation via OpenAI Codex Commands
...
**A production-grade shell script** that runs OpenAI Codex in a continuous loop, creating PRs, waiting for CI, and merging automatically. Created by AnandChowdhary (credit: @AnandChowdhary).
...
| Stage | Model | Purpose |
|-------|-------|---------|
| Research | Standard | Read codebase + RFC, produce context doc |
| Plan | Deep | Design implementation steps |
| Implement | Codex | Write code following the plan |
| Test | Standard | Run build + test suite |
| PRD Review | Standard | Spec compliance check |
...
4. **Ralphinho's tiered approach in simpler loops** — Even in a sequential pipeline, you can route simple tasks to Fast and complex tasks to Deep:
   ```bash
