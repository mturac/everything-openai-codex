...
|----------|----------------|
| [Hermes setup](docs/HERMES-SETUP.md) | Set up the Hermes x ecc operator workflow |
...
| [Security guide](the-security-guide.md) | Review sandboxing, hook risks, and AgentShield |
...
- **AgentShield integration** — `/security-scan` skill runs AgentShield directly from OpenAI Codex; package tests and the rule catalog are the source of truth
...
> AgentShield is the companion security-audit surface for ecc. Treat the package tests and rule catalog as the source of truth for current coverage.
...
# Deep analysis with three Deep 4.6 agents
...
**The `--deep` flag** runs three Codex Deep 4.6 agents in a red-team/blue-team/auditor pipeline. The attacker finds exploit chains, the defender evaluates protections, and the auditor synthesizes both into a prioritized risk assessment. Adversarial reasoning, not just pattern matching.
