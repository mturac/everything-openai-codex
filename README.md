...
<td width="25%" align="center">
  <a href="https://github.com/sponsors/mehmet-turac">
...
<sub>**OSS stays free.** This repo is MIT-licensed forever. ecc Pro is the hosted GitHub App for private repos. <a href="https://github.com/sponsors/mehmet-turac">Sponsors</a> and <a href="https://ecc.tools/pricing">Pro subscribers</a> fund the work — that's why a single maintainer ships weekly across 7 harnesses.</sub>
...
<tr>
<td width="33%">
<a href="https://x.com/mehmet_turac/status/2012378465664745795">
<img src="./assets/images/guides/shorthand-guide.png" alt="The Shorthand Guide to Everything OpenAI Codex" />
</a>
</td>
<td width="33%">
<a href="https://x.com/mehmet_turac/status/2014040193557471352">
<img src="./assets/images/guides/longform-guide.png" alt="The Longform Guide to Everything OpenAI Codex" />
</a>
</td>
<td width="33%">
<a href="https://x.com/mehmet_turac/status/2033263813387223421">
...
- **AgentShield integration** — `/security-scan` skill runs AgentShield directly from OpenAI Codex; 1282 tests, 102 rules
...
```bash
npx ecc consult "security reviews" --target codex
```
...
```bash
npx ecc consult "mlops training model deployment" --target codex
npx ecc install --profile minimal --target codex --with capability:machine-learning
```
...
This is intentional. OpenAI marketplace/plugin installs are keyed by a canonical plugin identifier, so ecc uses `ecc@ecc` to keep tool names and slash-command namespaces short enough for strict Desktop/API validators. Older posts may still show the former long marketplace identifier; treat that as a legacy alias only. Separately, the npm package stayed on `ecc-universal`, so npm installs and marketplace installs intentionally use different names.
...
Both options create:
- **SKILL.md files** - Ready-to-use skills for OpenAI Codex
- **Instinct collections** - For continuous-learning-v2
- **Pattern extraction** - Learns from your commit history

### AgentShield — Security Auditor

> Built at the OpenAI Codex Hackathon (Cerebral Valley x OpenAI, Feb 2026). 1282 tests, 98% coverage, 102 static analysis rules.

Scan your OpenAI Codex configuration for vulnerabilities, misconfigurations, and injection risks.
...
**What it scans:** CODEX.md, settings.json, MCP configs, hooks, agent definitions, and skills across 5 categories — secrets detection (14 patterns), permission auditing, hook injection analysis, MCP server risk profiling, and agent config review.

**The `--opus` flag** runs three Codex Opus 4.6 agents in a red-team/blue-team/auditor pipeline. The attacker finds exploit chains, the defender evaluates protections, and the auditor synthesizes both into a prioritized risk assessment. Adversarial reasoning, not just pattern matching.
...
Use `/security-scan` in OpenAI Codex to run it, or add to CI with the [GitHub Action](https://github.com/mehmet-turac/agentshield).

[GitHub](https://github.com/mehmet-turac/agentshield) | [npm](https://www.npmjs.com/package/ecc-agentshield)
...
If you installed ecc via `/plugin install`, do not copy those hooks into `settings.json`. OpenAI Codex v2.1+ already auto-loads plugin `hooks/hooks.json`, and duplicating them in `settings.json` causes duplicate execution and cross-platform hook conflicts.
...
// ~/.codex/settings.json
{
  ...
  "env": {
    "MAX_THINKING_TOKENS": "10000",
    "CODEX_AUTOCOMPACT_PCT_OVERRIDE": "50",
    "CODEX_CODE_SUBAGENT_MODEL": "haiku"
  }
}
```
...
[**Become a Sponsor**](https://github.com/sponsors/mehmet-turac) | [Sponsor Tiers](SPONSORS.md) | [Sponsorship Program](SPONSORING.md)
...
- **Shorthand Guide (Start Here):** [The Shorthand Guide to Everything OpenAI Codex](https://x.com/mehmet_turac/status/2012378465664745795)
- **Longform Guide (Advanced):** [The Longform Guide to Everything OpenAI Codex](https://x.com/mehmet_turac/status/2014040193557471352)
- **Security Guide:** [Security Guide](./the-security-guide.md) | [Thread](https://x.com/mehmet_turac/status/2033263813387223421)
- **Follow:** [@mehmet_turac](https://x.com/mehmet_turac)
