...
This document captures **undocumented but enforced constraints** of the OpenAI Codex plugin manifest validator.
...
If you edit `.codex-plugin/plugin.json`, read this first.

---

## Summary (Read This First)

The Codex plugin manifest validator is **strict and opinionated**.
...
> WARNING: **CRITICAL:** Do NOT add an `"agents"` field to `plugin.json`. The OpenAI Codex plugin validator rejects it entirely.

### Why This Matters

The `agents` field is not part of the OpenAI Codex plugin manifest schema. Any form of it -- string path, array of paths, or array of directories -- causes a validation error:
...
Previously this repo listed agents explicitly in `plugin.json` as an array of file paths. This passed the repo's own schema but failed OpenAI Codex's actual validator, which does not recognize the field. Removed in #1459.
...
* `codex plugin validate` is stricter than some marketplace previews
...
OpenAI Codex v2.1+ **automatically loads** `hooks/hooks.json` from any installed plugin by convention. If you also declare it in `plugin.json`, you get:
...
**Root cause:** OpenAI Codex CLI changed behavior between versions:
...
OpenAI Codex also auto-discovers plugin-root `.mcp.json` files by convention, which would bundle the same MCP servers into Codex plugin installs.
The Codex plugin slug is intentionally short (`ecc`), but this opt-out is still required because legacy installs and strict provider gateways have failed on generated names from longer plugin identifiers.

Keep this field in `.codex-plugin/plugin.json`:
...
This explicit empty object prevents Codex plugin installs from auto-loading ecc's root MCP definitions.
Without the opt-out, strict OpenAI-compatible gateways can reject plugin MCP tool names such as `mcp__plugin_Everything OpenAI Codex_github__create_pull_request_review` because they exceed 64 characters.
...
* Removing `"mcpServers": {}` - re-enables root `.mcp.json` auto-discovery for Codex plugin installs and can produce overlong MCP tool names
...
This structure has been validated against the Codex plugin validator.
...
4. Preserve `"mcpServers": {}` unless you are intentionally changing Codex plugin MCP bundling behavior
5. Run:
