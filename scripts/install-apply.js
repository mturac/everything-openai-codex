...
Usage: install.sh [--target <${LEGACY_INSTALL_TARGETS.join('|')}>] [--dry-run] [--json] <language> [<language> ...]
       ...
       install.sh [--target codex] [--dry-run] [--json] --locale <locale-code>
...
Options:
  ...
  --locale <code>     Install translated docs to ~/.codex/docs/<locale>/
                      (codex target only; can be combined with --profile or --with)
...
function main() {
  try {
    ...
    const plan = createInstallPlanFromRequest(request, {
      ...
      codexRulesDir: process.env.CODEX_RULES_DIR || null,
    });
