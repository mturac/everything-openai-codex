  ...
  for (const line of lines) {
    try {
      ...
      if (entry.type === 'user' || entry.role === 'user' || entry.message?.role === 'user') {
        // Support both direct content and nested message.content (OpenAI Codex JSONL format)
      ...
      // Extract tool uses from assistant message content blocks (OpenAI Codex JSONL format)
...
// Read hook input from stdin (OpenAI Codex provides transcript_path via stdin JSON)
...
async function main() {
  ...
  if (!transcriptPath) {
    const envTranscriptPath = process.env.CODEX_TRANSCRIPT_PATH;
  ...
  //
  // Without this, a parent session and any `codex -p ...` subprocess spawned by
