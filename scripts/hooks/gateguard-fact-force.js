 ...
 *
 * Forces Codex to investigate before editing files or running commands.
...
function resolveSessionKey(data) {
  const directCandidates = [data && data.session_id, data && data.sessionId, data && data.session && data.session.id, process.env.CODEX_SESSION_ID, process.env.ecc_SESSION_ID];
  ...
  const transcriptPath = (data && (data.transcript_path || data.transcriptPath)) || process.env.CODEX_TRANSCRIPT_PATH;
  if (transcriptPath && String(transcriptPath).trim()) {
    return hashSessionKey('tx', path.resolve(String(transcriptPath).trim()));
  }

  const projectFingerprint = process.env.CODEX_PROJECT_DIR || process.cwd();
...
function isCodexSettingsPath(filePath) {
  const normalized = normalizeForMatch(filePath);
  return /(^|\/)\.codex\/settings(?:\.[^/]+)?\.json$/.test(normalized);
}
...
function run(rawInput) {
  ...
  if (toolName === 'Edit' || toolName === 'Write') {
    ...
    if (!filePath || isCodexSettingsPath(filePath)) {
  ...
  if (toolName === 'MultiEdit') {
    ...
    for (const edit of edits) {
      ...
      if (filePath && !isCodexSettingsPath(filePath) && !isChecked(filePath)) {
        if (!markChecked(filePath)) {
          return allowWithStateWarning();
        }
