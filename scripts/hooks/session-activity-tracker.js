 ...
 * ~/.codex/metrics/tool-usage.jsonl for ecc2 metric sync.
 */
...
function buildActivityRow(input, env = process.env) {
  const hookEvent = String(env.CODEX_HOOK_EVENT_NAME || '').trim();
  if (hookEvent && hookEvent !== 'PostToolUse') {
    return null;
  }

  const toolName = String(input?.tool_name || '').trim();
  const sessionId = String(env.ecc_SESSION_ID || env.CODEX_SESSION_ID || '').trim();
...
function run(rawInput) {
  try {
    ...
    if (row) {
      appendFile(
        path.join(getCodexDir(), 'metrics', METRICS_FILE_NAME),
        `${JSON.stringify(row)}\n`
      );
    }
  } catch {
