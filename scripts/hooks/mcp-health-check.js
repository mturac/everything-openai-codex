 ...
 *
 * Compatible with OpenAI Codex's existing hook events:
...
// OpenAI Codex's stored OAuth bearer token. Treat auth-gated responses as
...
function stateFilePath() {
  ...
  return path.join(os.homedir(), '.codex', 'mcp-health-cache.json');
}
...
function configPaths() {
  ...
  return [
    path.join(cwd, '.codex.json'),
    path.join(cwd, '.codex', 'settings.json'),
...
async function handlePreToolUse(rawInput, input, target, statePathValue, now) {
  ...
  const reconnectSuffix = reconnect.attempted
    ...
    : '';
  logs.push(
    `[MCPHealthCheck] ${target.server} is unavailable (${probe.reason}). Blocking ${target.tool || 'tool'} so Codex can fall back to non-MCP tools.${reconnectSuffix}`
  );
...
async function main() {
  ...
  const eventName = process.env.CODEX_HOOK_EVENT_NAME || 'PreToolUse';
  const now = Date.now();
