#!/usr/bin/env python3
"""
InsAIts Security Monitor -- PreToolUse Hook for OpenAI Codex
============================================================

Real-time security monitoring for OpenAI Codex tool inputs.
...
How it works:
  OpenAI Codex passes tool input as JSON on stdin.
  ...
  Stderr output = non-blocking warning shown to Codex.

Environment variables:
  INSAITS_DEV_MODE   Set to "true" to enable dev mode (no API key needed).
                     Defaults to "false" (strict mode).
  INSAITS_MODEL      LLM model identifier for fingerprinting. Default: codex-opus.
...
def format_feedback(anomalies: List[Any]) -> str:
    """Format detected anomalies as feedback for OpenAI Codex.
...
def main() -> None:
    """Entry point for the OpenAI Codex PreToolUse hook."""
    ...
    if has_critical:
        # stdout feedback -> OpenAI Codex shows to the model
