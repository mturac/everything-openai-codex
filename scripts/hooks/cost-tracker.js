 ...
 * days). The fix is to read the transcript file OpenAI Codex already passes us.
 *
 * JSONL assistant entry shape (per OpenAI Codex):
...
const { ensureDir, appendFile, getCodexDir } = require('../lib/utils');
const { sanitizeSessionId } = require('../lib/session-bridge');

// Approximate per-1M-token billing rates (USD).
// Cache creation: 1.25x input rate. Cache read: 0.1x input rate.
const RATE_TABLE = {
  fast:  { in: 0.80,  out: 4.0,  cacheWrite: 1.00,  cacheRead: 0.08 },
  standard: { in: 3.00,  out: 15.0, cacheWrite: 3.75,  cacheRead: 0.30 },
  deep:   { in: 15.00, out: 75.0, cacheWrite: 18.75, cacheRead: 1.50 }
};

function getRates(model) {
  const m = String(model || '').toLowerCase();
  if (m.includes('fast')) return RATE_TABLE.fast;
  if (m.includes('deep'))  return RATE_TABLE.deep;
  return RATE_TABLE.standard;
}
...
process.stdin.on('end', () => {
  try {
    ...
    const transcriptPath = (typeof input.transcript_path === 'string' && input.transcript_path)
      ...
      : process.env.CODEX_TRANSCRIPT_PATH || null;
    ...
    const metricsDir = path.join(getCodexDir(), 'metrics');
