...
const { getCodexDir } = require('../lib/utils');
...
function readSessionCost(sessionId) {
  ...
  try {
    costsPath = path.join(getCodexDir(), 'metrics', 'costs.jsonl');
...
function run(rawInput) {
  try {
    ...
    const sessionId = sanitizeSessionId(input.session_id) || sanitizeSessionId(process.env.ecc_SESSION_ID) || sanitizeSessionId(process.env.CODEX_SESSION_ID);
