#!/usr/bin/env node
/**
 * NanoClaw v2 — Barebones Agent REPL for Everything OpenAI Codex
 *
 * Zero external dependencies. Session-aware REPL around `codex -p`.
 */
...
const DEFAULT_MODEL = process.env.CLAW_MODEL || 'standard';
...
function askCodex(systemPrompt, history, userMessage, model) {
  ...
  // On Windows, the `codex` binary installed via npm is `codex.cmd`.
  // Node's spawn() cannot resolve `.cmd` wrappers via PATH without shell: true,
  // so this call fails with `spawn codex ENOENT` on Windows otherwise.
  // 'codex' is a hardcoded literal here (not user input), so shell mode is safe.
  const result = spawnSync('codex', args, {
    encoding: 'utf8',
    stdio: ['pipe', 'pipe', 'pipe'],
    env: { ...process.env, CODEXCODE: '' },
    timeout: 300000,
...
function main() {
  ...
  const prompt = () => {
    rl.question('claw> ', (input) => {
      ...
      const response = askCodex(eccContext, history, line, state.model);
      console.log(`\n${response}\n`);
