...
function usage() {
  console.log([
    ...
    '  node scripts/loop-status.js --transcript ~/.codex/projects/-repo/session.jsonl'
  ].join('\n'));
}
...
function findTranscriptPaths(options = {}) {
  ...
  const transcriptRoot = path.join(homeDir, '.codex', 'projects');
...
function buildStatus(options = {}) {
  ...
  return {
    ...
    source: {
      ...
      transcriptRoot: path.join(homeDir, '.codex', 'projects'),
  ...
  if (payload.sessions.length === 0) {
    const lines = [
      ...
      skippedLines.length > 0
        ? 'No readable Codex transcript JSONL files were found.'
        : `No Codex transcript JSONL files found under ${payload.source.transcriptRoot}.`,
    ];
