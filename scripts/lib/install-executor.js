...
function planCodexLegacyInstall(context) {
  ...
  const rulesDir = context.codexRulesDir || path.join(targetRoot, 'rules', CODEX_ecc_NAMESPACE);
...
function createLegacyInstallPlan(options = {}) {
  ...
  const context = {
    ...
    codexRulesDir: options.codexRulesDir || process.env.CODEX_RULES_DIR || null,
  };
