...
function replacePluginRootPlaceholders(value, pluginRoot) {
  ...
  if (typeof value === 'string') {
    return value.split('${CODEX_PLUGIN_ROOT}').join(pluginRoot);
  }
...
function buildResolvedCodexHooks(plan) {
...
function applyInstallPlan(plan) {
  const resolvedCodexHooksPlan = buildResolvedCodexHooks(plan);
  ...
  if (resolvedCodexHooksPlan) {
    fs.mkdirSync(path.dirname(resolvedCodexHooksPlan.hooksDestinationPath), { recursive: true });
    fs.writeFileSync(
      resolvedCodexHooksPlan.hooksDestinationPath,
      JSON.stringify(resolvedCodexHooksPlan.resolvedHooksConfig, null, 2) + '\n',
      'utf8'
    );
  }
