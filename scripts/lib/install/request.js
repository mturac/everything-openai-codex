...
const LEGACY_INSTALL_TARGETS = ['codex', 'cursor', 'antigravity'];
...
function normalizeInstallRequest(options = {}) {
  ...
  const target = options.target || config?.target || 'codex';
  ...
  if (locale && target !== 'codex') {
    throw new Error('--locale can only be used with --target codex');
  }
