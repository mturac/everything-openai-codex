...
const { normalizeCodexHistorySession, persistCanonicalSnapshot } = require('./canonical-session');
...
function parseCodexTarget(target) {
  ...
  for (const prefix of ['codex-history:', 'codex:', 'history:']) {
...
function resolveSessionRecord(target, cwd) {
  ...
  if (explicitTarget) {
    if (explicitTarget === 'latest') {
      ...
      if (!latest) {
        throw new Error('No Codex session history found');
      }
    ...
    if (!session) {
      throw new Error(`Codex session not found: ${explicitTarget}`);
    }
...
function createCodexHistoryAdapter(options = {}) {
  const persistCanonicalSnapshotImpl = options.persistCanonicalSnapshotImpl || persistCanonicalSnapshot;

  return {
    id: 'codex-history',
    description: 'Codex local session history and session-file snapshots',
    targetTypes: ['codex-history', 'codex-alias', 'session-file'],
    canOpen(target, context = {}) {
      if (context.adapterId && context.adapterId !== 'codex-history') {
        return false;
      }

      if (context.adapterId === 'codex-history') {
        return true;
      }

      const cwd = context.cwd || process.cwd();
      return parseCodexTarget(target) !== null || isSessionFileTarget(target, cwd);
    },
    open(target, context = {}) {
      const cwd = context.cwd || process.cwd();
      ...
      return {
        ...
        getSnapshot() {
          ...
          const canonicalSnapshot = normalizeCodexHistorySession(session, sourceTarget);
