...
const { createCodexHistoryAdapter } = require('./codex-history');
...
const TARGET_TYPE_TO_ADAPTER_ID = Object.freeze({
  ...
  'codex-alias': 'codex-history',
  'session-file': 'codex-history'
});
...
function createDefaultAdapters(options = {}) {
  return [
    createCodexHistoryAdapter(buildDefaultAdapterOptions(options, 'codex-history')),
...
function normalizeStructuredTarget(target, context = {}) {
  ...
  if (type === 'codex-history' || type === 'codex-alias') {
    return {
      target: `codex:${value}`,
      context: nextContext
    };
