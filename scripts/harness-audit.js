...
function findPluginJsonUnder(installRoot) {
  const pluginJson = path.join(installRoot, '.codex-plugin', 'plugin.json');
...
function findPluginInstall(rootDir) {
  ...
  const pluginRoots = uniquePaths([
    ...
    ...homeDirs.map(homeDir => path.join(homeDir, '.codex', 'plugins')),
  ]);
  const installedPluginsPaths = uniquePaths([
    path.join(rootDir, '.codex', 'plugins', 'installed_plugins.json'),
    ...homeDirs.map(homeDir => path.join(homeDir, '.codex', 'plugins', 'installed_plugins.json')),
  ]);
...
function getConsumerChecks(rootDir) {
  ...
  const projectHooks = safeRead(rootDir, '.codex/settings.json');
  ...
  return [
    ...
    },
    {
      ...
      path: '.codex/',
      description: 'Project-specific harness overrides exist under .codex/',
      pass: countFiles(rootDir, '.codex/agents', '.md') > 0 ||
        countFiles(rootDir, '.codex/skills', 'SKILL.md') > 0 ||
        countFiles(rootDir, '.codex/commands', '.md') > 0 ||
        fileExists(rootDir, '.codex/settings.json') ||
        fileExists(rootDir, '.codex/hooks.json'),
      fix: 'Add project-local .codex hooks, commands, skills, or settings that tailor ecc to this repo.',
    },
    {
      ...
      pass: fileExists(rootDir, 'AGENTS.md') || fileExists(rootDir, 'CODEX.md') || fileExists(rootDir, '.codex/CODEX.md'),
      fix: 'Add AGENTS.md or CODEX.md so the harness has project-specific instructions.',
    },
    {
      ...
      scopes: ['repo', 'hooks'],
      path: '.mcp.json',
      description: 'The project declares local MCP or Codex settings',
      pass: fileExists(rootDir, '.mcp.json') || fileExists(rootDir, '.codex/settings.json') || fileExists(rootDir, '.codex/settings.local.json'),
      fix: 'Add .mcp.json or .codex/settings.json so project-local tool configuration is explicit.',
    },
    {
    ...
    },
    {
      ...
      pass: fileExists(rootDir, '.codex/memory.md') || countFiles(rootDir, 'docs/adr', '.md') > 0,
      fix: 'Add durable project memory such as .codex/memory.md or ADRs under docs/adr/.',
    },
    {
    ...
    },
    {
      ...
      pass: projectHooks.includes('PreToolUse') || projectHooks.includes('beforeSubmitPrompt') || fileExists(rootDir, '.codex/hooks.json'),
