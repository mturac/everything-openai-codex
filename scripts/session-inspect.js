...
function usage() {
  console.log([
    ...
    '  codex:latest        Most recent Codex session history entry',
    '  codex:<id|alias>    Specific Codex session or alias',
    '  <session.tmp>        Direct path to a Codex session file',
    ...
    '',
    'Examples:',
    '  node scripts/session-inspect.js .codex/plan/workflow.json',
    '  node scripts/session-inspect.js workflow-visual-proof',
    '  node scripts/session-inspect.js codex:latest',
    '  node scripts/session-inspect.js latest --target-type codex-history',
    '  node scripts/session-inspect.js skills:health',
    '  node scripts/session-inspect.js skills:amendify --skill api-design',
    '  node scripts/session-inspect.js codex:a1b2c3d4 --write /tmp/session.json'
  ].join('\n'));
}
