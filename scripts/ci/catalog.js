...
const PLUGIN_JSON_PATH = path.join(ROOT, '.codex-plugin', 'plugin.json');
const MARKETPLACE_JSON_PATH = path.join(ROOT, '.codex-plugin', 'marketplace.json');
...
function createDocumentSpecs(paths = {}) {
  ...
  return [
    ...
    },
    {
      ...
      parseExpectations: content => parseCatalogDescriptionExpectations(
        content,
        '.codex-plugin/marketplace.json plugin description',
...
function createDocumentSpecsForRoot(root) {
  return createDocumentSpecs({
    ...
    pluginJsonPath: path.join(root, '.codex-plugin', 'plugin.json'),
    marketplaceJsonPath: path.join(root, '.codex-plugin', 'marketplace.json'),
  });
}
