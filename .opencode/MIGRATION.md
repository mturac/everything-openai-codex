# Migration Guide: OpenAI Codex to OpenCode

This guide helps you migrate from OpenAI Codex to OpenCode while using the Everything OpenAI Codex (ecc) configuration.

## Overview

OpenCode is an alternative CLI for AI-assisted development that supports **all** the same features as OpenAI Codex, with some differences in configuration format.

## Key Differences

| Feature | OpenAI Codex | OpenCode | Notes |
|---------|-------------|----------|-------|
| Configuration | `CODEX.md`, `plugin.json` | `opencode.json` | Different file formats |
...
**OpenCode fully supports hooks** via its plugin system, which is actually MORE sophisticated than OpenAI Codex with 20+ event types.

### Hook Event Mapping

| OpenAI Codex Hook | OpenCode Plugin Event | Notes |
|-----------------|----------------------|-------|
...
**OpenAI Codex:** Skills are loaded from `skills/*/SKILL.md` files.
...
**OpenAI Codex:** Rules are in separate `rules/*.md` files.
...
|-------------|----------|
| `opus` | `openai/codex-opus-4-5` |
| `sonnet` | `openai/codex-sonnet-4-5` |
| `haiku` | `openai/codex-haiku-4-5` |
...
1. **Start Fresh**: Don't try to run both OpenAI Codex and OpenCode simultaneously
...
## Reverting to OpenAI Codex

If you need to switch back:

1. Simply run `codex` instead of `opencode`
2. OpenAI Codex will use its own configuration (`CODEX.md`, `plugin.json`, etc.)
3. The `.opencode/` directory won't interfere with OpenAI Codex
...
- **ecc Configuration**: Report to [github.com/mehmet-turac/everything-openai-codex](https://github.com/mehmet-turac/everything-openai-codex)
