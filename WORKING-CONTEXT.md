...
- Public plugin slug is now `ecc`; legacy `Everything OpenAI Codex` install paths remain supported for compatibility
...
- Closed on 2026-04-01 under backlog hygiene / merge policy:
  - `#1069` `feat: add Everything OpenAI Codex ecc bundle`
  - `#1068` `feat: add Everything OpenAI Codex-conventions ecc bundle`
  - `#1080` `feat: add Everything OpenAI Codex ecc bundle`
  - `#1079` `feat: add Everything OpenAI Codex-conventions ecc bundle`
...
- Direct-port candidates landed after audit:
  - `#1078` hook-id dedupe for managed Codex hook reinstalls
  - `#844` ui-demo skill
  - `#1110` install-time Codex hook root resolution
...
- 2026-04-01: Follow-up bundle cleanup closed `#1080` and `#1079`, which were generated `.codex/` bundle PRs duplicating command-first scaffolding instead of shipping canonical ecc source changes.
...
- 2026-04-01: Direct-ported the real fix from the unresolved hook-path PR lane into the active installer. Codex installs now replace `${CODEX_PLUGIN_ROOT}` with the concrete install root in both `settings.json` and the copied `hooks/hooks.json`, which keeps PreToolUse/PostToolUse hooks working outside plugin-managed env injection.
...
- 2026-04-02: Applied the same consolidation rule to the writing lane. `brand-voice` remains the canonical voice system, while `content-engine`, `crosspost`, `article-writing`, and `investor-outreach` now keep only workflow-specific guidance instead of duplicating a second mehet-turac/ecc voice model or repeating the full ban list in multiple places.
- 2026-04-02: Closed fresh auto-generated bundle PRs `#1182` and `#1183` under the existing policy. Useful ideas from generator output must be ported manually into canonical repo surfaces instead of merging `.codex`/bundle PRs wholesale.
...
- 2026-04-02: Adapted the design-quality reminder from `#1127` into the current ecc hook architecture with a local `scripts/hooks/design-quality-check.js`, Codex `hooks/hooks.json` wiring, Cursor `after-file-edit.js` wiring, and dedicated hook coverage in `tests/hooks/design-quality-check.test.js`.
