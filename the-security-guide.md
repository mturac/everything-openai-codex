...
_everything.openai.codex / research / security_
...
Widespread adoption of open source agents is here. OpenClaw and others run about your computer. Continuous run harnesses like OpenAI Codex and Codex (using ecc) increase the surface area; and on February 25, 2026, Check Point Research published a OpenAI Codex disclosure that should have ended the "this could happen but won't / is overblown" phase of the conversation for good. With the tooling reaching critical mass, the gravity of exploits multiplies.

One issue, CVE-2025-59536 (CVSS 8.7), allowed project-contained code to execute before the user accepted the trust dialog. Another, CVE-2026-21852, allowed API traffic to be redirected through an attacker-controlled `OPENAI_BASE_URL`, leaking the API key before trust was confirmed. All it took was that you clone the repo and open the tool.
...
## OpenAI Codex CVEs (February 2026)

Check Point Research published the OpenAI Codex findings on February 25, 2026. The issues were reported between July and December 2025, then patched before publication.
...
>
> Hijacking OpenAI Codex users via poisoned config files with rogue hooks actions.
>
...
> _I hacked OpenAI Codex! It turns out "agentic" is just a fancy new way to get a shell. I achieved full RCE and hijacked organization API keys. CVE-2025-59536 | CVE-2026-21852_
...
**CVE-2026-21852.** An attacker-controlled project could override `OPENAI_BASE_URL`, redirect API traffic, and leak the API key before trust confirmation. NVD says manual updaters should be on `2.0.65` or later.
...
OpenAI's own docs reflect that reality. Project settings live in `.codex/`. Project-scoped MCP servers live in `.mcp.json`. They are shared through source control. They are supposed to be guarded by a trust boundary. That trust boundary is exactly what attackers will go after.

## What Changed In The Last Year

This conversation moved fast in 2025 and early 2026.

OpenAI Codex had its repo-controlled hooks, MCP settings, and env-var trust paths tested publicly. Amazon Q Developer had a 2025 supply chain incident involving a malicious prompt payload in the VS Code extension, then a separate disclosure around overly broad GitHub token exposure in build infrastructure. Weak credential boundaries plus agent-adjacent tooling is an entrypoint for opportunists.
...
| Stat | Detail |
|------|--------|
| **CVSS 8.7** | OpenAI Codex hook / pre-trust execution issue: CVE-2025-59536 |
...
Root access is dangerous. Broad local access is dangerous. Long-lived credentials on the same machine are dangerous. "YOLO, Codex has me covered" is not the correct approach to take here. The answer is isolation.
...
For untrusted repos, attachment-heavy workflows, or anything that pulls lots of foreign content, run it in a container, VM, devcontainer, or remote sandbox. OpenAI explicitly recommends containers / devcontainers for stronger isolation. OpenAI's Codex guidance pushes the same direction with per-task sandboxes and explicit network approval. The industry is converging on this for a reason.
...
If you cannot see what the agent read, what tool it called, and what network destination it tried to hit, you cannot secure it (this should be obvious, yet I see you guys hit codex --dangerously-skip-permissions on a ralph loop and just walk away without a care in the world). Then you come back to a mess of a codebase, spending more time figuring out what the agent did than getting any work done.
...
OpenAI documents that OpenAI Codex loads memory at session start. So keep memory narrow:
...
OpenAI has hardened OpenAI Codex and published concrete security guidance around trust, permissions, MCP, memory, hooks, and isolated environments.
...
Scan your setup: [github.com/mehmet-turac/agentshield](https://github.com/mehmet-turac/agentshield)

---

## References

- Check Point Research, "Caught in the Hook: RCE and API Token Exfiltration Through OpenAI Codex Project Files" (February 25, 2026): [research.checkpoint.com](https://research.checkpoint.com/2026/rce-and-api-token-exfiltration-through-codex-project-files-cve-2025-59536/)
...
> [The Shorthand Guide to Everything OpenAI Codex](https://x.com/mehmet_turac/status/2012378465664745795)
>
> [The Longform Guide to Everything OpenAI Codex](https://x.com/mehmet_turac/status/2014040193557471352)
